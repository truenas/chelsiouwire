...
#include "cxgbtool_coiscsi_stor.h"
...
static void __attribute__((noreturn)) usage(FILE *fp)
{
	...
	fprintf(fp,
		...
		"\tdiag cim_mem <start|stop|status>\n"
		"\t     [duration <sec>]               manage cim_mem diag test\n"
#endif
		...
		"\tfec [<fec-param>] ...               display or set advertised Forward Error Correction parameters\n"
		...
		"\treadboot-cfg			       read boot settings\n"
		"\tmodifyboot-cfg                      modify boot settings\n"
		"\t      [bios <decimal/hexadecimal>] \n"
		"\t      [vlan <vlan id> port <port no>]\n"
		"\t      [en_nicboot port <port no>]\n"
		"\t      [dis_nicboot port <port no>]\n"
		...
		"\tsend-workreq <qid> <byte> [<byte> ...]  send Work Request on specified Queue\n"
		...
		"\tmpstrace <index> rx|tx|loopback|off\n"
		"\t         [<channel>] [not]\n"
		"\t	    [<param> <val>] ...        write MPS trace parameters\n"
		"\tmpstrace <index>                    read MPS trace parameters\n"
		"\tioqs                                dump uP ioqs\n"
...
#ifdef STORAGE
		"\n[ Storage ]\n\n"
		"\tstor --chnet                        access CHNET   subcommands\n"
#ifdef __CSIO_FOISCSI_ENABLED__
		"\tstor --foiscsi                      access FOiSCSI subcommands\n"
#endif
#ifdef __CSIO_COISCSI_ENABLED__
		"\tstor --coiscsi                      access COiSCSI subcommands\n"
		"\tstor --isns                         access iSNS    subcommands\n"
#endif
#endif
		);
	exit(fp == stderr ? 1 : 0);
...
static int get_arg_index(int argc, char *argv[], char *key)
{
	int i, ret = -1;
	...
	for (i = 1; i < argc; i++) {
		if (!strcmp(argv[i], key)) {
			if (ret == -1)
				...
				errx(1, "%s arg provided multiple times\n", key);
		}
	}
			 ...
			 const char *iff_name)
{
	...
	if (!strcmp(argv[start_arg], "eth") ||
	...
	else if (!strcmp(argv[start_arg], "crypto"))
		type = SGE_QTYPE_TX_CRYPTO;
		...
		if (!isdigit(*argv[start_arg]) ||
			...
			warnx("Possible Queue Types are: "
			      "eth, ofld, crypto, ctrl, fl and rsp\n");
			return -1;
		}
	}
...
/*
 * Display or set advertised Forward Error Correction parameters for a Link.
 */
static void dump_fec_config(const char *s, uint32_t fec_config)
{
	printf("%s", s);
	if (fec_config & FEC_TYPE_AUTO)
		printf(" auto (IEEE 802.3)");
	if (fec_config & FEC_TYPE_OFF)
		printf(" no FEC");
	if (fec_config & FEC_TYPE_BASER_RS)
		printf(" Base-R/Reed-Solomon");
	if (fec_config & FEC_TYPE_RS)
		printf(" Reed-Solomon");
	putchar('\n');
}

static int fec_config(int argc, char *argv[], int start_arg,
		      const char *iff_name)
{
	struct ch_fec_config op;

	if (argc == start_arg) {
		op.cmd = CHELSIO_GET_FEC;
		if (doit(iff_name, &op) < 0)
			err(1, "get FEC");
		dump_fec_config("supported:        ", op.supported_fec);
		dump_fec_config("IEEE 802.3 'auto':", op.supported_fec);
		dump_fec_config("requested:        ", op.requested_fec);
		dump_fec_config("actual:           ", op.actual_fec);
		return 0;
	}

	memset(&op, 0, sizeof op);
	op.cmd = CHELSIO_SET_FEC;
	while (start_arg < argc) {
		if (!strcasecmp(argv[start_arg], "auto"))
			op.requested_fec |= FEC_TYPE_AUTO;
		else if (!strcasecmp(argv[start_arg], "off"))
			op.requested_fec |= FEC_TYPE_OFF;
		else if (!strcasecmp(argv[start_arg], "baser"))
			op.requested_fec |= FEC_TYPE_BASER_RS;
		else if (!strcasecmp(argv[start_arg], "rs"))
			op.requested_fec |= FEC_TYPE_RS;
		else
			errx(1, "set FEC: invalid argument \"%s\"",
			     argv[start_arg]);
		start_arg++;
	}

	/*
	 * If either AUTO or OFF are set, nothing else may be set.
	 */
	if (((op.requested_fec & FEC_TYPE_AUTO) &&
	     (op.requested_fec & ~FEC_TYPE_AUTO)) ||
	    ((op.requested_fec & FEC_TYPE_OFF) &&
	     (op.requested_fec & ~FEC_TYPE_OFF)))
		errx(1, "\"auto\" or \"off\" must be used alone");

	if (!op.requested_fec)
		errx(1, "type of FEC must be selected");
		       ...
		       const char *iff_name)
{
	...
	op.polling = op.lro = op.intr_lat = op.cong_thres = op.cong_mode = -1;
	...
	while (start_arg + 2 <= argc) {
		...
		if (!strcmp(argv[start_arg], "txq0"))
			...
			param = &op.lro;
		else if (!strcmp(argv[start_arg], "congmode"))
			param = &op.cong_mode;
		else
			   ...
			   const char *iff_name)
{
	...
	else if (!strcmp(argv[start_arg], "iscsit"))
		r = qtype_num_config(argc, argv,
				start_arg+1, iff_name, QTYPE_ISCSIT);
	else if (!strcmp(argv[start_arg], "crypto"))
		r = qtype_num_config(argc, argv,
				start_arg+1, iff_name, QTYPE_CRYPTO);
	else if (!strcmp(argv[start_arg], "cryptot"))
		r = qtype_num_config(argc, argv,
				start_arg+1, iff_name, QTYPE_CRYPTOT);
	else
		warnx("Invalid type \"%s\": Valid types are "
			"\"eth|ofld|rdma|rciq|iscsi|iscsit|crypto\"\n",
			argv[start_arg]);
...
/*
 * Send an arbitrary Work Request on an arbitrary Queue.  Caution: This is
 * insanely dangerous!!  It's trivial to hang the chip or cause it to DMA
 * to/from arbitrary memory areas.
 */
static int send_workreq(int argc, char *argv[], int start_arg,
			const char *iff_name)
{
	struct ch_workreq op;
	unsigned int val;
	int i;

	/*
	 * We need a Queue ID and at least 1 byte to send.  (In true, we
	 * probably need a lot more than 1 byte and there are also probably
	 * all sorts of constraints on the number of bytes being some multiple
	 * of a base unit, say 16, and innumerable internal constraints on
	 * that length being represented in some manner internally, but it's
	 * not cxgbtool's job to analyze any of that since it's "Adapter
	 * Agnostic".  It's the job of the Host Driver to perform any further
	 * vetting it wants to do before handing this likely toxic workload to
	 * the hardware ...
	 */
	if (argc - start_arg < 2)
		errx(1, "send-workreq: missing Queue ID and at least 1 byte");

	op.cmd = CHELSIO_SEND_WORKREQ;
	if (get_int_arg(argv[start_arg], &val))
		errx(1, "send-workreq: invalid Queue ID \"%s\"",
		     argv[start_arg]);
	op.qid = val;
	start_arg++;

	for (i = 0; start_arg < argc && i < MAX_WORKREQ; i++, start_arg++) {
		if (get_int_arg(argv[start_arg], &val) || (val & ~0xff))
			errx(1, "send-workreq: bad Work Request byte \"%s\"",
			     argv[start_arg]);
		op.workreq[i] = val;
	}
	op.len = i;
	if (start_arg < argc) {
		errx(1, "send-workreq: too many Work Request bytes %d (max %d)",
		     op.len + (argc - start_arg), MAX_WORKREQ);
	}
...
static int parse_macaddr(const char *param, char *args[],
			 uint8_t addr[], uint8_t mask[])
{
	struct ether_addr *paddr, *pmask;
	char *slash;
	...
	if (slash)
		*slash = 0;
	paddr = ether_aton( args[1]);
	if (slash)
		*slash = '/';
	if (paddr == NULL)
		errx(1, "Cannot parse %s address %s", param, args[1]);

	memcpy(addr, paddr, ETH_ALEN);
	if (slash)
		pmask = ether_aton(slash+1);
	else {
		char zeroaddr[ETH_ALEN] = {0,0,0,0,0,0};

		/* No mask supplied. Check the value.
		 * If value is non-zero, make it exact-match in absence
		 * of mask
		 */
		if (memcmp(paddr, &zeroaddr, ETH_ALEN) == 0)
			pmask = ether_aton("00:00:00:00:00:00");
		else
			pmask = ether_aton("ff:ff:ff:ff:ff:ff");
	}

	if (pmask == NULL)
		errx(1, "Cannot parse %s address %s", param, args[1]);

	memcpy(mask, pmask, ETH_ALEN);
	return 1;
}
	...
	if (slash) {
		...
		if (prefix > masksize)
			errx(1, "prefix %u is too long for an %s address",
			     prefix, afn);
		if (!mask)
			errx(1, "no memory available to hold mask for %s",
			     param);
...
driver_file(int argc, char *argv[], int start_arg, const char *iff_name)
{
	...
	if (argc < 4) {
		snprintf(proc_cmd, sizeof(proc_cmd),
			 "ls "PROC_PATH"%s/ 2>/dev/null",
			 bus_slot_func);
		snprintf(debug_cmd, sizeof(debug_cmd),
			 "ls "DRIVER_PATH"%s/ 2>/dev/null",
			 bus_slot_func);

		if (system(proc_cmd) && system(debug_cmd))
			 ...
			 const char *iff_name)
{
	...
	while (start_arg + 2 <= argc) {
		...
		if (!strcmp(argv[start_arg], "type")) {
		...
		} else if (parse_val_mask("encap_vni", args, &val, &mask,
					  ENCAP_VNI_BITWIDTH, op.fs.cap)) {
			op.fs.val.vni = val;
			op.fs.mask.vni = mask;
			op.fs.val.encap_vld = 1;
			op.fs.mask.encap_vld = 1;
		} else if (parse_macaddr("encap_inner_mac", args,
					 op.fs.val.encap_inner_mac,
					 op.fs.mask.encap_inner_mac)) {
			op.fs.val.encap_vld = 1;
			op.fs.mask.encap_vld = 1;
		} else if (parse_val_mask("encap_matchtype", args, &val, &mask,
					  MATCHTYPE_BITWIDTH, op.fs.cap)) {
			op.fs.val.encap_matchtype = val;
			op.fs.mask.encap_matchtype = mask;
			op.fs.val.encap_vld = 1;
			op.fs.mask.encap_vld = 1;
		} else if (parse_val_mask("encap_lookup", args, &val, &mask,
					  ENCAP_LOOKUP_BITWIDTH, op.fs.cap)) {
			op.fs.val.encap_lookup = val;
			op.fs.mask.encap_lookup = mask;
			op.fs.val.encap_vld = 1;
			op.fs.mask.encap_vld = 1;
		} else if (parse_val("drop_encap_hdr", args, &val)) {
			op.fs.drop_encap_hdr = 1;
		...
		} else if (parse_val_mask("lport", args, &val, &mask,
					  L4_PORT_BITWIDTH, op.fs.cap)) {
			op.fs.val.lport = val;
			op.fs.mask.lport = mask;
		} else if (parse_val_mask("fport", args, &val, &mask,
					  L4_PORT_BITWIDTH, op.fs.cap)) {
			op.fs.val.fport = val;
			...
			/*nada*/;
			   ...
			   op.fs.cap)) {
			/*nada*/;
		} else if (parse_ipaddr("nat_lip", args, &af, op.fs.nat_lip, NULL, 1)) {
			/*nada*/;
		} else if (parse_ipaddr("nat_fip", args, &af, op.fs.nat_fip, NULL, 1)) {
			/*nada*/
		} else if (parse_val_mask("nat_lport", args, &val,
					  &mask, L4_PORT_BITWIDTH, 1)) {
			op.fs.nat_lport = val;
		} else if (parse_val_mask("nat_fport", args, &val,
					  &mask, L4_PORT_BITWIDTH, 1)) {
			op.fs.nat_fport = val;
			...
			if (!strcmp(argv[start_arg + 1], "dip"))
				op.fs.nat_mode = NAT_MODE_DIP;
			else if (!strcmp(argv[start_arg + 1], "dip-dp"))
				op.fs.nat_mode = NAT_MODE_DIP_DP;
			else if (!strcmp(argv[start_arg + 1], "dip-dp-sip"))
				op.fs.nat_mode = NAT_MODE_DIP_DP_SIP;
			else if (!strcmp(argv[start_arg + 1], "dip-dp-sp"))
				op.fs.nat_mode = NAT_MODE_DIP_DP_SP;
			else if (!strcmp(argv[start_arg + 1], "sip-sp"))
				op.fs.nat_mode = NAT_MODE_SIP_SP;
			else if (!strcmp(argv[start_arg + 1], "dip-sip-sp"))
				op.fs.nat_mode = NAT_MODE_DIP_SIP_SP;
			else if (!strcmp(argv[start_arg + 1], "all"))
				op.fs.nat_mode = NAT_MODE_ALL;
			else {
				errx(1, "unknown nat type \"%s\"; known types are dip, "
				      "dip-dp, dip-dp-sip, dip-dp-sp, sip-sp and "
				      "dip-sip-sp", argv[start_arg + 1]);
				return -1;
			}
		} else if (parse_val("natseq", args, &val)) {
			op.fs.nat_seq_chk = val;
		} else if (parse_val("natflag", args, &val)) {
			op.fs.nat_flag_chk = 1;
	...
	if (op.fs.action != FILTER_SWITCH &&
	    (op.fs.eport || op.fs.newdmac || op.fs.newsmac || op.fs.newvlan ||
	     op.fs.swapmac || op.fs.nat_mode))
		errx(1, "port, dmac, smac, swapmac and vlan only make"
	...
	if ((op.fs.nat_seq_chk || op.fs.nat_flag_chk || *op.fs.nat_lip ||
	     *op.fs.nat_fip || op.fs.nat_lport || op.fs.nat_fport) &&
	    !op.fs.nat_mode)
		errx(1, "nat params only make sense with valid nat mode");

	if (op.fs.cap && (!*op.fs.val.lip || !*op.fs.val.fip ||
	    !op.fs.val.lport || !op.fs.val.fport || !op.fs.val.proto))
		errx(1, "lip,fip,lport,fport and proto are mandatory with "
			"cap maskless specification");
...
static inline void tokenise(char *filter_arg, char **filter_argv)
{
	char *token = strtok(filter_arg, " ");
	int j = 0;

	memset(filter_arg, 0, sizeof(*filter_arg));
	while (token) {
		filter_argv[j] = token;
		token = strtok(NULL, " ");
		j++;
...
#define FILTER_RINGBB_SMAC_DROP 0
#define FILTER_RINGBB_SMAC_MVI_DROP 1
#define FILTER_RINGBB_VI_SWITCH 2
#define FILTER_RINGBB_MVI_SWITCH 3

#define SMAC_MPS_START 256

static inline int
tokenise_set_rbb_filter(char *argv[], const char *iff_name, int entry_idx,
			int type, int f_type, int pf, int vi, int mvi, int hits)
{
	struct ch_filter_id op;
	char *filter_argv[30];
	char filter_arg[256];
	char rule_str[25];
	int argc = 0;
	...
	op.type = type;
	op.cmd = CHELSIO_GET_FREE_FILTER_ID;
	/* All filters created in high priority region */
	op.prio = 1;

	if (doit(iff_name, &op) < 0)
		err(1, "Get Filter ID failed");
	switch (f_type) {
	case FILTER_RINGBB_SMAC_DROP:
		snprintf(rule_str, sizeof(rule_str), "%-20s",
			 type == FILTER_TYPE_IPV6 ? "SMAC_DROP_IPV6"
						  : "SMAC_DROP");
		if (type == FILTER_TYPE_IPV6)
			snprintf(filter_arg, sizeof(filter_arg),
				 "%s %s filter %d action drop iport 0 type ipv6 macidx %d pf %d vf %d hitcnts %d prio 1",
				 argv[0], iff_name, op.filter_id, entry_idx,
				 pf, vi, hits);
		else
			snprintf(filter_arg, sizeof(filter_arg),
				 "%s %s filter %d action drop iport 0 macidx %d pf %d vf %d hitcnts %d prio 1",
				 argv[0], iff_name, op.filter_id, entry_idx,
				 pf, vi, hits);
		argc = (type == FILTER_TYPE_IPV6) ? 20 : 18;
		break;
	case FILTER_RINGBB_SMAC_MVI_DROP:
		snprintf(rule_str, sizeof(rule_str), "%-20s",
			 type == FILTER_TYPE_IPV6 ? "SMAC_MVI_DROP_IPV6"
						  : "SMAC_MVI_DROP");
		if (type == FILTER_TYPE_IPV6)
			snprintf(filter_arg, sizeof(filter_arg),
				 "%s %s filter %d action drop iport 0 type ipv6 macidx %d pf %d vf %d hitcnts %d prio 1",
		...
		argc = (type == FILTER_TYPE_IPV6) ? 20 : 18;
		break;
	case FILTER_RINGBB_VI_SWITCH:
		snprintf(rule_str, sizeof(rule_str), "%-20s",
			 type == FILTER_TYPE_IPV6 ? "SWITCH_IPV6"
						  : "SWITCH");
		if (type == FILTER_TYPE_IPV6)
			snprintf(filter_arg, sizeof(filter_arg),
				 "%s %s filter %d action switch iport 0 eport 1 type ipv6 pf %d vf %d matchtype 4 hitcnts %d prio 1",
				 argv[0], iff_name, op.filter_id,
				 pf, vi, hits);
		else
			snprintf(filter_arg, sizeof(filter_arg),
				 "%s %s filter %d action switch iport 0 eport 1 pf %d vf %d matchtype 4 hitcnts %d prio 1",
				 argv[0], iff_name, op.filter_id,
				 pf, vi, hits);
		argc = (type == FILTER_TYPE_IPV6) ? 22 : 20;
		break;
	case FILTER_RINGBB_MVI_SWITCH:
		snprintf(rule_str, sizeof(rule_str), "%-20s",
			 type == FILTER_TYPE_IPV6 ? "SWITCH_MVI_IPV6"
						  : "SWITCH_MVI");
		if (type == FILTER_TYPE_IPV6)
			snprintf(filter_arg, sizeof(filter_arg),
				 "%s %s filter %d action switch iport 0 eport 1 type ipv6 pf %d vf %d hitcnts %d prio 1",
				 argv[0], iff_name, op.filter_id,
				 pf, mvi, hits);
		else
			snprintf(filter_arg, sizeof(filter_arg),
				 "%s %s filter %d action switch iport 0 eport 1 pf %d vf %d hitcnts %d prio 1",
				 argv[0], iff_name, op.filter_id,
				 pf, mvi, hits);
		argc = (type == FILTER_TYPE_IPV6) ? 20 : 18;
		break;
	...
	printf("Setting rule for %s : %s\n", rule_str, filter_arg);
	tokenise(filter_arg, filter_argv);
	if (filter_config(argc, (char **)filter_argv, 3, iff_name))
		err(1, "Filter rule %s failed", filter_arg);
	return 0;
}

static void set_mps(const char *iff_name, uint8_t *mac,
		    unsigned int lookup_type, unsigned int mac_type,
		    unsigned int mps_idx)
{
	struct ch_mps op;
	u8 mac_mask[] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};

	op.cmd = CHELSIO_SET_MPS;

	switch (mac_type) {
	case RAW_MAC:
		op.idx = mps_idx;
		memcpy(op.mac, mac, ETH_ALEN);
		memcpy(op.mac_mask, mac_mask, ETH_ALEN);
		op.lookup_type = lookup_type;
		break;
	default:
		errx(1, "Invalid value for %s", __func__);
	}
...
#define NETPATH "/sys/class/net/"
static int
configure_ringbb(int argc, char *argv[], int start_arg, const char *iff_name)
{
	FILE *fp, *fp1;
	char bus_slot_func[ETHTOOL_BUSINFO_LEN];
	char zero_addr[] = "00:00:00:00:00:00";
	char mps_dbg_file[PATH_MAX];
	char dev_port_file[PATH_MAX];
	char entry[256], ipcmd[256];
	char *p, *token;
	int viid_pos = 0, entry_idx, viid_entry, dev_port;
	unsigned int pf, vi, mvi, hits = 0, tun = 0;
	int i, j, pos, dmac_mps_idx = 0, smac_mps_idx = 0;

	if (get_adapter_ver(iff_name) < 5)
		errx(1, "%s is not an interface with Chelsio T5 or greater",
		     iff_name);

	pos = start_arg;
	while (pos + 2 <= argc) {
		if (!strcmp(argv[pos], "pf")) {
			if (get_int_arg(argv[pos + 1], &pf))
				errx(1, "Invalid pf value");
		} else if (!strcmp(argv[pos], "vi")) {
			if (get_int_arg(argv[pos + 1], &vi))
				errx(1, "Invalid vi value");
			viid_pos = pos + 1;
		} else if (!strcmp(argv[pos], "mvi")) {
			if (get_int_arg(argv[pos + 1], &mvi))
				errx(1, "Invalid mvi value");
		} else if (!strcmp(argv[pos], "hitcnts")) {
			if (get_int_arg(argv[pos + 1], &hits))
				errx(1, "Invalid hitcnts value");
		} else if (!strcmp(argv[pos], "tunnel")) {
			tun = 1;
		}
		pos += 2;
	}

	snprintf(dev_port_file, sizeof(dev_port_file), NETPATH"%s/dev_port",
		 iff_name);
	fp = fopen(dev_port_file, "r");
	if (!fp) {
		warn("%s %s %s", "Opening dev_port sysfs file failed..",
		     "This can be due to a lower kernel(< 3.15)",
		     "Please ensure you specified port 0 interface for cxgbtool");
	} else {
		if (fscanf(fp, "%d", &dev_port) == EOF)
			errx(1, "Error in parsing dev_port file");
		/* User specified an interface other than port 0, exit.. */
		if (dev_port)
			errx(1, "Please specify port0 interface");
	}
	fclose(fp);

	snprintf(ipcmd, sizeof(ipcmd), "ip link set dev %s up promisc on",
		 iff_name);
	if (system(ipcmd) < 0)
		errx(1, "Unable to change link parameters for %s", iff_name);

	get_pci_bus_slot_func(iff_name, bus_slot_func, sizeof(bus_slot_func));
	snprintf(mps_dbg_file, sizeof(mps_dbg_file),
		 DRIVER_PATH"%s/mps_tcam", bus_slot_func);
	fp = fopen(mps_dbg_file, "r");
	if (!fp)
		err(1, "Cannot open %s", mps_dbg_file);

	printf("%s\n%s\n%s\n%s\n%s\n\n%s", "Ensure that you have:",
	...
	/* Determine the first free entry in mps DMAC/SMAC regions
	 * to insert the tunnel interface mac
	 */
	i = -1;
	while (fgets(entry, sizeof(entry), fp)) {
		...
		p = strchr(entry, '-');
		if (p && !strcmp(p, "-\n")) {
			if (((i-1) < SMAC_MPS_START) && !dmac_mps_idx) {
				dmac_mps_idx = i - 1;
				continue;
			}
			if ((i-1) >= SMAC_MPS_START) {
				smac_mps_idx = i - 1;
				break;
			}
		}
	}
	if (!dmac_mps_idx || !smac_mps_idx)
		errx(1, "Unable to get mps index for tunnelling support");

	pos = start_arg;
	while (pos + 2 <= argc) {
		struct ether_addr *paddr;
		char tunif_file[PATH_MAX];
		char addr[20];
		uint8_t maddr[ETH_ALEN];

		if (!strcmp(argv[pos], "tunnel")) {
			snprintf(tunif_file, sizeof(tunif_file),
				 NETPATH"%s/address", argv[pos + 1]);
			fp1 = fopen(tunif_file, "r");
			if (!fp)
				errx(1, "Unable to get %s mac address from %s",
				     argv[pos + 1], tunif_file);
			if (!(fgets(addr, sizeof(addr), fp1)))
				errx(1, "Error in parsing %s file", tunif_file);
			paddr = ether_aton(addr);
			if (paddr == NULL)
				errx(1, "Cannot parse mac address of %s",
				     argv[pos + 1]);
			memcpy(maddr, paddr, ETH_ALEN);
			set_mps(iff_name, maddr, MAC_LOOKUP_INNER, RAW_MAC,
				dmac_mps_idx);
			dmac_mps_idx++;
			set_mps(iff_name, maddr, MAC_LOOKUP_INNER, RAW_MAC,
				smac_mps_idx);
...
set_rbb_filter:
	...
	while (fgets(entry, sizeof(entry), fp)) {
		...
		if (i == 0 || i == 1)
			continue;
		p = strchr(entry, '-');
		/* Skip all blank entries in mps tcam file */
		if (p && !strcmp(p, "-\n"))
			continue;

		entry_idx = i - 1;

		/* Skip all entries with viid other than that of port0 */
		token = strtok(entry, " ");
		j = 0;

		/* For T5, viid entry is at column 6, for T6, it is at
		 * column 12
		 */
		if (get_adapter_ver(iff_name) < 6)
			viid_entry = 5;
		else
			viid_entry = 12;
		...
		if (strcmp(token, argv[viid_pos])) {
			continue;
		}

		/* Create 4 categories of filters in both ipv4 and ipv6 mode.
		 * The order should be as below:
		 *
		 * 1. DROP filters
		 * 	Drop filter for all packets in default vi
		 *	with smac match for entries > 255 in mps tcam.
		 * 2. SWITCH filters for default vi
		 *	Switch filter for unicast promiscous packets in default
		 *	vi to egress port 1.
		 * 3. SWITCH filters for mirror vi
		 * 	Switch filter for all packets in mirror vi to egress
		 *	port 1.
		 */
		if (entry_idx > 255) {
			tokenise_set_rbb_filter(argv, iff_name, entry_idx,
						FILTER_TYPE_IPV4,
						FILTER_RINGBB_SMAC_DROP,
						pf, vi, mvi, hits);
			tokenise_set_rbb_filter(argv, iff_name, entry_idx,
						FILTER_TYPE_IPV6,
						FILTER_RINGBB_SMAC_DROP,
	...
	tokenise_set_rbb_filter(argv, iff_name, entry_idx,
				FILTER_TYPE_IPV4, FILTER_RINGBB_VI_SWITCH,
				pf, vi, mvi, hits);
	tokenise_set_rbb_filter(argv, iff_name, entry_idx,
				FILTER_TYPE_IPV6, FILTER_RINGBB_VI_SWITCH,
				pf, vi, mvi, hits);
	tokenise_set_rbb_filter(argv, iff_name, entry_idx,
				FILTER_TYPE_IPV4, FILTER_RINGBB_MVI_SWITCH,
				pf, vi, mvi, hits);
	tokenise_set_rbb_filter(argv, iff_name, entry_idx,
				FILTER_TYPE_IPV6, FILTER_RINGBB_MVI_SWITCH,
				pf, vi, mvi, hits);
	fclose(fp);
...
static void sched_show(int argc, char *argv[], int start_arg,
		       const char *iff_name)
{
	...
	int errs = 0;
	const char *s_type[] = {
		[SCHED_CLASS_TYPE_PACKET]	= "packet",
		[SCHED_CLASS_TYPE_STREAM]	= "stream",
	};
	const char *s_level[] = {
		[SCHED_CLASS_LEVEL_CL_RL]	= "cl-rl",
		[SCHED_CLASS_LEVEL_CL_WRR]	= "cl-wrr",
		[SCHED_CLASS_LEVEL_CH_RL]	= "ch-rl",
		[SCHED_CLASS_LEVEL_CH_WRR]	= "ch-wrr",
	};
	const char *s_mode[] = {
		[SCHED_CLASS_MODE_CLASS]	= "class",
		[SCHED_CLASS_MODE_FLOW]		= "flow",
	};
	const char *s_rateunit[] = {
		[SCHED_CLASS_RATEUNIT_BITS]	= "bits",
		[SCHED_CLASS_RATEUNIT_PKTS]	= "pkts",
	};

	memset(&op, ~0, sizeof(op));
	start_arg++;
	op.u.params.channel = -1;
	op.u.params.class = 0;	/* Assuming default value of class as 0 */
	for (errs = 0; argc > start_arg; start_arg += 2) {
		if (start_arg + 1 == argc) {
			warnx("missing argument for \"%s\"", argv[start_arg]);
			errs++;
			break;
		}
		if (!strcmp(argv[start_arg], "channel") &&
		    !get_sched_param(argc, argv, start_arg, &val))
			...
			warnx("Unknown scheduler read  parameter \"%s\"",
			      argv[start_arg]);
			errs++;
		}
	}

	/* Do a basic range check. We will recheck this with correct
	 * range values in driver
	 */
	if ((op.u.params.channel < 0 || op.u.params.channel > 3) ||
	    (op.u.params.class < 0 || op.u.params.class > INT_MAX)) {
		warnx("Invalid values for \"class\" or \"channel\"");
		errs++;
	}

	if (errs > 0)
		errx(1, "%d error%s in \'sched-class show\' command",
		     errs, errs == 1 ? "" : "s");

	op.cmd = CHELSIO_GET_SCHED_CLASS;
	op.subcmd = SCHED_CLASS_SUBCMD_PARAMS;
	if (doit(iff_name, &op) < 0)
		err(1, "sched-class show");

	printf("%-10s : \t%u\n", "channel", op.u.params.channel);
	printf("%-10s : \t%u\n", "class", op.u.params.class);
	printf("%-10s : \t%s\n", "level", s_level[op.u.params.level]);
	printf("%-10s : \t%s\n", "mode", s_mode[op.u.params.mode]);
	printf("%-10s : \t%s\n", "type", s_type[op.type]);
	printf("%-10s : \t%s\n", "rate-unit", s_rateunit[op.u.params.rateunit]);
	printf("%-10s : \t%u\n", "min-rate", op.u.params.minrate);
	printf("%-10s : \t%u\n", "max-rate", op.u.params.maxrate);
	printf("%-10s : \t%u\n", "weight", op.u.params.weight);
	printf("%-10s : \t%u\n", "pkt-size", op.u.params.pktsize);
	printf("%-10s : \t%u\n", "burst-size", op.u.params.burstsize);
}
...
static int sched_class(int argc, char *argv[], int start_arg, const char *iff_name)
{
	...
	if (!strcmp(argv[start_arg], "config")) {
		...
		= op.u.params.weight
		= op.u.params.pktsize
		= op.u.params.burstsize
		= -1;
	} else if (!strcmp(argv[start_arg], "show")) {
		sched_show(argc, argv, start_arg, iff_name);
		return 0;
	} else {
	...
	for (errs = 0; argc > start_arg; start_arg += 2) {
		...
		if (op.subcmd == SCHED_CLASS_SUBCMD_PARAMS) {
			...
			else if (!strcmp(argv[start_arg], "burst-size") &&
				 !get_sched_param(argc, argv, start_arg, &val))
				op.u.params.burstsize = val;
			else {
...
static int diag_cmd_memtest(int argc, char *argv[], int start_arg,
			    const char *iff_name)
{
	struct ch_diag_memtest op;
	u32 val;
	...
	memset(&op, 0, sizeof(op));
	if (!strcasecmp(argv[start_arg], "start")) {
		op.op = CH_DIAG_MEMTEST_OP_START;
		start_arg++;
		if (argc < start_arg + 2)
			errx(1, "%s: expecting arguments for start\n",
			     __func__);

		if (!strcasecmp(argv[start_arg], "duration")) {
			start_arg++;
			if (get_int_arg(argv[start_arg], &val))
				return -1;
			op.duration = val;
		} else {
			errx(1, "%s: unknown option: %s for start\n",
			     __func__, argv[start_arg]);
		}
	} else if (!strcasecmp(argv[start_arg], "stop")) {
		op.op = CH_DIAG_MEMTEST_OP_STOP;
	} else if (!strcasecmp(argv[start_arg], "status")) {
		op.op = CH_DIAG_MEMTEST_OP_STATUS;
	} else {
		errx(1, "%s: unknown option: %s\n", __func__, argv[start_arg]);
	}

	op.cmd = CHELSIO_DIAG_MEMTEST;
	if (doit(iff_name, &op) < 0)
		err(1, "diag cmd failed");

	if (op.op == CH_DIAG_MEMTEST_OP_STATUS) {
		printf("Test memory size (KB): %u\n", op.size);
		printf("Test duration remaining (sec): %u\n", op.duration);
		switch (op.status) {
		case CH_DIAG_MEMTEST_STATUS_IDLE:
			printf("Test status: IDLE\n");
			break;
		case CH_DIAG_MEMTEST_STATUS_RUNNING:
			printf("Test status: RUNNING\n");
			break;
		case CH_DIAG_MEMTEST_STATUS_FAILED:
			printf("Test status: FAILED\n");
			break;
		case CH_DIAG_MEMTEST_STATUS_PASSED:
			printf("Test status: PASSED\n");
			break;
		default:
			printf("Test status UNKNOWN: %u\n", op.status);
			break;
		}
	}
...
static int diag_cmd(int argc, char *argv[], int start_arg,
		    const char *iff_name)
{
	if (argc < start_arg + 1)
		usage(stderr);

	if (!strcasecmp(argv[start_arg], "cim_mem")) {
		start_arg++;
		return diag_cmd_memtest(argc, argv, start_arg, iff_name);
	}
...
#define FLASH_BOOTCFG_MAX_SIZE (64 * 1024)

#define NIC_BOOT	0x1
#define FCoE_BOOT	0x2
#define iSCSI_BOOT	0x4

static inline int is_bootcfg_valid(t_ConfigHeader *cf)
{
	return !strncmp((const char *)cf->Signature, "CBIO", strlen("CBIO"));
}

static struct struct_load_cfg *_read_boot_cfg(const char *iff_name)
{
	size_t bootcfg_size = sizeof(t_ConfigHeader);
	struct struct_load_cfg *op;
	int len;

	len = (bootcfg_size + 4-1) & ~3;

	op = calloc(sizeof(*op) + len, 1);
	if (!op)
		err(1, "readboot-cfg - memory allocation failed\n");

	op->cmd = CHELSIO_READ_BOOTCFG;
	op->len = len;
	if (doit(iff_name, op) < 0) {
		free(op);
		err(1, "readboot-cfg");
...
static int read_boot_cfg(int argc, char *argv[],
			 int start_arg, const char *iff_name)
{
	char *bl[] = {
			[NIC_BOOT]   = "NIC_BOOT",
	...
	struct struct_load_cfg *op = NULL;
	t_ConfigHeader *cf = NULL;
	uint8_t bios = 0, bios_nic = 0;
	int i;

	if (argc != start_arg)
		return -1;

	op = _read_boot_cfg(iff_name);
	if (!op)
		err(1, "readboot-cfg");

	cf = (t_ConfigHeader *)op->buf;

	if (!is_bootcfg_valid(cf)) {
		free(op);
		errx(1, "Invalid/No boot configuration found\n"
			"Please load valid configuration and retry");
	}

	if (cf->Bios) {
		for (i = 0; i < op->nports; i++) {
			if (cf->NIC.Bios[i])
				bios_nic |= (1 << i);
		}
		if (bios_nic)
	...
	printf("current boot setting is : 0x%x (%s: %s;  %s: %s;  %s: %s)\n",
	       bios,
	       bl[NIC_BOOT],   state[!!(bios & NIC_BOOT)],
	       bl[FCoE_BOOT],  state[!!(bios & FCoE_BOOT)],
	       bl[iSCSI_BOOT], state[!!(bios & iSCSI_BOOT)]);

	printf("\n");
	printf("%s: ", bl[NIC_BOOT]);
	for (i = 0; i < op->nports; i++) {
		printf("Port[%d] : %s",  i,
		       state[!!(bios_nic & (1 << i))]); 
		if (i != op->nports - 1)
	...
	printf("Vlan:    ");
	for (i = 0; i < op->nports; i++) {
		printf(" Port[%d] : %d",  i, cf->NIC.VlanId[i]); 
		if (i != op->nports - 1)
			printf(",");
	}
...
static int modify_boot_cfg(int argc, char *argv[],
			   int start_arg, const char *iff_name)
{
	size_t bootcfg_size = sizeof(t_ConfigHeader);
	struct struct_load_cfg *op = NULL;
	t_ConfigHeader *cf = NULL;
	uint32_t value;
	int port = -1, bios = -1, vlan = -1;
	int j, indx, pi;
	int en_nicboot = -1, dis_nicboot = -1;
	int nicboot_count = 0;

	if (argc < start_arg + 2) {
		printf("incorrect boot arguments\n");
		goto usage;
	}

	op = _read_boot_cfg(iff_name);
	if (!op)
		err(1, "modifyboot-cfg");

	indx = get_arg_index(argc, argv, "bios");
	if (indx != -1 && argv[indx + 1]) {
		if (get_int_arg(argv[indx + 1], &value))
			errx(1, "bios value not provided\n");

		bios = value;
	}

	if (bios > 0x7) {
		printf("argument 'bios' should be in the range [0x0 - 0x7]\n");
		goto usage;
	}

	indx = get_arg_index(argc, argv, "en_nicboot");
	if (indx != -1)
		en_nicboot = 1;

	indx = get_arg_index(argc, argv, "dis_nicboot");
	if (indx != -1)
		dis_nicboot = 1;

	indx = get_arg_index(argc, argv, "port");
	if (indx != -1 && argv[indx + 1]) {
		if (get_int_arg(argv[indx + 1], &value)) {
			printf("port value not provided\n");
			return -1;
		}
	...
	if (port > (int)(op->nports - 1)) {
		printf("argument 'port' should be in the range [0 - %d]\n",
		       op->nports - 1);
		goto usage;
	}

	indx = get_arg_index(argc, argv, "vlan");
	if (indx != -1 && argv[indx + 1]) {
		if (get_int_arg(argv[indx + 1], &value)) {
			printf("vlan value not provided\n");
			return -1;
		}
	...
	if (vlan > 4095) {
		printf("argument 'vlan' should be in the range [0 - 4095]\n");
		goto usage;
	}

	if (en_nicboot == 1 && dis_nicboot == 1)
		errx(1, "provide either en_nicboot or dis_nicboot \n");

	if ((vlan != -1 || en_nicboot != -1 || dis_nicboot != -1) &&
	    port == -1)
		errx(1, "port param is required\n");

	if (bios != -1 && port != -1)
		errx(1, "bios param does not support port option\n");

	if (port != -1 && vlan == -1 && en_nicboot == -1 && dis_nicboot == -1)
		errx(1, "Incorrect param provided\n");
	...
	if (bios != -1) {
		for(pi = 0; pi < op->nports; pi++)
			cf->NIC.Bios[pi] = !!(bios & NIC_BOOT);
		cf->FCoE.Bios  = !!(bios & FCoE_BOOT);
		cf->iSCSI.Bios = !!(bios & iSCSI_BOOT);
	}

	if (en_nicboot == 1)
		cf->NIC.Bios[port] = 1;
	else if (dis_nicboot == 1)
		cf->NIC.Bios[port] = 0;
	...
	/*
 	 ...
 	 * pad it up with 0's.
 	 */
	op->len = (bootcfg_size + 4 - 1) & ~3;
	for (j = bootcfg_size; j < op->len; j++)
		op->buf[j] = 0;

	/*
	 * Send the load configuration file command down to the driver.
	 */
	op->cmd  = CHELSIO_LOAD_BOOTCFG;
	if (doit(iff_name, op) < 0) {
		free(op);
		errx(1, "modifyboot-cfg failed on %s", iff_name);
	}

	free(op);
	return 0;
usage:
	errx(1, "usage: %s [bios <decimal/hexadecimal>]] "
	     "[port <port no> vlan <vlan id>] [en_nicboot port <port no>]"
	     "[dis_nicboot port <port no>]", argv[2]);
}
...
static int mpstrace_get_int(int argc, char *argv[], int start_arg)
{
	uint32_t val;

	if (argc <= start_arg)
		errx(1, "mpstrace set: no value found for %s",
		     argv[start_arg - 1]);
...
static int mpstrace_string_to_int(int argc, char *argv[], int start_arg,
				  u8 *dst, u32 len)
{
	uint32_t i, j, arglen;
	char c;
	...
	arglen = strlen(argv[start_arg]);
	if (arglen > len)
		errx(1, "mpstrace set: length of string for %s > MAX(%u)",
		     argv[start_arg - 1], len);

	for (i = 0, j = 0; i < len; i++) {
		for (k = 1; k >= 0; k--, j++) {
			if (!isxdigit(argv[start_arg][j]))
				errx(1, "mpstrace set: found non numeric value for %s: %s",
				     argv[start_arg - 1], argv[start_arg]);

			c = argv[start_arg][j];
			dst[i] |= strtoul(&c, NULL, 16) << (k * 4);
			if (j >= arglen - 1)
				goto out;
		}
	}
...
static void mpstrace_display(struct ch_mps_trace_filter *op)
{
	uint32_t i;

	printf("Tracer index: %u\n", op->index);
	if (op->mps_op == CH_MPS_TRACE_FILTER_OP_DISABLE) {
		printf("Tracer disabled\n");
		return;
	} else if (op->mps_op == CH_MPS_TRACE_FILTER_OP_RX) {
		printf("Tracer type: rx on channel %u\n", op->rx);
	} else if (op->mps_op == CH_MPS_TRACE_FILTER_OP_TX) {
		printf("Tracer type: tx on channel %u\n", op->tx);
	} else if (op->mps_op == CH_MPS_TRACE_FILTER_OP_LOOPBACK) {
		printf("Tracer type: loopback on channel %u\n", op->loopback);
	} else {
		printf("Unknown tracer type found: %u\n", op->mps_op);
		return;
	}
	...
	printf("Min len: %u\n", op->min_len);
	printf("Invert: %u\n", op->invert);
	printf("Skip offset: %u\n", op->skip_offset);
	printf("Skip len: %u\n", op->skip_len);
	if (op->mask_size) {
		printf("Data pattern:\n");
		for (i = 0; i < op->data_size; i++) {
			if (i && !(i % 4))
				printf(" ");
			if (i && !(i % 32))
				printf("\n");
		...
		printf("\nMask pattern:\n");
		for (i = 0; i < op->mask_size; i++) {
			if (i && !(i % 4))
				printf(" ");
			if (i && !(i % 32))
				printf("\n");
			printf("%02x", op->mask[i]);
		}
	}
	printf("\n");
}

static int mpstrace_cmd(int argc, char *argv[], int start_arg,
			const char *iff_name)
{
	uint32_t i, data_size = 0, mask_size = 0;
	struct ch_mps_trace_filter op;
	int ret = 0;
	...
	memset(&op, 0, sizeof(op));
	ret = mpstrace_get_int(argc, argv, start_arg);
	if (ret < 0)
		goto out;
	...
	start_arg++;
	if (argc == start_arg) {
		op.cmd = CHELSIO_GET_MPS_TRACE_FILTER;
		if (doit(iff_name, &op) < 0)
			err(1, "mpstrace get:");
	...
	if (!strcmp(argv[start_arg], "rx")) {
		...
		op.rx = ret;
		op.mps_op = CH_MPS_TRACE_FILTER_OP_RX;
		start_arg++;
	} else if (!strcmp(argv[start_arg], "tx")) {
		start_arg++;
		ret = mpstrace_get_int(argc, argv, start_arg);
		...
		op.tx = ret;
		op.mps_op = CH_MPS_TRACE_FILTER_OP_TX;
		start_arg++;
	} else if (!strcmp(argv[start_arg], "loopback")) {
		start_arg++;
		ret = mpstrace_get_int(argc, argv, start_arg);
		if (ret < 0)
			goto out;
		...
		op.loopback = ret;
		op.mps_op = CH_MPS_TRACE_FILTER_OP_LOOPBACK;
		start_arg++;
	} else if (!strcmp(argv[start_arg], "off")) {
		op.mps_op = CH_MPS_TRACE_FILTER_OP_DISABLE;
		goto send_ioctl;
	} else {
		errx(1, "mpstrace set: tx|rx|loopback|off not found");
	}

	if (start_arg == argc)
		errx(1, "mpstrace set: missing 1 or more params");

	while (start_arg < argc) {
		if (!strcmp(argv[start_arg], "not")) {
			op.invert = 1;
		} else if (!strcmp(argv[start_arg], "snaplen")) {
			start_arg++;
			ret = mpstrace_get_int(argc, argv, start_arg);
			if (ret < 0)
				goto out;
			...
			op.snap_len = ret;
		} else if (!strcmp(argv[start_arg], "minlen")) {
			start_arg++;
			ret = mpstrace_get_int(argc, argv, start_arg);
			if (ret < 0)
				goto out;
			...
			op.min_len = ret;
		} else if (!strcmp(argv[start_arg], "skipoff")) {
			start_arg++;
			ret = mpstrace_get_int(argc, argv, start_arg);
			if (ret < 0)
				goto out;
			...
			op.skip_offset = ret;
		} else if (!strcmp(argv[start_arg], "skiplen")) {
			start_arg++;
			ret = mpstrace_get_int(argc, argv, start_arg);
			if (ret < 0)
				goto out;
			...
			op.skip_len = ret;
		} else if (!strcmp(argv[start_arg], "data")) {
			start_arg++;
			ret = mpstrace_string_to_int(argc, argv, start_arg,
						     op.data,
						     sizeof(op.data) * 2);
			if (ret < 0)
				goto out;
			data_size = strlen(argv[start_arg]);
			op.data_size = (data_size + 1) / 2;
		} else if (!strcmp(argv[start_arg], "mask")) {
			start_arg++;
			ret = mpstrace_string_to_int(argc, argv, start_arg,
						     op.mask,
						     sizeof(op.mask) * 2);
			if (ret < 0)
				goto out;
			mask_size = strlen(argv[start_arg]);
			op.mask_size = (mask_size + 1) / 2;
		} else {
			errx(1, "mpstrace set: unknown parameter: %s",
			     argv[start_arg]);
		}
	...
	if (mask_size && mask_size != data_size) {
		errx(1, "mpstrace set: length of mask (%u) != length of data (%u)\n",
		     mask_size, data_size);
	} else if (!mask_size && data_size) {
		for (i = 0; i < op.data_size; i++)
			op.mask[i] = 0xff;

		op.mask_size = op.data_size;
	}

send_ioctl:
	op.cmd = CHELSIO_SET_MPS_TRACE_FILTER;
	if (doit(iff_name, &op) < 0)
		err(1, "mpstrace set:");
out:
	return ret;
	...
	struct ethtool_drvinfo *drvinfo)
{
	...
	if (!is_chelsio_iface(iff_name)) {
		...
		if (!strcmp(iff_name, "stor")) {
			...
			if(!strcmp(argv[2], "--chnet")) {
				r = run_chnet_cmd(argc, argv);
				goto done;
			}
#ifdef __CSIO_FOISCSI_ENABLED__		
			if(!strcmp(argv[2], "--foiscsi")) {
				r = run_foiscsi_stor(argc, argv);
				goto done;
			}
#endif
#ifdef __CSIO_COISCSI_ENABLED__		
			if(!strcmp(argv[2], "--coiscsi")) {
				r = run_coiscsi_cmd(argc, argv);
				goto done;
			} else if(!strcmp(argv[2], "--isns")) {
				r = run_isns_cmd(argc, argv);
				goto done;
			}
#endif
		}
	...
	else if (!strcmp(argv[2], "fec"))
		r = fec_config(argc, argv, 3, iff_name);
	...
	else if (!strcmp(argv[2], "readboot-cfg"))
		r = read_boot_cfg(argc, argv, 3, iff_name);
	else if (!strcmp(argv[2], "modifyboot-cfg"))
		r = modify_boot_cfg(argc, argv, 3, iff_name);
	...
	else if (!strcmp(argv[2], "send-workreq"))
		r = send_workreq(argc, argv, 3, iff_name);
	...
	else if (!strcmp(argv[2], "diag"))
		r = diag_cmd(argc, argv, 3, iff_name);
#endif
	else if (!strcmp(argv[2], "driver-file"))
		r = driver_file(argc, argv, 3, iff_name);
	else if (!strcmp(argv[2], "configure_ringbb"))
		r = configure_ringbb(argc, argv, 3, iff_name);
	else if (!strcmp(argv[2], "mpstrace"))
		r = mpstrace_cmd(argc, argv, 3, iff_name);
