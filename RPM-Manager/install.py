...
if pathVar.strip() != '':
	os.chdir(pathVar)
pack_dist = 'rhel7u3'
pack_kver = '3.10.0-514.el7'
pack_dver = 'Red Hat Enterprise Linux Server release 7.3'
pack_arch = [ "x86_64" ]
#pack_arch = [ "x86_64", "ppc64", "ppc64le" ]
dist = None
DISTRO = None
rpm_dir = None
fcoe_support = False
...
handler.writelines("\n+----------------------------------------+" \
	      "\n|Chelsio Unified Wire " + version + " Installer |" \
	      "\n+----------------------------------------+\n")
handler.close()
config_dict = {}
install_dict = {}
...
cluster_file = ""
CUSTOMENU = 0
nonofedconf = ["conf_hctoe", "conf_udpso", "conf_hfilter", "conf_iscsi_perf"]
...
def dist_check():
	...
	if re.search('sles-release-12\.2',dist_rpm) != None:
                dist = 'sles12sp2'
                DISTRO = 'SLES12.2'
                bypass_support = False
                fcoe_support = False
 	...
 	elif re.search('sles-release-12\.1',dist_rpm) != None:
                dist = 'sles12sp1'
                DISTRO = 'SLES12.1'
                bypass_support = False
                fcoe_support = False
		...
		cluster_file = "SLES11sp3"
	...
	elif re.search('redhat-release-.*-6.8|centos-release-6-8',dist_rpm) != None:
                dist = 'rhel6u8'
                DISTRO = 'RHEL6.8'
                fcoe_support = False
                fcoe_tgt_support = False
                ...
                udpso_support = True
                um_support = False
                cluster_file = "RHEL6.8"
	elif re.search('redhat-release-.*-6.9|centos-release-6-9',dist_rpm) != None:
                dist = 'rhel6u9'
                DISTRO = 'RHEL6.9'
                fcoe_support = False
                fcoe_tgt_support = False
                ...
                udpso_support = True
                um_support = False
                cluster_file = "RHEL6.9"
	elif re.search('redhat-release-.*-7.3|centos-release-7-3',dist_rpm) != None:
                dist = 'rhel7u3'
                DISTRO = 'RHEL7.3'
                fcoe_support = True
                fcoe_tgt_support = False
	...
	config_dict['WIRE_DIRECT'] = 'conf_edc_only'
	...
	if wd_support:
		install_dict['wd'] = 'wd'
	if crypto_support:
		install_dict['crypto'] = 'crypto'
	if ovs_support:
		install_dict['ovs'] = 'ovs'

def kernel_check():
	global pack_dist
	global pack_kver
	global pack_dver
	"""if re.search('2.6.32-431.el6|2.6.32-504.el6|2.6.32-573.el6|2.6.32-642.el6|2.6.32-696.el6|3.10.0-229.el7|3.10.0-229.el7.ppc64|3.10.0-229.ael7b.ppc64le|3.10.0-327.el7|3.10.0-514.el7|' \
			'3.0.76-0.11-default|3.0.101-63-default|3.12.28-4-default|3.12.49-11-default|4.4.21-69',kernel_ver ) == None:
		print BLUE + 'The Operating System kernel version is not supported.\n' + RESET
		print BLUE + 'The package is supported only for %s, base kernel version %s\n'%(pack_dver, pack_kver) + RESET
		print BLUE + 'For updated/custom kernels please use the source package from service.chelsio.com.\n' + RESET
		print BLUE + 'For the list of Supported Platforms and Operating Systems see' + RESET
		print BLUE + '%s/docs/README.txt'%(os.getcwd()) + RESET
		sys.exit(1)
	if dist != pack_dist:
                print FAIL + 'The package is supported only for %s, base kernel version %s'%(pack_dver, pack_kver) + RESET
...
def arch_check():
	"""if arch not in pack_arch :
		print BLUE + 'The %s architecture is not supported.\n'%(arch) + RESET
		print BLUE + 'The package supports only %s architecture.'%("/".join(pack_arch)) + RESET
		sys.exit(1)"""
	print
...
chelsio_package = [ "chelsio-series4-firmware", "cxgb4", "cxgb4toe", "cxgb4vf", "chiwarp",
		    ...
		    "sniffer", "libcxgb4_udp", "libcxgb4_sock", "iwpmd", "iwpmd-debuginfo", "chelsio-utils",
...
chelsio_crypto_package = ["chcr", "chopenssl", "chopenssl-devel", "chopenssl-doc" ]

chelsio_ovs_package = ["openvswitch", "openvswitch-debuginfo", "openvswitch-selinux-policy", "openvswitch-devel" ]
...
chelsio_wdtoe_package = [ "chelsio-series4-firmware", "cxgb4", "chiwarp", "libcxgb4", 
                    ...
                    "libcxgb4_udp_debug", "libcxgb4_sock_debug", "iwpmd", "iwpmd-debuginfo", "cxgb4wdtoe", "libwdtoe", "libwdtoe_dbg","chelsio-utils" ]
...
chelsio_package_info = {'chelsio-series4-firmware' : { 'name' : 'Chelsio Terminator 4 firmware',
			...
			'libcxgb4_sock_debug' : { 'name' : 'WD UDP Sock Debug lib',
                                            'available' : 1, 'requires' : ['libcxgb4_udp'] },
			'iwpmd' : { 'name' : 'iWARP Port Mapper library',
                                       'available' : 1, 'requires' : [] },
			'iwpmd-debuginfo' : { 'name' : 'iWARP Port Mapper library',
                                       'available' : 1, 'requires' : [] },
...
chelsio_crypto_package_info = { 'chcr' : { 'name' : 'Chelsio Crypto driver',
					   'available' : 1, 'requires' : ['cxgb4toe']},
				'chopenssl' : { 'name' : 'Chelsio Openssl package',
					   'available' : 1, 'requires' : []},
				'chopenssl-devel' : { 'name' : 'Chelsio Openssl-devel package',
					   'available' : 1, 'requires' : ['chopenssl']},
				'chopenssl-doc' : { 'name' : 'Chelsio Openssl-doc package',
					   'available' : 1, 'requires' : ['chopenssl']},
# disable component is used for disabling all the features present in the current dictionary
				'disable'  : { 'name' : 'Disable All (for script purpose)',
				           'available' : 1, 'requires' : [] }
...
chelsio_ovs_package_info = { 'openvswitch' : { 'name' : 'Openvswitch package',
					   'available' : 1, 'requires' : []},
				'openvswitch-selinux-policy' : { 'name' : 'openvswitch selinux policy package',
					   'available' : 1, 'requires' : []},
				'openvswitch-devel' : { 'name' : 'Openvswitch devel package',
					   'available' : 1, 'requires' : ['openvswitch']},
# disable component is used for disabling all the features present in the current dictionary
				'disable'  : { 'name' : 'Disable All (for script purpose)',
				           'available' : 1, 'requires' : [] }
				}
...
def prompt_ofed():
	...
	menu ={}	
	os.system('clear')
	show_heading("Chelsio Unified Wire "+ version +" Installer\nSelect the OFED installation: ")
	data += \
                '''Press %d, Install OFED-3.18-3,Ideal for iWARP/WD Users'''%(ix)
        menu[ix] = 'ofed'
        ix += 1
	if data != '':
                data = data + '\n'
	...
	if CUSTOMENU == 1 :
		...
		if fcoe_support:
			...
			if conf_hfilter :
				...
				data += '''Press %d, Install all Chelsio drivers,NIC/iWARP/WDUDP'''%(ix)
		...
		if crypto_support:
        	        if data != '':
                	        data = data + '\n'
	                data += '''Press %d, Install Crypto and SSL libraries,Crypto TLS/SSL'''%(ix)
        	        menu[ix] = 'crypto'
                	ix += 1
		if ovs_support:
        	        if data != '':
                	        data = data + '\n'
	                data += '''Press %d, Install OVS,Openvswitch'''%(ix)
        	        menu[ix] = 'ovs'
                	ix += 1
		if data != '':
			data = data + '\n' 
...
def promptCustomConfig():
	...
	global version
        ...
        ix = 1
        data = ''
        show_heading("Chelsio Unified Wire "+ version +" Installer\nSelect the Terminator Configuration:")
        if fcoe_support:
                data += \
        ...
        if not clusterInstall :
                if data != '':
                        data = data + '\n'
                data += \
                '''Press %d, Wire Direct Latency, NIC;TOE;RDMA;WD*'''%(ix)
        ...
        handler.writelines("\nTerminator Configuration selection: %s\n"%(chtoprint))
        handler.close()
        if exit :
                sys.exit(1)
	return False
	...
	if not clusterInstall :
		...
		if arch.find("ppc") == -1 :
			if data != '':
			        data = data + '\n'
			data += \
			'''Press %d, High Capacity WD, NIC;TOE;RDMA;WD*'''%(ix)
		        menu[ix] = 'conf_hcll'
			ix += 1
...
def prompt_welcome():
	global version
	os.system('clear')
	show_heading("Welcome to the Chelsio Unified Wire "+ version +" Installer")
	print ""
	while True :
...
def set_availablity() :
	...
	global chelsio_crypto_package
	global chelsio_ovs_package
	...
	global chelsio_crypto_package_info
	global chelsio_ovs_package_info
	...
	if conf_ll or conf_edc_only or conf_hcll :
		chelsio_stor_package_info['disable']['available'] = 0
                chelsio_bypass_package_info['disable']['available'] = 0
                chelsio_udpso_package_info['disable']['available'] = 0
		chelsio_crypto_package_info['disable']['available'] = 0
		chelsio_ovs_package_info['disable']['available'] = 0
		if inbox:
			if wdtoe_support:
				chelsio_wdtoe_package_info['disable']['available'] = 1
				for packs in ['chiwarp', 'libcxgb4', 'libcxgb4-devel', 'libcxgb4_udp', 'libcxgb4_sock', "iwpmd", "iwpmd-debuginfo",\
				...
				chelsio_wdtoe_package_info['chelsio-series4-firmware']['available'] = 0
				#chelsio_package_info['chelsio-series4-firmware']['available'] = 1
	...
	if conf_hctoe or conf_hfilter:
		...
		chelsio_package_info['iwpmd']['available'] = 0
		chelsio_package_info['iwpmd-debuginfo']['available'] = 0
		chelsio_bypass_package_info['disable']['available'] = 0
		chelsio_udpso_package_info['disable']['available'] = 0
		chelsio_wdtoe_package_info['disable']['available'] = 0
		chelsio_package_info['cxgb4vf']['available'] = 0
		...
		if conf_hfilter :
			chelsio_package_info['cxgb4toe']['available'] = 0
			chelsio_package_info['bonding']['available'] = 0
			chelsio_ovs_package_info['disable']['available'] = 1
	if conf_rdma_perf or conf_memfree :
		...
		chelsio_package_info['sniffer']['available'] = 0
                ...
                chelsio_package_info['bonding']['available'] = 0
		if conf_memfree :
			for udp_libs in ['libcxgb4_udp', 'libcxgb4_sock', 'libcxgb4_udp_debug', 'libcxgb4_sock_debug'] :
				chelsio_package_info[udp_libs]['available'] = 0
	if conf_iscsi_perf :
                chelsio_package_info['sniffer']['available'] = 0
                chelsio_package_info['chiwarp']['available'] = 0
                chelsio_package_info['libcxgb4']['available'] = 0
	...
	if ( chelsio_stor_package_info['disable']['available'] == 1 and chelsio_stor_package_info['cxgb4i']['available'] == 1 ) :
		if checkOpenssl():
			openssldep = 1
	...
	if crypto:
		...
		for package in [ 'cxgb4vf', 'chiwarp', 'bonding', 'libcxgb4', \
				...
				'libcxgb4_sock_debug','libcxgb4_udp_debug', 'rdma-block-device']:
			chelsio_package_info[package]['available'] = 0
		chelsio_crypto_package_info['disable']['available'] = 1
	if ovs:
		chelsio_stor_package_info['disable']['available'] = 0
		chelsio_crypto_package_info['disable']['available'] = 0
		chelsio_ovs_package_info['disable']['available'] = 1
		chelsio_bypass_package_info['disable']['available'] = 0
	        chelsio_wdtoe_package_info['disable']['available'] = 0
		for package in [ 'cxgb4toe', 'cxgb4vf', 'chiwarp', 'bonding', 'libcxgb4', \
				'libcxgb4-debuginfo', 'libcxgb4-devel', 'libcxgb4_udp',\
				'libcxgb4_sock', 'sniffer',\
				'libcxgb4_sock_debug','libcxgb4_udp_debug', 'rdma-block-device']:
			chelsio_package_info[package]['available'] = 0
	...
	#print chelsio_wdtoe_package_info
...
def install_ofed():
	...
	if dist in ["rhel6u5", "rhel6u6", "rhel6u7", "rhel6u8", "rhel7u1", "rhel7u2", "sles11sp3","sles11sp4", "sles12", "sles12sp1"] :
		...
		for package in chelsio_package:
			if chelsio_package_info[package]['available'] == 1 :
				full_pack_name = None
					...
					if re.search('^' + package + '-' + version,i) != None:
                                                full_pack_name=i
				if package.find("iwpmd") != -1 : 
					for i in content_ofed_build_drv_rpm:
						if re.search('^' + package+"-1",i) != None:
							full_pack_name=i
				if full_pack_name != None:
                                        path_to_pack = ofed_build_drv_rpm + os.sep + full_pack_name
                                        inst_cmd = 'rpm -ivh ' + path_to_pack
			...
			full_pack_name = None
				...
				if sts != 0 : 
					...
					sys.exit(1)
                                ...
                                print "Package : %s not found"%(package)
	if crypto :
		if chelsio_package_info['disable']['available'] == 1 :
			content_kernel_build_drv_rpm = os.listdir(kernel_build_drv_rpm)
			nic_pack_toe = ["chelsio-series4-firmware", "cxgb4", "cxgb4toe", "chelsio-utils"]
			for package in nic_pack_toe:
				full_pack_name = None
					...
					if sts != 0 : 
						...
						sys.exit(1)
        	                        ...
        	                        print "Package : %s not found"%(package)
		if chelsio_crypto_package_info['disable']['available'] == 1 :
			content_kernel_build_drv_rpm = os.listdir(kernel_build_drv_rpm)
			for package in chelsio_crypto_package:
				full_pack_name = None
        	                for i in content_kernel_build_drv_rpm:
					if re.search('^' + package + '-' + version,i) != None:
        	                                full_pack_name=i
				if package.find("chopenssl") != -1 : 
					for i in content_kernel_build_drv_rpm:
						if re.search('^' + package + "-1",i) != None:
							full_pack_name=i
        	                if full_pack_name != None:
        	                        ...
        	                        inst_cmd = 'rpm -ivh ' + path_to_pack
					if package.find('chopenssl') != -1:
        	                        	inst_cmd = 'rpm -ivh --nodeps ' + path_to_pack
        	                        print "\nInstalling Package : %s"%(package)
					handler = open(log_file,'a')
					handler.writelines("\nInstalling package : %s\n"%(package))
					sts,out = commands.getstatusoutput(inst_cmd)
					...
					if sts != 0 : 
						...
						sys.exit(1)
        	                        ...
        	                        print "Package : %s not found"%(package)
			sts, out = commands.getstatusoutput("scripts/instKernel.sh RHEL7.3 1")
			print out
			sts, out = commands.getstatusoutput("service sshd restart &> /dev/null")

	if ovs :
		if chelsio_package_info['disable']['available'] == 1 :
			content_kernel_build_drv_rpm = os.listdir(kernel_build_drv_rpm)
			nic_pack_toe = ["chelsio-series4-firmware", "cxgb4", "chelsio-utils"]
			for package in nic_pack_toe:
				full_pack_name = None
        	                for i in content_kernel_build_drv_rpm:
					if re.search('^' + package + '-' + version,i) != None:
					...
					if sts != 0 : 
						...
						sys.exit(1)
        	                        ...
        	                        print "Package : %s not found"%(package)
		if chelsio_ovs_package_info['disable']['available'] == 1 :
			content_kernel_build_drv_rpm = os.listdir(kernel_build_drv_rpm)
			for package in chelsio_ovs_package:
				full_pack_name = None
        	                '''for i in content_kernel_build_drv_rpm:
					if re.search('^' + package + '-' + version,i) != None:
        	                                full_pack_name=i
...
def Usage():
	...
	print "\nUSAGE: python install.py -c <CONF> -i <INSTALL> "
        print "\nOptions: "
        print "\t-c, --config       Terminator configuration selection."
	print "\t		    Available config opts : %s"%(','.join(config_dict.keys()))
        print "\t-i, --install      Select Installation mode."
	print "\t		    Available Installation modes : %s"%(', '.join(sorted(install_dict.keys(), key=lambda x:x)))
	#print "\t		    Available Installation modes : %s"%(', '.join(install_dict.keys()))
	#print ', '.join(sorted(install_dict.keys(), key=lambda x:x))
	if not ofed_not_supported_on_platform:
...
def parse_args(cmdArgv):
	...
	install_key = None
	support_dict = \
			{ 'conf_uwire' : ['all', 'nic_toe', 'bypass','wd', 'crypto', 'ovs'],
			  'conf_tgt' : ['target'],
	...
	for opt, arg in opts:
		...
		if opt in ("-h", "--help"):
			...
			if arg not in config_dict.keys():
				...
				handler = open(log_file,'a')
				handler.writelines("Terminator Configuration selection: %s\n"%(config_dict[arg]))
				handler.close()
			...
			if arg not in install_dict.keys():
				...
				handler.writelines("Terminator Installation selection: %s\n"%(install_dict_new[arg]))
				handler.close()
	...
	if install_key not in support_list:
		...
		sys.exit(1)
	if not clusterInstall :
		if not ofed and not config in nonofedconf :
			checkOfedOnMachine()
	conf_key = ""
	if config in nonofedconf and ofed :
		for keys in config_dict.keys() :
			if config_dict[keys] == config :
				conf_key = keys
				break ;
		print FAIL + "\"-o\" OFED options cannot be used with "+ conf_key  +" config file" + RESET
		sys.exit(1)
...
def copy_config():
	...
	if conf_uwire:
		os.system('install -m 644 %s/t4-config.txt /lib/firmware/cxgb4'%(config_dir))
                ...
                os.system('install -m 644 %s/t6-config.txt /lib/firmware/cxgb4'%(config_dir))
	elif conf_ll:
		os.system('install -m 644 %s/t4-config.txt /lib/firmware/cxgb4'%(config_dir + os.sep + 'low_latency_config'))
                ...
                os.system('install -m 644 %s/t6-config.txt /lib/firmware/cxgb4'%(config_dir + os.sep + 'low_latency_config'))
	elif conf_hcrdma:
		os.system('install -m 644 %s/t4-config.txt /lib/firmware/cxgb4'%(config_dir + os.sep + 'high_capacity_rdma'))
                ...
                os.system('install -m 644 %s/t6-config.txt /lib/firmware/cxgb4'%(config_dir + os.sep + 'high_capacity_rdma'))
	elif conf_hctoe:
		os.system('install -m 644 %s/t4-config.txt /lib/firmware/cxgb4'%(config_dir + os.sep + 'high_capacity_config'))
                ...
                os.system('install -m 644 %s/t6-config.txt /lib/firmware/cxgb4'%(config_dir + os.sep + 'high_capacity_config'))
	elif conf_tgt:
		os.system('install -m 644 %s/t4-config.txt /lib/firmware/cxgb4'%(config_dir + os.sep + 'fcoe_target_config'))
                ...
                os.system('install -m 644 %s/t6-config.txt /lib/firmware/cxgb4'%(config_dir + os.sep + 'fcoe_target_config'))
	elif conf_udpso:
		os.system('install -m 644 %s/t4-config.txt /lib/firmware/cxgb4'%(config_dir + os.sep + 'udp_so_config'))
                ...
                os.system('install -m 644 %s/t6-config.txt /lib/firmware/cxgb4'%(config_dir + os.sep + 'udp_so_config'))
	elif conf_edc_only :
		os.system('install -m 644 %s/t4-config.txt /lib/firmware/cxgb4'%(config_dir + os.sep + 'edc_only_config'))
                ...
                os.system('install -m 644 %s/t6-config.txt /lib/firmware/cxgb4'%(config_dir + os.sep + 'edc_only_config'))
	elif conf_hcll :
		os.system('install -m 644 %s/t4-config.txt /lib/firmware/cxgb4'%(config_dir + os.sep + 'high_capacity_wd'))
                ...
                os.system('install -m 644 %s/t6-config.txt /lib/firmware/cxgb4'%(config_dir + os.sep + 'high_capacity_wd'))
	elif conf_hfilter :
		...
		os.system('install -m 644 %s/t6-config.txt /lib/firmware/cxgb4'%(config_dir + os.sep + 'hash_filter_config'))
	elif conf_rdma_perf:
                ...
                os.system('install -m 644 %s/t6-config.txt /lib/firmware/cxgb4'%(config_dir + os.sep + 'rdma_perf_config'))
	elif conf_iscsi_perf :
                ...
                os.system('install -m 644 %s/t6-config.txt /lib/firmware/cxgb4'%(config_dir + os.sep + 'iscsi_perf_config'))
	elif conf_memfree :
                ...
                os.system('install -m 644 %s/t6-config.txt /lib/firmware/cxgb4'%(config_dir + os.sep + 'memfree_config'))
	...
	os.system('install -m 644 %s/vpds/chelsio_adapter_config /sbin'%(config_dir))
	os.system('install -m 644 %s/vpds/*.bin /lib/firmware/cxgb4/'%(config_dir))

def checkOpenssl():
	ssl_head = "/usr/include/openssl/evp.h"
	if not os.path.isfile(ssl_head) :
		print "Openssl devel is needed for Chelsio ISCSI DATA Path Accelerator."
		print "Please install Openssl devel and restart the installation or Press 'Y' to skip ISCSI DATA Path Accelerator and continue installation"
		print "Press 'N' to install openssl dependency using package manager."
		while True :
...
def fixInitrd() :
	unamever = shell("uname -r")[1]
	initimg = "/boot/initramfs-" + unamever + ".img"
	if not os.path.isfile(initimg) :
		initimg = "/boot/initrd-" + unamever
		if not os.path.isfile(initimg) :
			initimg = "/boot/initrd.img-"+unamever
	CMD = "which dracut"
	stat, out = commands.getstatusoutput(CMD)
	if stat == 0 :
		if os.path.isfile(out) :
			CMD = "export logfile=" + os.getcwd() +"/install.log ; "+ os.getcwd() +"/scripts/fix_init.sh -r cxgb4,csiostor,cxgb4vf,csiostor,libcxgbi,cxgb4i,cxgbit -y "
...
def checkdeps():
	...
	if nc_lib :
		for upr in ["yum", "zypper", "apt-get"]:
			...
			if upr == "zypper" :
			   ff = "-n"
			cmd = upr + " " + ff + " install " + " ".join(nc_lib)
			status, output = commands.getstatusoutput(cmd)
			if status != 0 :
			    print "\nPlease configure %s to install dependencies"%upr
		    ...
		    print "\nUnable to install dependencies using yum/zypper/apt-get,"
