...
out:
		mutex_unlock(&notify_mutex);
	} /* CONFIG_CHELSIO_OFFLOAD_EXISTING_LISTENERS */
}
						  ...
						  int cop_managed_offloading)
{
	static struct offload_settings allow_offloading_settings = {
		1, -1, -1, -1, QUEUE_RANDOM, -1, -1, -1, 0, -1, 0
	};
...
{
	struct offload_policy *p = dev->policy;
	if (p && p->txt_data) {
		*addr = p->txt_data;
		*vers = p->cop_txt_hdr_vers;
		return p->txt_len;
	}

	return -1;
}
EXPORT_SYMBOL(get_cop_txt_data);

int set_offload_policy(struct toedev *dev,
		       ...
		       uint32_t tot_len)
{
	...
	if (f) {
		op_len = (f->nrules + 1) * sizeof(struct offload_settings) +
			 f->prog_size * sizeof(struct ofld_prog_inst) +
			 f->opt_prog_size * sizeof(u32);

		p = kmalloc(op_len + sizeof(*p), GFP_KERNEL);
		if (!p)
			return -ENOMEM;
		...
		p->use_opt = 1;
		memcpy(p->prog, f->prog, op_len);

		/* if total len greater than offload policy len,
		 * then look for cop text header
		 */
		if (tot_len > op_len + sizeof(struct ofld_policy_file)) {
			th = (const struct cop_txt_hdr *)((char *)f->prog + op_len);
			/* if cop text data is present, then store it */
			if (th->size) {
				p->txt_data = kmalloc(th->size, GFP_KERNEL);
				if (!p->txt_data) {
					kfree(p);
					return -ENOMEM;
				}
				memcpy(p->txt_data,
				       (char *)th + th->offset, th->size);
				p->txt_len = th->size;
				p->cop_txt_hdr_vers = th->vers;
			}
		}
