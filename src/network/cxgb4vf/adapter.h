...
/*
 * Macros used by the common code to log information.
 */
#define CH_ERR(adapter, fmt, ...) \
	dev_err(adapter->pdev_dev, fmt, ## __VA_ARGS__)

#define CH_WARN(adapter, fmt, ...) \
	dev_warn(adapter->pdev_dev, fmt, ## __VA_ARGS__)

#define CH_ALERT(adapter, fmt, ...) \
	dev_alert(adapter->pdev_dev, fmt, ## __VA_ARGS__)

#define CH_WARN_RATELIMIT(adapter, fmt, ...) \
	do {\
		if (printk_ratelimit()) \
			dev_warn(adapter->pdev_dev, fmt, ## __VA_ARGS__); \
	} while (0)
...
struct tx_desc {
	/*
	 * Egress Queues are measured in units of X_IDXSIZE_UNIT by the
	 * hardware: Sizes, Producer and Consumer indices, etc.
	 */
	__be64 flit[X_IDXSIZE_UNIT/sizeof(__be64)];
};
struct tx_sw_desc;
struct sge_txq {
