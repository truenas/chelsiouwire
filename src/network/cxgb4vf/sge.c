...
static inline void ring_fl_db(struct adapter *adapter, struct sge_fl *fl)
{
	...
	if (fl->pend_cred >= FL_PER_EQ_UNIT) {
		if (is_t4(adapter->params.chip))
			val |= V_PIDX(fl->pend_cred / FL_PER_EQ_UNIT);
		else
			val |= V_PIDX_T5(fl->pend_cred / FL_PER_EQ_UNIT);
		...
		if (unlikely(fl->bar2_addr == NULL))
			writel(val | V_QID(fl->cntxt_id),
			       adapter->regs + T4VF_SGE_BASE_ADDR + A_SGE_VF_KDOORBELL);
		else
			writel_wc(val | V_QID(fl->bar2_qid),
			      ...
			      int n)
{
	...
	if (unlikely(tq->bar2_addr == NULL)) {
		u32 val = val = V_PIDX(n);

		writel(val | V_QID(tq->cntxt_id),
		       adapter->regs + T4VF_SGE_BASE_ADDR + A_SGE_VF_KDOORBELL);
	} else {
		u32 val = V_PIDX_T5(n);
		 ...
		 */
		if (n == 1 && adapter->tx_db_wc && tq->bar2_qid == 0) {
			...
			__raw_writel((__force u32)cpu_to_le32(val | V_QID(tq->bar2_qid)),
				     tq->bar2_addr + SGE_UDB_KDOORBELL);
...
nocsum:
	...
	if (likely(csum_type >= TX_CSUM_TCPIP)) {
		u64 hdr_len = V_TXPKT_IPHDR_LEN(skb_network_header_len(skb));
...
netdev_tx_t t4vf_eth_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (pi->vlan_id && !skb_vlan_tag_present(skb))
		__vlan_hwaccel_put_tag(skb, htons(ETH_P_8021Q), pi->vlan_id);
		   ...
		   const struct cpl_rx_pkt *pkt)
{
	...
	if (unlikely(pkt->vlan_ex) && !unlikely(pi->vlan_id)) {
		       ...
		       const struct pkt_gl *gl)
{
	...
	if (csum_ok && !pkt->err_vec &&
	    (be32_to_cpu(pkt->l2info) & (F_RXF_UDP|F_RXF_TCP))) {
		if (!pkt->ip_frag) {
			skb->ip_summed = CHECKSUM_UNNECESSARY;
			rxq->stats.rx_cso++;
...
{
	return ((rc->u.type_gen >> S_RSPD_GEN) & 0x1) == rspq->gen;
}
 ...
 *
 *	Called when we find out that the current packet, @gl, can't be
...
int process_responses(struct sge_rspq *rspq, int budget)
{
	...
	while (likely(budget_left)) {
		...
		if (likely(rsp_type == X_RSPD_TYPE_FLBUF)) {
			...
			/*
			 * Gather packet fragments.  Note that the use of
			 * rspq->offset in the loop below works because either
			 * the new Ingress Packet will be a single chunk in an
			 * existing Packed Buffer, or it will be in multiple
			 * chunks across more than one buffer.  In the latter
			 * case, the hardware will always start with a new
			 * buffer and so the offset for all of the chunks will
			 * be 0 ...
			 */
			...
			WARN_ON(rsp_type > X_RSPD_TYPE_CPL);
			ret = 0;
		}
		...
		if (unlikely(ret)) {
			...
			rspq->next_intr_params =
				V_QINTR_TIMER_IDX(NOMEM_TIMER_IDX);
			break;
		}
	 ...
	 */
	if (rspq->offset >= 0 &&
	    fl_cap(&rxq->fl) - rxq->fl.avail >= FL_PER_EQ_UNIT)
...
static int napi_rx_handler(struct napi_struct *napi, int budget)
{
	...
	if (likely(work_done < budget)) {
		...
		intr_params = V_QINTR_TIMER_IDX(X_TIMERREG_UPDATE_CIDX);

	if (unlikely(work_done == 0))
		rspq->unhandled_irqs++;

	val = V_CIDXINC(work_done) | V_SEINTARM(intr_params);

	/*
 	 * If we don't have access to the new User GTS (T5+), use the old
 	 * doorbell mechanism; otherwise use the new BAR2 mechanism.
 	 */
	if (unlikely(rspq->bar2_addr == NULL)) {
		t4_write_reg(rspq->adapter,
			     T4VF_SGE_BASE_ADDR + A_SGE_VF_GTS,
			     val | V_INGRESSQID((u32)rspq->cntxt_id));
	} else
		writel_wc(val | V_INGRESSQID(rspq->bar2_qid),
...
static unsigned int process_intrq(struct adapter *adapter)
{
	...
	for (work_done = 0; ; work_done++) {
		 ...
		 */
		dma_rmb();
		if (unlikely(G_RSPD_TYPE(rc->u.type_gen) != X_RSPD_TYPE_INTR)) {
			dev_err(adapter->pdev_dev,
				"Unexpected INTRQ response type %d\n",
				G_RSPD_TYPE(rc->u.type_gen));
			continue;
		}
		 ...
		 * DEBUG.
		 */
		qid = G_RSPD_QID(be32_to_cpu(rc->pldbuflen_qid));
		iq_idx = IQ_IDX(s, qid);
	...
	val = V_CIDXINC(work_done) | V_SEINTARM(intrq->intr_params);

	/* If we don't have access to the new User GTS (T5+), use the old
	 * doorbell mechanism; otherwise use the new BAR2 mechanism.
	 */
	if (unlikely(intrq->bar2_addr == NULL)) {
		t4_write_reg(adapter, T4VF_SGE_BASE_ADDR + A_SGE_VF_GTS,
			     val | V_INGRESSQID(intrq->cntxt_id));
	} else
		writel_wc(val | V_INGRESSQID(intrq->bar2_qid),
...
/*
 * The AMD ARM A1100 (their "Seattle" SoC) has a bug in its PCIe Root Complex
 * where Transaction Layer Packets with the Relaxed Ordering Attribute clear
 * are allowed to bypass earlier TLPs with Relaxed Ordering set.  This is a
 * violation of the PCIe 3.0 Transaction Ordering Rules outlined in Section
 * 2.4.1 (PCI Express(r) Base Specification Revision 3.0 November 10, 2010).
 * As a result, on this platform we can't use Relaxed Ordering for Ingress
 * Packet Data delivery to the Free Lists because the Response Queue messages
 * notifying us of newly delivered data can sometimes arrive before the
 * Ingress Packet Data has been delivered, which leads to random memory values
 * being treated as Ingress Packet Data.  There's not currently a Linux ARM64
 * Erratum for this, so for now we're forced to simply use the Seattle
 * Platform as our test.
 */
#ifdef CONFIG_ARCH_SEATTLE
#define USE_FL0DATARO(__x) 0
#else
		       ...
		       struct sge_fl *fl, rspq_handler_t hnd)
{
	...
	if ((adapter->flags & USING_MSI) && rspq != &adapter->sge.intrq) {
		iqandst = X_INTERRUPTDESTINATION_IQ;
		intr_dest = adapter->sge.intrq.abs_id;
	} else
		iqandst = X_INTERRUPTDESTINATION_PCIE;
	...
	cmd.op_to_vfn = cpu_to_be32(V_FW_CMD_OP(FW_IQ_CMD) |
				    ...
				    F_FW_CMD_WRITE |
				    F_FW_CMD_EXEC);
	cmd.alloc_to_len16 = cpu_to_be32(F_FW_IQ_CMD_ALLOC |
					 F_FW_IQ_CMD_IQSTART |
					 V_FW_CMD_LEN16(sizeof(cmd)/16));
	cmd.type_to_iqandstindex =
		cpu_to_be32(V_FW_IQ_CMD_TYPE(FW_IQ_TYPE_FL_INT_CAP) |
			    V_FW_IQ_CMD_IQASYNCH(iqasynch) |
			    V_FW_IQ_CMD_VIID(pi->viid) |
			    V_FW_IQ_CMD_IQANDST(iqandst) |
			    V_FW_IQ_CMD_IQANUS(1) |
			    V_FW_IQ_CMD_IQANUD(X_UPDATEDELIVERY_INTERRUPT) |
			    V_FW_IQ_CMD_IQANDSTINDEX(intr_dest));
	cmd.iqdroprss_to_iqesize =
		cpu_to_be16(V_FW_IQ_CMD_IQPCIECH(pi->port_id) |
			    F_FW_IQ_CMD_IQGTSMODE |
			    V_FW_IQ_CMD_IQINTCNTTHRESH(rspq->pktcnt_idx) |
			    V_FW_IQ_CMD_IQESIZE(ilog2(rspq->iqe_len) - 4));
			   ...
			   unsigned int iqid)
{
	 ...
	 * Page after the end of the TX Queue) in units of TX Descriptors.
	 */
	...
	cmd.op_to_vfn = cpu_to_be32(V_FW_CMD_OP(FW_EQ_ETH_CMD) |
				    ...
				    F_FW_CMD_WRITE |
				    F_FW_CMD_EXEC);
	cmd.alloc_to_len16 = cpu_to_be32(F_FW_EQ_ETH_CMD_ALLOC |
					 F_FW_EQ_ETH_CMD_EQSTART |
					 V_FW_CMD_LEN16(sizeof(cmd)/16));
	cmd.autoequiqe_to_viid = cpu_to_be32(F_FW_EQ_ETH_CMD_AUTOEQUEQE |
					     V_FW_EQ_ETH_CMD_VIID(pi->viid));
	cmd.fetchszm_to_iqid =
		cpu_to_be32(V_FW_EQ_ETH_CMD_HOSTFCMODE(X_HOSTFCMODE_STATUS_PAGE) |
			    V_FW_EQ_ETH_CMD_PCIECHN(pi->port_id) |
			    V_FW_EQ_ETH_CMD_IQID(iqid));
	cmd.dcaen_to_eqsize =
		cpu_to_be32(V_FW_EQ_ETH_CMD_FBMIN(chip_ver <= CHELSIO_T5
						  ? X_FETCHBURSTMIN_64B
						  : X_FETCHBURSTMIN_64B_T6) |
	...
	txq->q.cntxt_id = G_FW_EQ_ETH_CMD_EQID(be32_to_cpu(rpl.eqid_pkd));
	txq->q.bar2_addr = bar2_address(adapter,
					txq->q.cntxt_id,
					T4_BAR2_QTYPE_EGRESS,
					&txq->q.bar2_qid);
	txq->q.abs_id = G_FW_EQ_ETH_CMD_PHYSEQID(be32_to_cpu(rpl.physeqid_pkd));
	txq->txq = devq;
	txq->tso = 0;
	txq->tx_cso = 0;
	txq->vlan_ins = 0;
...
int t4vf_sge_init(struct adapter *adapter)
{
	...
	if ((sge_params->sge_control & F_RXPKTCPLMODE) !=
	...
	s->stat_len = ((sge_params->sge_control & F_EGRSTATUSPAGESIZE)
		       ? 128 : 64);
	s->pktshift = G_PKTSHIFT(sge_params->sge_control);
	...
	case CHELSIO_T4:
		s->fl_starve_thres =
		   G_EGRTHRESHOLD(sge_params->sge_congestion_control);
		break;
	case CHELSIO_T5:
		s->fl_starve_thres =
		   G_EGRTHRESHOLDPACKING(sge_params->sge_congestion_control);
		break;
	case CHELSIO_T6:
	default:
		s->fl_starve_thres =
		   G_T6_EGRTHRESHOLDPACKING(sge_params->sge_congestion_control);
		break;
	}
