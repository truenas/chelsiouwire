 ...
 */
void t4vf_record_mbox(struct adapter *adapter, const __be64 *cmd, int size,
...
#define T4VF_RECORD_MBOX(__adapter, __cmd, __size, __access, __execute) \
	t4vf_record_mbox(__adapter, __cmd, __size, __access, __execute)

#else /* !T4VF_OS_LOG_MBOX_CMDS */

#define T4VF_RECORD_MBOX(__adapter, __cmd, __size, __access, __execute) \
	/* nothing */

#endif /* !T4VF_OS_LOG_MBOX_CMDS */

/**
 *	t4vf_record_mbox_marker - record a marker in the mailbox log
 *	@adapter: the adapter
			     ...
			     const void *marker, unsigned int size)
{
	...
	t4vf_record_mbox(adapter, marker_cmd, marker_cmd_size, 0, 0);
		      ...
		      void *rpl, bool sleep_ok)
{
	...
	u32 cmd_op = G_FW_CMD_OP(be32_to_cpu(((struct fw_cmd_hdr *)cmd)->hi));
	...
	for (i = 0; ; i++) {
		...
		if (i > 2*FW_CMD_MAX_TIMEOUT) {
			t4_os_atomic_list_del(&entry, &adapter->mbox_lock);
			ret = -EBUSY;
			T4VF_RECORD_MBOX(adapter, cmd, size, ret, 0);
			return ret;
		}
	 ...
	 * transfer ownership of the mailbox to the firmware.  (We skip
	 * loging FW_VI_STATS_CMDs because they're so prevelent and will
	 * just make it hard to read the log.)
	 *
	...
	for (i = 0; i < FW_CMD_MAX_TIMEOUT; i++) {
		...
		if (G_MBOWNER(v) == X_MBOWNER_PL) {
			...
			if (rpl) {
				...
				WARN_ON((be32_to_cpu(*(u32 *)cmd_rpl)
					 & F_FW_CMD_REQUEST) != 0);
			}
	...
	ret = -ETIMEDOUT;
	T4VF_RECORD_MBOX(adapter, cmd, size, access, ret);
	CH_ERR(adapter, "command %#x in mailbox timed out\n", *(const u8 *)cmd);
	CH_ERR(adapter, "    Control = %#x\n", t4_read_reg(adapter, mbox_ctl));
	return ret;
}
...
static inline enum cc_pause fwcap_to_cc_pause(fw_port_cap32_t fw_pause)
{
	...
	if (fw_pause & FW_PORT_CAP32_FC_TX)
		cc_pause |= PAUSE_TX;
	if ((fw_pause & FW_PORT_CAP32_ANEG) &&
	    !(fw_pause & FW_PORT_CAP32_FORCE_PAUSE))
		cc_pause |= PAUSE_AUTONEG;
...
#ifndef T4VF_OS_NEW_FW_CAPS32
	/*
	 * Provide old 16-bit Port Capabilities to Host OS Drivers which
	 * haven't been converted to use the new 32-bit Port Capabilities.
	 */
	lc->supported = fwcaps32_to_caps16(lc->pcaps);
	lc->advertising = fwcaps32_to_caps16(lc->acaps);
	lc->lp_advertising = fwcaps32_to_caps16(lc->lpacaps);
	lc->requested_speed = fwcaps32_to_caps16(lc->speed_caps);
#endif
}
...
int t4vf_port_init(struct adapter *adapter, int pidx)
{
	...
	vi_cmd.type_to_viid = cpu_to_be16(V_FW_VI_CMD_VIID(pi->viid));
	ret = t4vf_wr_mbox(adapter, &vi_cmd, sizeof(vi_cmd), &vi_rpl);
	if (ret != FW_SUCCESS)
		return ret;

	BUG_ON(pi->port_id != G_FW_VI_CMD_PORTID(vi_rpl.portid_pkd));
	pi->rss_size = G_FW_VI_CMD_RSSSIZE(be16_to_cpu(vi_rpl.norss_rsssize));
	...
	if (fw_caps == FW_CAPS16) {
		...
		u32 lstatus32 = be32_to_cpu(port_rpl.u.info32.lstatus32_to_cbllen32);
			    ...
			    union rss_vi_config *config)
{
	...
	case FW_RSS_GLB_CONFIG_CMD_MODE_BASICVIRTUAL: {
		...
		config->basicvirtual.ip6fourtupen =
			((word & F_FW_RSS_VI_CONFIG_CMD_IP6FOURTUPEN) != 0);
		config->basicvirtual.ip6twotupen =
			((word & F_FW_RSS_VI_CONFIG_CMD_IP6TWOTUPEN) != 0);
		config->basicvirtual.ip4fourtupen =
			((word & F_FW_RSS_VI_CONFIG_CMD_IP4FOURTUPEN) != 0);
		config->basicvirtual.ip4twotupen =
			((word & F_FW_RSS_VI_CONFIG_CMD_IP4TWOTUPEN) != 0);
		config->basicvirtual.udpen =
			((word & F_FW_RSS_VI_CONFIG_CMD_UDPEN) != 0);
		config->basicvirtual.defaultq =
			G_FW_RSS_VI_CONFIG_CMD_DEFAULTQ(word);
		break;
	}
			     ...
			     union rss_vi_config *config)
{
	...
	case FW_RSS_GLB_CONFIG_CMD_MODE_BASICVIRTUAL: {
		...
		if (config->basicvirtual.ip6fourtupen)
			word |= F_FW_RSS_VI_CONFIG_CMD_IP6FOURTUPEN;
		if (config->basicvirtual.ip6twotupen)
			word |= F_FW_RSS_VI_CONFIG_CMD_IP6TWOTUPEN;
		if (config->basicvirtual.ip4fourtupen)
			word |= F_FW_RSS_VI_CONFIG_CMD_IP4FOURTUPEN;
		if (config->basicvirtual.ip4twotupen)
			word |= F_FW_RSS_VI_CONFIG_CMD_IP4TWOTUPEN;
		if (config->basicvirtual.udpen)
			word |= F_FW_RSS_VI_CONFIG_CMD_UDPEN;
		word |= V_FW_RSS_VI_CONFIG_CMD_DEFAULTQ(
				config->basicvirtual.defaultq);
...
int t4vf_free_vi(struct adapter *adapter, int viid)
{
	...
	cmd.alloc_to_len16 = cpu_to_be32(FW_LEN16(cmd) |
					 F_FW_VI_CMD_FREE);
	cmd.type_to_viid = cpu_to_be16(V_FW_VI_CMD_VIID(viid));
	return t4vf_wr_mbox(adapter, &cmd, sizeof(cmd), NULL);
}
		   ...
		   bool rx_en, bool tx_en)
{
	...
	cmd.ien_to_len16 = cpu_to_be32(V_FW_VI_ENABLE_CMD_IEN(rx_en) |
		       ...
		       unsigned int nblinks)
{
	...
	cmd.op_to_viid = cpu_to_be32(V_FW_CMD_OP(FW_VI_ENABLE_CMD) |
				     ...
				     F_FW_CMD_EXEC |
				     V_FW_VI_ENABLE_CMD_VIID(viid));
	cmd.ien_to_len16 = cpu_to_be32(F_FW_VI_ENABLE_CMD_LED |
				       FW_LEN16(cmd));
			...
			u64 *hash, bool sleep_ok)
{
	...
	for (offset = 0; offset < naddr; /**/) {
		...
		cmd.freemacs_to_len16 =
			cpu_to_be32(V_FW_VI_MAC_CMD_FREEMACS(free) |
	...
	if (ret == 0) {
		...
		if (smt_idx) {
			...
			if (adapter->params.viid_smt_extn_support)
				*smt_idx = G_FW_VI_MAC_CMD_SMTID(be32_to_cpu(rpl.op_to_viid));
			else {
		       ...
		       bool ucast, u64 vec, bool sleep_ok)
{
	...
	val = V_FW_VI_MAC_CMD_ENTRY_TYPE(FW_VI_MAC_TYPE_HASHVEC) |
	      V_FW_VI_MAC_CMD_HASHUNIEN(ucast) | V_FW_CMD_LEN16(len16);
	cmd.freemacs_to_len16 = cpu_to_be32(val);
 ...
 *	commands.
 */
int t4vf_get_port_stats_fw(struct adapter *adapter, int pidx,
			...
			struct t4vf_port_stats *s)
{
	 ...
	 */
	if (pidx != 0)
		return t4vf_get_port_stats_fw(adapter, pidx, s);
	...
	s->tx_bcast_bytes = GET_STAT(TX_VF_BCAST_BYTES);
	s->tx_bcast_frames = GET_STAT(TX_VF_BCAST_FRAMES);
	s->tx_mcast_bytes = GET_STAT(TX_VF_MCAST_BYTES);
	s->tx_mcast_frames = GET_STAT(TX_VF_MCAST_FRAMES);
	s->tx_ucast_bytes = GET_STAT(TX_VF_UCAST_BYTES);
	s->tx_ucast_frames = GET_STAT(TX_VF_UCAST_FRAMES);
	s->tx_drop_frames = GET_STAT(TX_VF_DROP_FRAMES);
	s->tx_offload_bytes = GET_STAT(TX_VF_OFFLOAD_BYTES);
	s->tx_offload_frames = GET_STAT(TX_VF_OFFLOAD_FRAMES);

	s->rx_bcast_bytes = GET_STAT(RX_VF_BCAST_BYTES);
	s->rx_bcast_frames = GET_STAT(RX_VF_BCAST_FRAMES);
	s->rx_mcast_bytes = GET_STAT(RX_VF_MCAST_BYTES);
	s->rx_mcast_frames = GET_STAT(RX_VF_MCAST_FRAMES);
	s->rx_ucast_bytes = GET_STAT(RX_VF_UCAST_BYTES);
	s->rx_ucast_frames = GET_STAT(RX_VF_UCAST_FRAMES);

	s->rx_err_frames = GET_STAT(RX_VF_ERR_FRAMES);
	#undef GET_STAT
		 ...
		 unsigned int iqid, unsigned int fl0id, unsigned int fl1id)
{
	...
	cmd.op_to_vfn = cpu_to_be32(V_FW_CMD_OP(FW_IQ_CMD) |
				    F_FW_CMD_REQUEST |
				    F_FW_CMD_EXEC);
	cmd.alloc_to_len16 = cpu_to_be32(F_FW_IQ_CMD_FREE |
					 FW_LEN16(cmd));
...
int t4vf_eth_eq_free(struct adapter *adapter, unsigned int eqid)
{
	...
	cmd.op_to_vfn = cpu_to_be32(V_FW_CMD_OP(FW_EQ_ETH_CMD) |
				    F_FW_CMD_REQUEST |
				    F_FW_CMD_EXEC);
	cmd.alloc_to_len16 = cpu_to_be32(F_FW_EQ_ETH_CMD_FREE |
					 FW_LEN16(cmd));
	cmd.eqid_pkd = cpu_to_be32(V_FW_EQ_ETH_CMD_EQID(eqid));
	return t4vf_wr_mbox(adapter, &cmd, sizeof(cmd), NULL);
}
			       ...
			       const struct fw_port_cmd *cmd)
{
	 ...
	 * Extract the various fields from the Port Information message.
	 */
	action = G_FW_PORT_CMD_ACTION(be32_to_cpu(cmd->action_to_len16));
	switch (action) {
...
#ifndef T4VF_OS_NEW_FW_CAPS32
		/*
		 * Provide old 16-bit Port Capabilities to Host OS Drivers
		 * which haven't been converted to use the new 32-bit Port
		 * Capabilities.
		 */
		lc->supported = fwcaps32_to_caps16(lc->pcaps);
		lc->advertising = fwcaps32_to_caps16(lc->acaps);
...
int t4vf_handle_fw_rpl(struct adapter *adapter, const __be64 *rpl)
{
	...
	case FW_PORT_CMD: {
		...
		int action = G_FW_PORT_CMD_ACTION(
			be32_to_cpu(port_cmd->action_to_len16));
		int port_id, pidx;

		if (action != FW_PORT_ACTION_GET_PORT_INFO &&
		    action != FW_PORT_ACTION_GET_PORT_INFO32) {
			CH_ERR(adapter, "Unknown firmware PORT reply"
			       " action %#x\n", action);
			break;
		}
	...
	default:
		CH_ERR(adapter, "Unknown firmware reply %X\n", opcode);
	}
	return 0;
}
...
/**
 */
int t4vf_prep_adapter(struct adapter *adapter, u32 device_id)
{
	int err;
	unsigned int chipid;
	...
	switch (CHELSIO_PCI_ID_VER(device_id)) {
	case CHELSIO_T4_FPGA:
		adapter->params.chip |= CHELSIO_CHIP_FPGA;
		/*FALLTHROUGH*/
	case CHELSIO_T4:
