...
void t4vf_os_link_changed(struct adapter *adapter, int pidx, int link_ok)
{
	...
	if (link_ok) {
		...
		u8 cur_fc = pi->link_cfg.fc & ~PAUSE_AUTONEG;
		const char *s;
 ...
 *	@smt_idx: if valid add the address to the HW SMT and
 *	          store the SMT index
 *
			  ...
			  const struct pkt_gl *gl)
{
	...
	case CPL_FW6_MSG: {
		...
		else if (fw_msg->type == FW_TYPE_WRERR_RPL) {
			const struct fw_error_cmd *cmd =
						(const void *)fw_msg->data;
			...
			if (printk_ratelimit()) {
				if (opcode == FW_ERROR_TYPE_ACL)
					dev_warn(adapter->pdev_dev,
						 "ACL error received for Tx ring %u\n",
...
static int extension_do_ioctl(struct net_device *dev, void __user *useraddr)
{
	...
	case CHELSIO_GET_FEC: {
		...
		/*
		 * Translate the current internal Requested FEC support into
		 * the cxgbtool enum value.
		 */
		t.auto_fec      = cc_to_cx_fec(lc->auto_fec);
		t.requested_fec = cc_to_cx_fec(lc->requested_fec);
	...
	case CHELSIO_SET_QSET_PARAMS: {
		...
		txq = &s->ethtxq[t.qset_idx + pi->first_qset];
		rxq = &s->ethrxq[t.qset_idx + pi->first_qset];
	...
	case CHELSIO_GET_QSET_PARAMS: {
		...
		t.intr_lat    = rspq_intr_timer(s, &rxq->rspq);
		t.cong_thres  = 0;
	...
	case CHELSIO_SET_QUEUE_INTR_PARAMS: {
		...
		if (op.qid < s->ingr_base ||
			...
			return -EINVAL;
		rspq = IQ_MAP(s, op.qid);
		if (rspq == NULL)
			return -EINVAL;

		cur_us = rspq_intr_timer(s, rspq);
		cur_cnt = rspq_intr_pktcnt(s, rspq);

		new_us = op.timer >= 0 ? op.timer : cur_us;
		new_cnt  = op.count >= 0 ? op.count : cur_cnt;
		ret = set_rspq_intr_params(rspq, new_us, new_cnt);
	...
	case CHELSIO_GET_QUEUE_INTR_PARAMS: {
		...
		op.timer = rspq_intr_timer(s, rspq);
		op.count = rspq_intr_pktcnt(s, rspq);
	...
	case CHELSIO_SET_QSET_NUM: {
		 ...
		 * for "edata.val + other_qsets > s->max_ethqsets",
		...
		other_qsets = s->ethqsets - pi->nqsets;
		if (edata.val + other_qsets > s->max_ethqsets ||
		    edata.val > pi->rss_size)
			return -EINVAL;
		pi->nqsets = edata.val;
		netif_set_real_num_tx_queues(dev, pi->nqsets);
		netif_set_real_num_rx_queues(dev, pi->nqsets);
		s->ethqsets = other_qsets + pi->nqsets;
...
static int cxgb4vf_set_mac_addr(struct net_device *dev, void *_addr)
{
	...
	ret = cxgb4vf_change_mac(pi, pi->viid,
				 &pi->xact_addr_filt,
				 addr->sa_data, true, NULL);
	if (ret < 0)
		return ret;

	ether_addr_copy(dev->dev_addr, addr->sa_data);
	ether_addr_copy(dev->perm_addr, addr->sa_data);
	return 0;
}
				...
				struct ethtool_coalesce *coalesce)
{
	...
	const struct sge_rspq *rspq = &s->ethrxq[pi->first_qset].rspq;

	coalesce->rx_coalesce_usecs = rspq_intr_timer(s, rspq);
	coalesce->rx_max_coalesced_frames = rspq_intr_pktcnt(s, rspq);
	return 0;
}
				   ...
				   struct ethtool_pauseparam *pauseparam)
{
	...
	pauseparam->autoneg = (pi->link_cfg.fc & PAUSE_AUTONEG) != 0;
...
{
	const struct adapter *adapter = seq->private;
	const struct sge *s = &adapter->sge;
	int eth_entries = DIV_ROUND_UP(s->ethqsets, QPL);
	...
	if (r < eth_entries) {
		const struct sge_eth_rxq *rxq = &s->ethrxq[r * QPL];
		const struct sge_eth_txq *txq = &s->ethtxq[r * QPL];
		int n = min(QPL, s->ethqsets - QPL * r);
		...
		R("RspQ Gen:", rspq.gen);
		S3("u", "Intr delay:", rspq_intr_timer(s, &rxq[qs].rspq));
		S3("u", "Intr pktcnt:", rspq_intr_pktcnt(s, &rxq[qs].rspq));
		R("FL ID:", fl.abs_id);
	...
	if (r == 0) {
		...
		const struct sge_rspq *intrq = &s->intrq;
		...
		seq_printf(seq, "%-12s %16u\n", "RspQ Gen:", intrq->gen);
		seq_printf(seq, "%-12s %16u\n", "Intr delay:",
			   rspq_intr_timer(s, intrq));
		seq_printf(seq, "%-12s %16u\n", "Intr pktcnt:",
			   rspq_intr_pktcnt(s, intrq));
	}
...
static int adap_init0(struct adapter *adapter)
{
	...
	ret = t4vf_query_params(adapter, 1, &param, &val);
	adapter->params.viid_smt_extn_support = (ret == 0 && val != 0);
...
static int enable_msix(struct adapter *adapter)
{
	...
	allocated = pci_alloc_irq_vectors(adapter->pdev, need, want,
					  PCI_IRQ_MSIX);
	if (allocated < 0) {
	...
	for (i = 0; i < allocated; ++i)
		adapter->msix_info[i].vec = pci_irq_vector(adapter->pdev, i);

	dev_info(adapter->pdev_dev, "%d MSI-X vectors allocated, vnic %d",
				       ...
				       const struct pci_device_id *ent)
{
	...
	if (msi == MSI_MSIX && enable_msix(adapter) == 0)
		...
		allocated = pci_alloc_irq_vectors(pdev, 1, 1, PCI_IRQ_MSI);
		if (allocated != 1) {
	...
	for_each_port(adapter, pidx) {
		...
		adapter->port[pidx]->dev_port = pi->port_id;
