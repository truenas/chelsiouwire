...
enum { /* taken from t4_hw.h */
	...
	CSIO_T6_PM_NSTATS   = 7,     /* # of PM stats in T6 */
...
typedef struct _csio_portinfo_t {
	uint8_t		link_ok;
	uint32_t	pcaps;
}CSIO_PORTINFO, *PCSIO_PORTINFO, csio_portinfo_t;
...
typedef struct _csio_hw_info_t {
	...
	csio_portinfo_t		pi[CSIO_MAX_T4PORTS];
...
typedef struct _csio_pm_stats {
	uint32_t tx_cnt[CSIO_T6_PM_NSTATS];
	uint32_t rx_cnt[CSIO_T6_PM_NSTATS];
	uint64_t tx_cyc[CSIO_T6_PM_NSTATS];
	uint64_t rx_cyc[CSIO_T6_PM_NSTATS];
