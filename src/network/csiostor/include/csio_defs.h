...
#define csio_tcp_dbg(__hw, __fmt, ...)	\
do {	\
	csio_oss_err((__hw)->os_dev, "%ld: %s: "__fmt, jiffies, \
...
#define CSIO_GET_STATS(elem, val)		(elem)->stats.val
#define CSIO_SET_STATS(elem, val, __cnt) 	(elem)->stats.val = (__cnt)
#define CSIO_INC_STATS(elem, val)		(elem)->stats.val++
#define CSIO_DEC_STATS(elem, val)		(elem)->stats.val--
#define CSIO_INC_STATS(elem, val)	(elem)->stats.val++
#define CSIO_DEC_STATS(elem, val)	(elem)->stats.val--
#define CSIO_INC_STATS_BY(elem, val, __cnt)     ((elem)->stats.val += (__cnt))
#define CSIO_DEC_STATS_BY(elem, val, __cnt)     ((elem)->stats.val -= (__cnt))
...
typedef struct csio_oss_rwlock csio_rwlock_t;
...
#define csio_rwlock_init(__l)		csio_oss_rwlock_init((__l))
#define csio_read_lock(_hw, __l)			\
			csio_oss_read_lock((_hw)->os_dev, (__l))
#define csio_read_unlock(_hw, __l)			\
			csio_oss_read_unlock((_hw)->os_dev, (__l))
#define csio_write_lock_irqsave(_hw, __l, __f) 		\
			csio_oss_write_lock_irqsave((_hw)->os_dev, (__l), (__f))
#define csio_write_unlock_irqrestore(_hw, __l, __f) 		\
			csio_oss_write_unlock_irqrestore((_hw)->os_dev, (__l), (__f))
...
#define csio_container_of(__p, __t, __m)	csio_oss_container_of(__p, __t, __m)
...
#define csio_dma_alloc_ddp_buf(__dobj, __dev, __sz, __aln, __phys, __fl)	     \
					csio_oss_dma_alloc_ddp_buf((__dobj), (__dev),\
					(__sz), (__aln), (__phys), (__fl))
#define csio_dma_free_ddp_buf(__d, __v)		csio_oss_dma_free_ddp_buf((__d), (__v))
...
#define csio_is_kdump_kernel(__dev)     csio_oss_is_kdump_kernel((__dev))
