...
#define CSIO_MAX_SCSI_QSETS	(CSIO_MAX_SCSI_CPU * CSIO_MAX_SQS_CLNT)

#ifdef __CSIO_PORT_EVTQ__
#define CSIO_MAX_MSIX_VECS	\
	(CSIO_MAX_SCSI_QSETS + CSIO_EXTRA_VECS + CSIO_MAX_T4PORTS)
#else
...
#define	CSIO_SQS_CLNT_INIT	0
#define CSIO_SQS_CLNT_TRGT	1

#define CSIO_ISNS_IQ_IDX_CPU	0
...
struct csio_scsi_cpu_info {
	int16_t	max_cpus;
#if defined(__CSIO_TARGET__) || defined(__CSIO_COISCSI_ENABLED__)
	int16_t cur_iq_cpu;
...
struct csio_os_hw {
	...
	uint32_t num_scsi_msix_cpus;	/* Number of CPUs that will be used 
					 ...
					 */
	struct csio_scsi_qset sqset[CSIO_MAX_SQS_CLNT][CSIO_MAX_SCSI_CPU];
	struct csio_scsi_cpu_info scsi_cpu_info[CSIO_MAX_SQS_CLNT];
	...
	/* Private data */
	unsigned long prv_data[0] __attribute__((aligned (sizeof(unsigned long))));
};

/* Helper */

static inline void *csio_oshw_to_prv_data(struct csio_os_hw *oshw)
{
	return (void*)oshw->prv_data;
}
