...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3,18,0)) && defined(CONFIG_CRASH_DUMP)
/* For kernels < 3.18, elfcorehdr_addr variable used in is_kdump_kernel()
 * was not exported. Hence use reset_devices instead to detect kdump kernel.
 */
#define csio_oss_is_kdump_kernel(_dev)		reset_devices
#else
#define csio_oss_is_kdump_kernel(_dev)		is_kdump_kernel()
...
#define csio_oss_dma_alloc_ddp_buf(__dobj, __dev, __sz, __aln, __phys, __fl)    \
					csio_oss_dma_alloc((__dobj), (__dev),\
					(__sz), (__aln), (__phys), (__fl))
#define csio_oss_dma_free_ddp_buf(__d, __v)	csio_oss_dma_free((__d), (__v))
...
#define csio_oss_rwlock_init(l)		rwlock_init(&(l)->oss_lock)
#define csio_oss_read_lock(_osdev, l)	read_lock(&(l)->oss_lock)
#define csio_oss_read_unlock(_osdev, l)	read_unlock(&(l)->oss_lock)
#define csio_oss_write_lock_irqsave(_osdev, l, f)	\
				write_lock_irqsave(&(l)->oss_lock, (f))
#define csio_oss_write_unlock_irqrestore(_osdev, l, f)	\
				write_unlock_irqrestore(&(l)->oss_lock, (f))
...
#define csio_oss_dbg(pdev, fmt, arg...) 	\
	dev_printk(KERN_INFO, &((struct pci_dev *)pdev)->dev, "%s: "fmt, __FUNCTION__,##arg)
#else
#define csio_oss_dbg(pdev, fmt, arg...) 
#endif
...
/* Timers */
#define csio_oss_container_of(ptr, type, member) \
...
void csio_oss_timer_init(struct csio_oss_timer *,
				void (*)(struct csio_oss_timer *), unsigned int);
