		     ...
		     struct csio_fl_dma_buf *flb, void *priv, uint8_t **scsiwr)
{
	...
	if(csio_is_fcoe(hw)) {
		...
		if (csio_tgt_isr(hw, wr, len, flb, priv))
			return NULL;
#endif
	} else if(csio_is_iscsi(hw)) {
#ifdef __CSIO_COISCSI_ENABLED__
		if (csio_coiscsi_tgt_isr(hw, wr, len, flb, priv))
			return NULL;
#endif
	}
	...
	if (csio_unlikely(cpl->opcode != CPL_FW6_MSG)) {
		csio_err(hw, "Error: Invalid CPL msg 0x%x instead of 0x%x recvd on SCSI q\n",
			cpl->opcode, CPL_FW6_MSG);
		...
		int buf_size, int num_buf)
{
	...
	for (n = 0; n < num_buf; n++) {
		...
		ddp_desc->vaddr = csio_dma_alloc_ddp_buf(&ddp_desc->dmahdl, hw->os_dev,
						unit_size, 8,
...
no_mem:
	...
	csio_list_for_each(tmp, &scm->ddp_freelist) {
		...
		csio_dma_free_ddp_buf(&ddp_desc->dmahdl, ddp_desc->vaddr);
