...
static int
csio_foiscsi_get_ibft_data(struct csio_hw *hw,
			   void *ibft_loc, struct foiscsi_login_info *linfo)
{
	...
	if (!nic->ip_address[0] && !nic->ip_address[1] && !nic->ip_address[2] &&
		...
		subop = CSIO_CHNET_IFCONF_IPV4_SET_IOCTL;
	} else {
		csio_dbg(hw,"\nSetting IP address of the Chelsio NIC "
				"to %d.%d.%d.%d.%d.%d.%d.%d\n",
		...
		subop = CSIO_CHNET_IFCONF_IPV6_SET_IOCTL;
	}
...
retry_find_iface:
	for (j = 0; j < hw->num_t4ports; j++) {
		iface = &chnet_cdev->ifaces[hw->t4port[j].portid];
	...
	for (i = 0; i < link_tmo; i+=delay) {
		iface = &chnet_cdev->ifaces[(((int8_t)ifconf_req->ifid < 0) ? 0 : ifconf_req->ifid)];

		if (iface->if_state == (int)CHNET_IFACE_STATE_LINK_UP)
			break;
	...
	if ((vlan_id & 0x0fff) >=2 && (vlan_id & 0x0fff) < 4095) {
		...
		csio_transport_ioctl_handler(hw,
			...
			0, ifconf_req, sizeof(struct csio_chnet_ifconf_ioctl));
...
void csio_foiscsi_ibft_login(struct csio_hw *hw)
{
	...
	if (csio_foiscsi_get_ibft_data(hw, csio_ibft_addr, linfo))
		return;
	csio_transport_ioctl_handler(hw,
			CSIO_IOCTL_TYPE_FOISCSI,
