...
void csio_foiscsi_clear_logout_all(struct csio_hw *hw, int id) {

	struct csio_lnode *ln = csio_foiscsi_get_lnode(hw, id);
	struct csio_lnode_iscsi *lni = NULL;
	...
	if (lni)
		lni->logout_all = 0;
	csio_info(hw, "%s id %u ln 0x%p lni 0x%p\n",
			__func__, id, ln , lni);
	return;
}
...
static void csio_foiscsi_iface_cmd_mb_init(struct fw_chnet_iface_cmd *cmdp,
			uint8_t portid, uint8_t flags,
#ifdef __CSIO_PORT_EVTQ__
			uint8_t flags_ext, uint8_t subop, uint16_t rss_iqid)
#else
			uint8_t subop)
#endif
{
...
#ifdef __CSIO_PORT_EVTQ__
	cmdp->retval_len16 |= csio_htonl(V_FW_CHNET_IFACE_CMD_RSS_IQID(
			      rss_iqid));
	cmdp->flags = flags_ext;
#endif
	cmdp->subop = subop;
	cmdp->r2[0] = flags;
	return;
...
#ifdef __CSIO_PORT_EVTQ__
	csio_foiscsi_iface_cmd_mb_init(&c, portid, flags,
				F_FW_CHNET_IFACE_CMD_RSS_IQID_F, link_op,
				csio_q_physiqid(hw, hw->portevt_iq_idx[portid]));
#else
	...
	struct foiscsi_logout_info *linfo, unsigned long handle)
{
	...
	if (linfo->sess_id < 0) {
		 ...
		 * we return error or CSIO_EZERO_OBJ_FOUND */
		lni->logout_all = 1;
	}
	...
	if (!csio_list_empty(&ln->rnhead)) {
		...
		csio_list_for_each_safe(tmp, next, rnhead) {
			...
			if ((csio_rnism_in_ready(rni) || csio_rnism_in_reconnect(rni) ||
				csio_rnism_in_recovery(rni)) && !found &&
				(lni->logout_all || linfo->sess_id == rni->sess_id) && !rni->logged_out) {
				rni->logged_out = 1;
				csio_info(hw, "%s: Logging out from session-id [%d].\n",
						__FUNCTION__, rni->sess_id);
				       ...
				       struct foiscsi_login_info *linfo)
{
	...
	csio_list_for_each_safe(tmp, next, rnhead) {
		...
		if ((strlen(linfo->tgt_name) ==
			...
			if ((linfo->ip_type ==  CSIO_CHNET_L3CFG_TYPE_IPV4) &&
			    (linfo->tgt_ip.ip4 == rni->login_info.tgt_ip.ip4) &&
			    (linfo->src_ip.ip4 == rni->login_info.src_ip.ip4))
				return 1;
			else if ((linfo->ip_type == CSIO_CHNET_L3CFG_TYPE_IPV6) &&
...
csio_get_rni(struct csio_lnode *ln)
{
	...
	if (!rn) {
		...
		if(csio_is_iscsi(hw)) {
			cpu = (rni->sess_id) %
				(oshw->scsi_cpu_info[CSIO_SQS_CLNT_INIT].max_cpus);
			sqset = &oshw->sqset[CSIO_SQS_CLNT_INIT][cpu];
			rn->eq_idx = hw->mgmtm.eq_idx;
		...
		bool do_disc, unsigned long handle)
{
	...
	if (sess_type == FW_FOISCSI_SESSION_TYPE_NORMAL) {
		if (csio_foiscsi_session_exists(lni, linfo)) {
			...
			linfo->status = CSIO_ESESSION_EXISTS;
			rc = CSIO_ESESSION_EXISTS;
			goto lni_mutex_unlock;
		}
	}

	if (G_FW_FOISCSI_CTRL_WR_AUTH_METHOD(linfo->conn_attr.hdigest_to_tcp_ws_en) &&
		G_FW_FOISCSI_CTRL_WR_AUTH_POLICY(linfo->conn_attr.hdigest_to_tcp_ws_en)) {
		...
		struct foiscsi_instance *ini_inst)
{
	...
	if (ini_inst->id > 0 && ini_inst->id <= FW_FOISCSI_INIT_NODE_MAX) {
		...
		return CSIO_EINVALID_INIT_INST;
	}
		...
		struct foiscsi_instance *ini_inst, unsigned long handle)
{
	...
	if (!ln) {
		csio_tcp_dbg(hw, "node id %d doesnot exist\n", ini_inst->id);
		return CSIO_EINST_NOT_FOUND;
	}
	...
	lni->inst.recovery_timeout = ini_inst->recovery_timeout;
	lni->inst.login_retry_cnt = ini_inst->login_retry_cnt;
		...
		struct foiscsi_instance *ini_inst)
{
	...
	lni = csio_lnode_to_iscsi(ln);
	if (lni->valid) {
		csio_dbg(hw, "Add: name = %s alias = %s vend_key %s vend_val "
			"%s at idx = %d\n",
			lni->inst.name, lni->inst.alias,
			ini_inst->vend_key, ini_inst->vend_val, lni->inode_id);
		...
		/*
		 * Using the same FW_FOISCSI_CHAP_WR for VENDOR specific
		 * key value pair as well.
		 */
		strcpy(lni->inst.vend_key, ini_inst->vend_key);
		strcpy(lni->inst.vend_val, ini_inst->vend_val);

		csio_issue_foiscsi_chap_wr(hw, lni, NULL,
				lni->inode_id, lni->inode_flowid,
				FW_FOISCSI_NODE_TYPE_INITIATOR, 1);
	}
	return CSIO_SUCCESS;
...
csio_retval_t
csio_foiscsi_do_pmtu6_clean_req(struct csio_hw *hw, uint8_t opc,
				unsigned int if_id,
				...
				unsigned long handle)
{
	...
	unsigned long flags;
	uint64_t cookie_val = (uint64_t)handle;
	...
	ifconf_wr.subop = FW_CHNET_IFCONF_WR_SUBOP_PMTU6_CLEAR;

	if (csio_vlan_valid(req->vlanid)) {
		ifconf_wr.param.vlanid = csio_cpu_to_be16(req->vlanid);
		cookie_val |= (req->vlanid << 8);
	}

	csio_dbg(hw, "cookie_val 0x%x\n", (unsigned int)cookie_val);
	ifconf_wr.cookie = csio_cpu_to_be64(cookie_val);
			...
			unsigned int if_id, uint16_t mtu, unsigned long handle)
{
	...
	ifconf_wr.subop = ((opc == CSIO_CHNET_IFCONF_MTU_SET_IOCTL) ?
...
csio_foiscsi_ifconf_ip_set(struct csio_hw *hw, int8_t opc, unsigned int if_id,
		struct csio_chnet_ifconf_ioctl *req, unsigned long handle)
{
	enum csio_oss_error rc = CSIO_SUCCESS;
...
csio_retval_t
csio_foiscsi_ifconf_ip_ping(struct csio_hw *hw, int8_t opc, unsigned int if_id,
		struct csio_chnet_ifconf_ioctl *req, unsigned long handle)
{
	enum csio_oss_error rc = CSIO_SUCCESS;
	struct csio_wr_pair wrp;
	...
	if (req->type == CSIO_CHNET_L3CFG_TYPE_IPV4) {
		ifconf_wr.subop = FW_CHNET_IFCONF_WR_SUBOP_ICMP_PLD_PING4;

		ifconf_wr.param.in_attr.ipv4.addr =
				csio_cpu_to_be32(req->v4.ipv4_addr);
	} else { /* IPv6 */
		ifconf_wr.subop = FW_CHNET_IFCONF_WR_SUBOP_ICMP_PLD_PING6;
	...
	ifconf_wr.u.ping.ping_time = csio_cpu_to_be16(req->ping_time);
	ifconf_wr.u.ping.ping_seq = csio_cpu_to_be16(req->ping_seq);
	ifconf_wr.u.ping.ping_param_rspcode_to_fin_bit = req->ping_param_rspcode;
	ifconf_wr.param.ping_pldsize = csio_cpu_to_be16(req->ping_pldsize);
	csio_dbg(hw, "ifconf ping fin 0x%x\n", ifconf_wr.u.ping.ping_param_rspcode_to_fin_bit);
	if (csio_vlan_valid(req->vlanid)) {
		ifconf_wr.param.vlanid = csio_cpu_to_be16(req->vlanid);
		cookie_val |= (req->vlanid << 8);
	}
...
#ifdef __CSIO_DEBUG__
	if (rc)
		csio_err(hw, "%s: Out of credits, cannot allocate wr\n", __FUNCTION__);
	else
		csio_dump_wr_buffer((uint8_t *)&ifconf_wr, sizeof(struct fw_chnet_ifconf_wr));
#endif
		...
		struct csio_chnet_ifconf_ioctl *req, unsigned long handle)
{
	...
	ifconf_wr.cookie = (uint64_t)handle;
	if (req->type == CSIO_CHNET_L3CFG_TYPE_DHCP)
		...
		unsigned int if_id, uint16_t vlanid, unsigned long handle)
{
	...
	if (opc == CSIO_CHNET_IFCONF_VLAN_SET_IOCTL) {
		...
		u8 status)
{
	...
	int state = 0, len = 0, rc = CSIO_SUCCESS, cpu = 0;
	...
	if (status) {
		...
		case FW_EINVAL:
			...
			rc = CSIO_EINVALID_REQUEST;
			break;

		case FW_ENETUNREACH:
			csio_err(hw, "Network not reachable\n");
			rc = CSIO_ELOGIN_TIMEDOUT;
			break;
	...
	csio_err(hw, "%s rni->sess_id [0x%x] state %d handle [0x%x] f 0x%x\n",
			__FUNCTION__, rni->sess_id, state, rni->sess_handle,
			rni->flags);
	...
	case FW_FOISCSI_CTRL_STATE_FREE:
		...
		if (!status && (rni->sess_type ==
					...
					rni->flags & CSIO_RNI_CHAPL_PENDING) {
			rni->flags &= ~CSIO_RNI_CHAPL_PENDING;
			if (csio_issue_foiscsi_chap_wr(hw, NULL, rni,
					0, rni->sess_handle,
					FW_FOISCSI_NODE_TYPE_TARGET, 0)) {
				rc = -CSIO_INVAL;
			}
			if (!rc)
				break;
	...
	case FW_FOISCSI_CTRL_STATE_ONLINE:
		...
		if (rni->sess_type == FW_FOISCSI_SESSION_TYPE_DISCOVERY) {
			...
			if (!(csio_be32_to_cpu(ctrl_wr->op_to_no_fin) & F_FW_WR_COMPL)) {
				csio_spin_unlock_irqrestore(hw, &hw->lock, flags);
				break;
			}
		...
		if (rni->sess_type != FW_FOISCSI_SESSION_TYPE_DISCOVERY)
			csio_dbg(hw, "%s: wscale:%u\n", __FUNCTION__, 
				G_FW_FOISCSI_CTRL_WR_TCP_WS(csio_be32_to_cpu(
				ctrl_wr->conn_attr.hdigest_to_tcp_ws_en)));
