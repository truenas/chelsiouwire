...
{	
	...
	WARN_ON(val_len > U16_MAX);

	len = val_len;

	ae->type = csio_htons(type);
	len += 4; 		/* includes attribute type and length */
	pad_len = (len & 0x3) ? (4 - (len & 0x3)) : 0;
	len += pad_len;

	ae->len = csio_htons(len);
	csio_memcpy(ae->val.string, val, val_len);
	if (pad_len)
		csio_memset(ae->val.string + val_len, 0, pad_len);
	*ptr += len;
}
...
csio_lnf_fdmi_rhba_cbfn(struct csio_hw *hw, struct csio_ioreq *fdmi_req)
{
	...
	if (hw->adap.pi[lnf->ln->portid].link_cfg.pcaps &
			FW_PORT_CAP32_SPEED_100G)
		val = CSIO_HBA_PORTSPEED_100GBIT;
	else if (hw->adap.pi[lnf->ln->portid].link_cfg.pcaps &
			FW_PORT_CAP32_SPEED_50G)
		val = CSIO_HBA_PORTSPEED_50GBIT;
	else if (hw->adap.pi[lnf->ln->portid].link_cfg.pcaps &
			FW_PORT_CAP32_SPEED_40G)
		val = CSIO_HBA_PORTSPEED_40GBIT;
	else if (hw->adap.pi[lnf->ln->portid].link_cfg.pcaps &
			FW_PORT_CAP32_SPEED_25G)
		val = CSIO_HBA_PORTSPEED_25GBIT;
	else if (hw->adap.pi[lnf->ln->portid].link_cfg.pcaps &
