...
static int rnode_show(struct seq_file *seq, void *v)
{
	...
	while ((ln = csio_fcoe_get_next_lnode(hw, vnpi)) != NULL) {
		...
		csio_list_for_each(cur_rn, &rnhead->rnlist) {
...
static int isession_show(struct seq_file *seq, void *v)
{
	...
	csio_list_for_each(tmp, sln_head) {
		...
		csio_list_for_each_safe(curr_rnode, next, rnhead) {
			...
			seq_printf(seq, "\tconn flowid: 0x%x\n", rn->flowid);
			if (rni->login_info.ip_type == CSIO_CHNET_L3CFG_TYPE_IPV4)
			...
			else if (rni->login_info.ip_type == CSIO_CHNET_L3CFG_TYPE_IPV6)
...
static int coiscsi_sessions_show(struct seq_file *seq, void *v)
{
	struct csio_os_hw *oshw = csio_adap_to_oshw((struct adapter *)seq->private);
	struct csio_hw *hw = csio_oshw_to_hw(oshw);
	...
	size_t sz = 128 * 1024; //ISCSI_CONTROL_DATA_MAX_BUFLEN
	...
	csio_list_for_each(tmp, &tgtm->sln_head) {
		...
		if (lncoi != NULL) {
			if (chiscsi_handlers && chiscsi_handlers->display_target_info)
				chiscsi_handlers->display_target_info(&lncoi->tinst, dbuf);
			seq_printf(seq, "%s\n", dbuf);
			memset(dbuf, 0, sz);
		}
	}
...
DEFINE_SIMPLE_DEBUGFS_FILE(coiscsi_sessions);

static int
coiscsi_tgtm_stats_show(struct seq_file *seq, void *v)
{
	struct csio_os_hw *oshw = csio_adap_to_oshw((struct adapter *)seq->private);
	struct csio_hw *hw = csio_oshw_to_hw(oshw);
	struct csio_coiscsi_tgtm *tgtm = csio_hw_to_coiscsi_tgtm(hw);

	struct csio_coiscsi_tgtm_stats *stats = &tgtm->stats;;

	seq_printf(seq, "COISCSI Target Module Statistics:\n");
	seq_printf(seq, "\tGood completions: %lld\n", stats->n_good_cmpl);
	seq_printf(seq, "\tMax outstanding I/Os reached: %d\n",
		   stats->n_max_active);
	seq_printf(seq, "\tNo. of I/Os Aborted at Driver: %d\n",
		   stats->n_abrtd_driver);
	seq_printf(seq, "\tNo. of I/Os Aborted at FW: %d\n",
		   stats->n_abrtd_fw);
	seq_printf(seq, "\n\n");

	seq_printf(seq, "COISCSI Target Module Counters:\n");
	seq_printf(seq, "\tNo. of Active I/Os: %d\n", stats->n_active);
	seq_printf(seq, "\tNo. of I/Os being drained: %d\n",
		   stats->n_draining);
	seq_printf(seq, "\tNo. of free tgtreq: %u\n",
		   stats->n_free_tgtreq);
	seq_printf(seq, "\tNo. of free rcvreq: %u\n",
		   stats->n_free_rcvreq);
	seq_printf(seq, "\tNo. of free dbuf: %u\n",
		   stats->n_free_dbuf);
	seq_printf(seq, "\tNo. of ddp miss: %u\n",
		   stats->n_ddp_miss);
	seq_printf(seq, "\tNo. of ddp pass: %llu\n",
		   stats->n_ddp_pass);
	seq_printf(seq, "\tNo. of ddp skip: %u\n",
		   stats->n_ddp_skip);
	seq_printf(seq, "\tNo. of non-zero page offset: %llu\n",
		   stats->n_poff_cnt);
	seq_printf(seq, "\tNo. of ipv4 listen conns: %d\n",
		   stats->n_ipv4_sn_cnt);
	seq_printf(seq, "\tNo. of ipv6 listen conns: %d\n",
		   stats->n_ipv6_sn_cnt);
	seq_printf(seq, "\tNo of credit requests: %d \n",
		   stats->tcredit_req_cnt);
	seq_printf(seq, "\tAvailaable credits : %d \n",
		   stats->tcredit_avail);
	seq_printf(seq, "\tMax credit requests: %d \n",
		   stats->tcredit_req_max);
	seq_printf(seq, "\tISO: NOT SUPPORTED\n");
	seq_printf(seq, "\tLRO: NOT SUPPORTED\n");

	return 0;
}

static int
coiscsi_tgtm_stats_open(struct inode *inode, struct file *file)
{
	return single_open(file, coiscsi_tgtm_stats_show, inode->i_private);
}

#if 0
static ssize_t
coiscsi_tgtm_stats_clear(struct file *file, const char __user *buf,
...
static const struct file_operations coiscsi_tgtm_stats_fops = {
	.owner   = THIS_MODULE,
	.open    = coiscsi_tgtm_stats_open,
	.read    = seq_read,
	.llseek  = seq_lseek,
	.release = single_release,
	/*.write   = coiscsi_tgtm_stats_clear*/
};
#endif
#endif
...
static int hw_show(struct seq_file *seq, void *v)
{
	...
	for (i=0; i < CSIO_MAX_T4PORTS; i++) {
		seq_printf(seq, "Port:%d status:%s\n", i,
			hw->adap.pi[i].link_cfg.link_ok ? "LINK UP": "LINK DOWN");

		if (!(hw->adap.pi[i].link_cfg.link_ok))
			continue;

		if (hw->adap.pi[i].link_cfg.pcaps & FW_PORT_CAP32_SPEED_100G)
			seq_printf(seq, "Port:%d speed:100G\n", i);
		else if (hw->adap.pi[i].link_cfg.pcaps & FW_PORT_CAP32_SPEED_50G)
			seq_printf(seq, "Port:%d speed:50G\n", i);
		else if (hw->adap.pi[i].link_cfg.pcaps & FW_PORT_CAP32_SPEED_40G)
			seq_printf(seq, "Port:%d speed:40G\n", i);
		else if (hw->adap.pi[i].link_cfg.pcaps & FW_PORT_CAP32_SPEED_25G)
			seq_printf(seq, "Port:%d speed:25G\n", i);
		else if (hw->adap.pi[i].link_cfg.pcaps & FW_PORT_CAP32_SPEED_10G)
			seq_printf(seq, "Port:%d speed:10G\n", i);
		else if (hw->adap.pi[i].link_cfg.pcaps & FW_PORT_CAP32_SPEED_1G)
			seq_printf(seq, "Port:%d speed:1G\n", i);
		else if (hw->adap.pi[i].link_cfg.pcaps & FW_PORT_CAP32_SPEED_100M)
...
static inline char *qtype_to_str(int qtype)
{
	if (qtype == CSIO_INGRESS)
		return "INGRESS";
	else if (qtype == CSIO_EGRESS)
		return "EGRESS";
	else if (qtype == CSIO_FREELIST)
		return "FREELIST";
	else
		return "UNKNOWN";
}

static int sge_q_entries(const struct adapter *adap, int qtype)
{
	struct csio_os_hw *oshw = csio_adap_to_oshw((struct adapter *)adap);
	struct csio_hw *hw = csio_oshw_to_hw(oshw);
	struct csio_wrm *wrm = csio_hw_to_wrm(hw);
	int i, count = 0;

	for (i = 0; i < wrm->free_qidx; i++)
		if (wrm->q_arr[i]->type == qtype)
			count++;
	return count;
}

static void fill_dq(struct csio_wrm *wrm, struct dump_q_arr dq_arr[], int qtype)
{
	int i, j = 0;
	for (i = 0; i < wrm->free_qidx; i++) {
		if (wrm->q_arr[i]->type == qtype) {
			memcpy(&dq_arr[j].q, wrm->q_arr[i],
...
#define SGE_Q_OPS(qstr, qtype) \
void *sge_##qstr##_start(struct seq_file *seq, loff_t *pos) \
{ \
	int entries = DIV_ROUND_UP(sge_q_entries(seq->private, qtype), 4); \
	return *pos < entries ? (void *)((uintptr_t)*pos + 1) : NULL; \
} \
\
void *sge_##qstr##_next(struct seq_file *seq, void *v, loff_t *pos) \
{ \
	int entries = DIV_ROUND_UP(sge_q_entries(seq->private, qtype), 4); \
\
	++*pos; \
...
\
void sge_##qstr##_stop(struct seq_file *seq, void *v) \
{ \
} \
\
static const struct seq_operations sge_##qstr##_seq_ops = { \
	.start = sge_##qstr##_start, \
	.next  = sge_##qstr##_next, \
	.stop  = sge_##qstr##_stop, \
	.show  = sge_##qstr##_show \
}; \
\
static int sge_##qstr##_open(struct inode *inode, struct file *file) \
{ \
	int res = seq_open(file, &sge_##qstr##_seq_ops); \
	if (!res) { \
...
\
static const struct file_operations sge_##qstr##_debugfs_fops = { \
	.owner   = THIS_MODULE, \
	.open    = sge_##qstr##_open, \
	.read    = seq_read, \
	.llseek  = seq_lseek, \
	.release = seq_release, \
}
...
#define Q(s, v) S3("d", s, dq_arr[base_qset + i].q.v)
#define IQ(s, v) S3("d", s, (&dq_arr[base_qset + i].q.un.iq)->v)
#define EQ(s, v) S3("d", s, (&dq_arr[base_qset + i].q.un.eq)->v)
#define FL(s, v) S3("d", s, (&dq_arr[base_qset + i].q.un.fl)->v)
#define ST(s, v) S3("d", s, dq_arr[base_qset + i].q.stats.v)

static int sge_iq_show(struct seq_file *seq, void *v)
{
	struct csio_os_hw *oshw = csio_adap_to_oshw(seq->private);
	struct csio_hw *hw = csio_oshw_to_hw(oshw);
	struct csio_wrm *wrm = csio_hw_to_wrm(hw);
	...
	base_qset = r * 4;
	n = min(4, iq_cnt - 4 * r);
	S3("d", "qidx:", dq_arr[base_qset + i].qidx);
	S3("s", "qtype:", qtype_to_str(dq_arr[base_qset + i].q.type));
	Q("pidx:", pidx);
	Q("cidx:", cidx);
	S3("p", "base addr:", dq_arr[base_qset + i].q.vstart);
	Q("qsize:", size);
	Q("inc idx:", inc_idx);
	Q("wr size:", wr_sz);
	Q("credits:", credits);
	...
	IQ("iqid:", iqid);
	IQ("phy iqid:", physiqid);
	IQ("genbit:", genbit);
	IQ("flq idx:", flq_idx);

	ST("qentries:", n_qentry);
	ST("qempty:", n_qempty);
	ST("qfull:", n_qfull);
	ST("qwrap:", n_qwrap);
	ST("n_tot_reqs:", n_tot_reqs);
	ST("eq_wr_split:", n_eq_wr_split);
	ST("n_tot_rsps:", n_tot_rsps);
	ST("rsp_unknown:", n_rsp_unknown);
...
static int sge_eq_show(struct seq_file *seq, void *v)
{
	struct csio_os_hw *oshw = csio_adap_to_oshw(seq->private);
	struct csio_hw *hw = csio_oshw_to_hw(oshw);
	struct csio_wrm *wrm = csio_hw_to_wrm(hw);
...
static int sge_fl_show(struct seq_file *seq, void *v)
{
	struct csio_os_hw *oshw = csio_adap_to_oshw(seq->private);
...
SGE_Q_OPS(iq, CSIO_INGRESS);
SGE_Q_OPS(eq, CSIO_EGRESS);
SGE_Q_OPS(fl, CSIO_FREELIST);
...
static int flbuf_info_show(struct seq_file *seq, void *v)
{
	struct csio_os_hw *oshw = csio_adap_to_oshw(seq->private);
	struct csio_hw *hw = csio_oshw_to_hw(oshw);
	struct csio_wrm *wrm = csio_hw_to_wrm(hw);
...
DEFINE_SIMPLE_DEBUGFS_FILE(flbuf_info);
...
static int csio_setup_debugfs(struct csio_os_hw *oshw)
{
	...
	static struct t4_linux_debugfs_entry csio_t4_debugfs_files[] = {
		...
		{ "sge_iq_queues", &sge_iq_debugfs_fops, S_IRUSR, 0 },
		{ "sge_eq_queues", &sge_eq_debugfs_fops, S_IRUSR, 0 },
		{ "sge_fl_queues", &sge_fl_debugfs_fops, S_IRUSR, 0 },
		{ "flbuf_info", &flbuf_info_debugfs_fops, S_IRUSR, 0 },
...
#ifdef __CSIO_COISCSI_ENABLED__
		{ "coiscsi_sessions", &coiscsi_sessions_debugfs_fops, S_IRUSR, 0 },
		{ "coiscsi_stats", &coiscsi_tgtm_stats_fops, S_IRUSR, 0 },
#endif
	};
#else
