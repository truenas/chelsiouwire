...
int csio_iscsi_session_chkready(struct csio_rnode *rn)
{
	...
	if (csio_rnism_in_login(rni) ||
		...
		csio_err(csio_lnode_to_hw(csio_rnode_to_lnode(rn)),
			"%s: in %s, rc [%d]\n", __FUNCTION__,
...
int csio_rni_reg_rnode(struct csio_rnode *rn)
{
	struct csio_lnode *ln = csio_rnode_to_lnode(rn);
	struct csio_rnode_iscsi *rni = csio_rnode_to_iscsi(rn);
	...
	struct csio_os_lnode *osln = csio_lnode_to_os(ln);
	struct Scsi_Host *shost = csio_osln_to_shost(osln);
	int err = CSIO_SUCCESS;

	scsi_host_get(shost);
	memset(&osrn->rsess->dev, 0, sizeof(osrn->rsess->dev));
...
void csio_rni_unreg_rnode(struct csio_rnode *rn)
{
	...
	csio_info(hw, "%s: Number of sessions [%d], rnodes [%d].\n",
...
int csio_iscsi_send_logout(struct csio_hw *hw, struct csio_rnode *rn)
{
	...
	csio_info(hw, "%s no recov %d in ready %d in reconnect %d\n",
		__FUNCTION__, lni->no_recov,
		csio_rnism_in_ready(rni), csio_rnism_in_reconnect(rni));

	rni->logout = 1;

	if (csio_rnism_in_ready(rni) && lni->no_recov)
		csio_post_event(&rni->sm, CSIO_RNIE_DRIVER_INI_CLOSE);
	else if (!csio_rnism_in_reconnect(rni) && !csio_rnism_in_recovery(rni))
