...
int csio_issue_foiscsi_chap_wr(struct csio_hw *hw,
		...
		short inode_id, unsigned int flowid, u8 node_type, u8 cvflag)
{
	...
	chap_wr.op_to_kv_flag = csio_cpu_to_be32 (
			V_FW_WR_OP(FW_FOISCSI_CHAP_WR));

	if (node_type == FW_FOISCSI_NODE_TYPE_INITIATOR && cvflag)
		chap_wr.op_to_kv_flag |= csio_cpu_to_be32(F_FW_FOISCSI_CHAP_WR_KV_FLAG);
	...
	if (node_type == FW_FOISCSI_NODE_TYPE_TARGET) {
		...
		len1 = csio_strlen(rni->login_info.tgt_id);
		len2 = csio_strlen(rni->login_info.tgt_sec);

		csio_memcpy(chap_wr.u.chap.chap_id, rni->login_info.tgt_id,
									len1);
		csio_memcpy(chap_wr.u.chap.chap_sec, rni->login_info.tgt_sec,
									len2);
		...
		if (!G_FW_FOISCSI_CHAP_WR_KV_FLAG(csio_be32_to_cpu(chap_wr.op_to_kv_flag))) {
			len1 = csio_strlen(inode->inst.chap_id);
			len2 = csio_strlen(inode->inst.chap_sec);

			csio_memcpy(chap_wr.u.chap.chap_id,
						inode->inst.chap_id, len1);
			csio_memcpy(chap_wr.u.chap.chap_sec,
						inode->inst.chap_sec, len2);
		} else {
			len1 = csio_strlen(inode->inst.vend_key);
			len2 = csio_strlen(inode->inst.vend_val);

			csio_memcpy(chap_wr.u.vend_kv.vend_key,
						inode->inst.vend_key, len1);
			csio_memcpy(chap_wr.u.vend_kv.vend_val,
						inode->inst.vend_val, len2);
		}
	}

	if (!G_FW_FOISCSI_CHAP_WR_KV_FLAG(csio_be32_to_cpu(chap_wr.op_to_kv_flag))) {
		chap_wr.lenu.chapl.id_len = len1;
		chap_wr.lenu.chapl.sec_len = len2;
	} else {
		chap_wr.lenu.vend_kvl.key_len = len1;
		chap_wr.lenu.vend_kvl.val_len = len2;
	}
		...
		short inode_id, unsigned int flowid, u8 subop)
{
	...
	no_recov = (((short)ini_inst->recovery_timeout) == -1);

	csio_info(hw, "ini_inst no recovery: %d\n", no_recov);

	node_wr.no_sess_recv_to_len16 = csio_cpu_to_be32 (
			V_FW_WR_FLOWID(flowid) |
			V_FW_WR_LEN16(CSIO_ROUNDUP(size, 16))|
			V_FW_FOISCSI_NODE_WR_NO_SESS_RECV(no_recov));
	...
	/* Initialize ISID */
	isid_prefix |= V_FW_FOISCSI_NODE_WR_ISID_TVAL(0);
	isid_prefix |= V_FW_FOISCSI_NODE_WR_ISID_AVAL(0);
	isid_prefix |= V_FW_FOISCSI_NODE_WR_ISID_BVAL(0x0743);
	isid_prefix |= V_FW_FOISCSI_NODE_WR_ISID_CVAL(0x00);
	node_wr.isid_tval_to_isid_cval = csio_cpu_to_be32(isid_prefix);	

	csio_dbg(hw, "%s: login_retry [%u], retry_timeout [%u], isid_prefix [0x%x]\n",
...
int csio_issue_foiscsi_ctrl_wr(struct csio_hw *hw,
		...
		unsigned int node_id, unsigned int sess_id, char no_fin)
{
	...
	ctrl_wr.op_to_no_fin = csio_cpu_to_be32(V_FW_WR_OP(FW_FOISCSI_CTRL_WR));
#ifdef __CSIO_PORT_EVTQ__
	iface = csio_chnet_iface_addr_get(hw, rni->login_info.ip_type,
					  &rni->login_info.src_ip);
	ctrl_wr.op_to_no_fin |= csio_cpu_to_be32(V_FW_FOISCSI_CTRL_WR_PORTID(
				iface->tport->portid));
#endif
	...
	if (subop == FW_FOISCSI_WR_SUBOP_MOD &&
		...
		csio_info(hw, "FW_FOISCSI_WR_SUBOP_DEL sent: no fin %d\n", no_fin);
		ctrl_wr.op_to_no_fin = csio_cpu_to_be32(V_FW_WR_OP(FW_FOISCSI_CTRL_WR) |
					V_FW_FOISCSI_CTRL_WR_NO_FIN(no_fin));
#ifdef __CSIO_PORT_EVTQ__
		ctrl_wr.op_to_no_fin |= csio_cpu_to_be32(V_FW_FOISCSI_CTRL_WR_PORTID(
					iface->tport->portid));
#endif
	...
	if (tlogin->ip_type == CSIO_CHNET_L3CFG_TYPE_IPV6)
		tlogin->conn_attr.hdigest_to_tcp_ws_en |=
					F_FW_FOISCSI_CTRL_WR_IPV6;
	ctrl_wr.conn_attr.hdigest_to_tcp_ws_en =
		csio_cpu_to_be32(tlogin->conn_attr.hdigest_to_tcp_ws_en);

	if (!G_FW_FOISCSI_CTRL_WR_TCP_WS_EN(
	    ctrl_wr.conn_attr.hdigest_to_tcp_ws_en)) {

		ctrl_wr.conn_attr.hdigest_to_tcp_ws_en |=
			csio_cpu_to_be32(V_FW_FOISCSI_CTRL_WR_TCP_WS(
			ISCSI_DEFAULT_WSF) | F_FW_FOISCSI_CTRL_WR_TCP_WS_EN);
	}

	if ((subop == FW_FOISCSI_WR_SUBOP_ADD) &&
				G_FW_FOISCSI_CTRL_WR_AUTH_METHOD(tlogin->
				conn_attr.hdigest_to_tcp_ws_en) &&
				G_FW_FOISCSI_CTRL_WR_AUTH_POLICY(tlogin->
				conn_attr.hdigest_to_tcp_ws_en) &&
				(G_FW_FOISCSI_CTRL_WR_SESS_TYPE(tlogin->
				sess_attr.sess_type_to_erl) ==
	...
	if (tlogin->ip_type == CSIO_CHNET_L3CFG_TYPE_IPV4) {
	...
	*iq_idx = csio_cpu_to_be32(csio_q_physiqid(hw, rn->iq_idx));
	csio_info(hw, "%s: iqid:%d eqid:%d subop:%d\n",
		__FUNCTION__, csio_q_physiqid(hw, rn->iq_idx),
		csio_q_physiqid(hw, rn->eq_idx), subop);
