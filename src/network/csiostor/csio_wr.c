...
csio_wr_ring_fldb(struct csio_hw *hw, struct csio_q *flq)
{
	...
	if (flq->inc_idx >= 8) {
		...
		if (csio_unlikely(flq->bar2_addr == NULL)) {
...
csio_wr_fill_fl(struct csio_hw *hw, struct csio_q *flq)
{
	...
	csio_dbg(hw, "flq:%p, credits:%u, sreg:%u\n", flq, n, sreg);
	while (n--) {
		...
		int md_idx, iq_handler_t iq_intx_handler)
{
	...
	if (type == CSIO_INGRESS) {
		...
		if (nflb > 0) {
			...
			csio_dbg(hw, "nflb [%u], IQ idx [%u], FLQ idx "
				"[%u], physiqid [%u], credits [%u].\n",
				nflb, free_idx, flq_idx,
				q->un.iq.physiqid, flq->credits);
		...
		csio_dbg(hw, "%s: flq_idx:%d, iq_idx:0x%x, nflb:%d\n",
			__FUNCTION__, q->un.iq.flq_idx, ret_idx, nflb);
	...
	csio_dbg(hw, "qtype [%u], qsize [%u], qwrsz [%u], "
		"qcredits [%u], qretidx [%u]\n",
		q->type, q->size, q->wr_sz, q->credits, ret_idx);
		  ...
		  uint32_t vec, uint8_t portid, bool async)
{
	...
	if ((flq_idx = csio_q_iq_flq_idx(hw, iq_idx)) != -1) {
		...
		csio_dbg(hw, "iq_idx [%2u], flq_idx [%2u], fl0size [%2u], fl0addr [%p].\n",
				iq_idx, flq_idx,
				iqp.fl0size, (void *)iqp.fl0addr);
	}
	...
	if (ret) {
		csio_err(hw, "IQ cmd returned: mbox %d %d!\n",  adap->mbox, ret);
		return CSIO_INVAL;
	}
		   ...
		   void *priv)
{
	...
	/*csio_dbg(hw, "flxidx [%u], len [%x::%u]\n",
			q->un.iq.flq_idx, len, len);*/
...
restart:
	 ...
	 */
	if (csio_unlikely(q->bar2_addr == NULL))
...
#else
	ingpad += is_t6(hw->adap.params.chip)? X_T6_INGPADBOUNDARY_SHIFT : X_INGPADBOUNDARY_SHIFT;
	sge->csio_fl_align = (1 << ingpad);
#endif
	...
	for (i = 0; i < wrm->num_q; i++) {
		...
		if (wrm->free_qidx && (i < wrm->free_qidx)) {
			if (q->type == CSIO_FREELIST) {
				if (!q->un.fl.bufs)
					continue;
				//This block is commented to prevent
				//possible double free of freelist buffers.
				//Need to revisit this logic later.
				/*
