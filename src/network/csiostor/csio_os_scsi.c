...
csio_iscsi_scsi_err_handler(struct csio_hw *hw, struct csio_ioreq *req)
{
	...
	case FW_SCSI_TASK_ERR:
		...
		if (host_status != DID_REQUEUE)
			csio_err(hw, "%s: wr_status [%u], %s req [%p]. "
				"host_status 0x%x\n",
				__FUNCTION__, req->wr_status,
...
csio_queuecommand_lck(struct scsi_cmnd *cmnd, void (*done)(struct scsi_cmnd *))
{
	...
	if(csio_is_fcoe(hw)) {
		...
		ioreq->iq_idx 		= csio_osrn_to_rn(osrn)->iq_idx;
		ioreq->eq_idx 		= csio_osrn_to_rn(osrn)->eq_idx;
	}
	ioreq->wr_status	= 0;
...
csio_do_abrt_cls(struct csio_hw *hw, struct csio_ioreq *ioreq, bool abort)
{
	...
	struct scsi_cmnd *cmnd = (struct scsi_cmnd *) csio_scsi_osreq(ioreq);
	struct csio_os_rnode *osrn =
			(struct csio_os_rnode *)(cmnd->device->hostdata);
...
csio_eh_abort_handler(struct scsi_cmnd *cmnd)
{
	...
	if (rv != CSIO_SUCCESS) {
		if (rv == CSIO_INVAL) {
			/*
			 * we will fall here, when ioreq is in
			 * csio_scsis_uninit. Either we received
			 * error/completion from fw for this ioreq (or),
			 * fw returned STATE_FAILED for the connection, In
			 * both these cases, we can expect an abort from
			 * scsi mid-layer. Returning failure wont queue again.
			 */
			return FAILED;
		}
	...
	if (((struct scsi_cmnd *)csio_scsi_osreq(ioreq)) == cmnd) {
		csio_err(hw, "Abort timed out -- req: %p sess_id [0x%x]\n", ioreq, rni->sess_id);
...
csio_eh_lun_reset_handler(struct scsi_cmnd *cmnd)
{
	...
	csio_err(hw, "Request to reset LUN:%llu (ssni:0x%x tgtid:%d)\n",
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,19,0)
static int
csio_foiscsi_change_queue_depth(struct scsi_device *sdev, int depth)
{
	if (depth > CSIO_MAX_CMD_PER_LUN)
		depth = CSIO_MAX_CMD_PER_LUN;

	scsi_change_queue_depth(sdev, depth);

	printk(KERN_INFO "qdepth changed to %d\n", depth);
