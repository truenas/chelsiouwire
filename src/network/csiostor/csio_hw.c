...
csio_hw_show(struct csio_hw *hw, void *buffer, uint32_t buffer_len)
{
	...
	csio_memcpy(hw_info->name, hw->name, 32);
	...
	for (ii = 0; ii < CSIO_MAX_T4PORTS; ii++) {
		...
		hw_info->pi[ii].link_ok = hw->adap.pi[ii].link_cfg.link_ok;
		hw_info->pi[ii].pcaps   = hw->adap.pi[ii].link_cfg.pcaps;
	}
	...
	if (csio_drv_ver != NULL && csio_strlen(csio_drv_ver) >0) {
		...
		csio_strncpy(hw_info->drv_version, csio_drv_ver,
			     CSIO_ARRAY_SIZE(hw_info->drv_version));
	}
...
csio_hw_get_mps_tcam(struct csio_hw *hw, void *buffer, uint32_t buffer_len)
{
	...
	u64	val;
	u32 ctl, data2, vnix = 0, vniy = 0;
	u16 ivlan = 0;
	u8 lookup_type = 0, port_num = 0;
	bool dip_hit = false, vlan_vld = false;

	if (buffer_len < (uint32_t)sizeof(csio_mps_tcam_data_t))
		return CSIO_NOMEM;

	if (CHELSIO_CHIP_VERSION((&hw->adap)->params.chip) >= CHELSIO_T6)
	{
		...
		if (mps_tcam->index < 256)
			ctl |= V_CTLTCAMINDEX(mps_tcam->index) | V_CTLTCAMSEL(0);
		else
			ctl |= V_CTLTCAMINDEX(mps_tcam->index - 256) |
				    V_CTLTCAMSEL(1);

		t4_write_reg(&hw->adap, A_MPS_CLS_TCAM_DATA2_CTL, ctl);
		val = t4_read_reg(&hw->adap, A_MPS_CLS_TCAM_RDATA1_REQ_ID1);
		mps_tcam->tcamy = G_DMACH(val) << 32;
		mps_tcam->tcamy |= t4_read_reg(&hw->adap, A_MPS_CLS_TCAM_RDATA0_REQ_ID1);
		data2 = t4_read_reg(&hw->adap, A_MPS_CLS_TCAM_RDATA2_REQ_ID1);
		...
		t4_write_reg(&hw->adap, A_MPS_CLS_TCAM_DATA2_CTL, ctl);
		val = t4_read_reg(&hw->adap, A_MPS_CLS_TCAM_RDATA1_REQ_ID1);
		mps_tcam->tcamx = G_DMACH(val) << 32;
		mps_tcam->tcamx |= t4_read_reg(&hw->adap, A_MPS_CLS_TCAM_RDATA0_REQ_ID1);
		data2 = t4_read_reg(&hw->adap, A_MPS_CLS_TCAM_RDATA2_REQ_ID1);
		if (lookup_type &&
			(lookup_type != M_DATALKPTYPE)) {
...
csio_hw_get_cpl_stats(struct csio_hw *hw, void *buffer, uint32_t buffer_len)
{
	...
	t4_tp_get_cpl_stats(&hw->adap, (struct tp_cpl_stats *)stats, true);
...
csio_hw_get_ddp_stats(struct csio_hw *hw, void *buffer, uint32_t buffer_len)
{
	...
	t4_get_usm_stats(&hw->adap, (struct tp_usm_stats *)stats, true);
...
csio_hw_get_tp_err_stats(struct csio_hw *hw, void *buffer, uint32_t buffer_len)
{
	...
	t4_tp_get_err_stats(&hw->adap, (struct tp_err_stats *)stats, true);
...
csio_do_hello(struct csio_hw *hw, enum dev_state *state)
{
	...
	if((mpfn < 0) && csio_is_kdump_kernel(hw->os_dev))
		mpfn = t4_fw_hello(adap, adap->mbox, adap->mbox, MASTER_MUST, state);
...
csio_get_dcbx_info(struct csio_hw *hw, csio_dcbx_info_t *dcbx_info)
{
	...
	for( i = 0; i < CSIO_DCBX_NUM_PARAMS; i++ ) {
		...
		case 1 :
			...
			break;
		case 2 :
			/* FW_PORT_DCB_TYPE_PRIORATE is not supported by
			 * FW starting from 1.15.9.0 
			 */
...
csio_enable_ports(struct csio_hw *hw)
{
	...
	for (i = 0; i < hw->num_t4ports; i++) {
		...
		csio_spin_unlock_irq(hw, &hw->lock);
		ret = t4_init_portinfo_viid(pi, adap->mbox, portid, adap->mbox, 0, NULL, false);
		if (ret) {
		...
		csio_spin_unlock_irq(hw, &hw->lock);
		ret = t4_link_l1cfg(adap, adap->mbox, portid, &pi->link_cfg);
		if (ret) {
...
#ifdef __CSIO_IPV6_SLAAC__
	val[0] |= FW_PARAMS_PARAM_CHNET_FLAGS_ENABLE_IPV6_SLAAC;
#endif
...
static csio_retval_t
csio_hw_check_fwver(struct csio_hw *hw)
{
	if (csio_is_fcoe(hw) && is_t6(hw->adap.params.chip) &&
		(hw->fwrev < CSIO_MIN_T6_FW)) {
		csio_hw_print_fw_version(hw, "unsupported fw");
		return CSIO_INVAL;
	}
...
csio_hw_configure(struct csio_hw *hw)
{
	...
	rv = t4_prep_adapter(&hw->adap, false);
	if (rv != CSIO_SUCCESS) {
		csio_err(hw, "Failed to prepare the adapter rv:%d\n", rv);
		csio_post_event(&hw->sm, CSIO_HWE_FATAL);
		goto out;
	}
...
csio_hw_initialize(struct csio_hw *hw)
{
	...
	if (csio_is_hw_master(hw) && hw->fw_state != DEV_STATE_INIT) {
		...
		if (is_t6(adap->params.chip)) {
			ret = setup_ppod_edram(adap);
		if (!ret)
			csio_info(hw, "Successfully enabled "
					"ppod edram feature\n");
		}
		/*
		 * And finally tell the firmware to initialize itself using the
	...
	adap->params.nports = CSIO_MAX_T4PORTS;	
	for (i = 0; i < hw->num_t4ports; i++) {
		hw->t4port[i].mod_type = FW_PORT_MOD_TYPE_NA;
		adap->pi[i].adapter = adap;
		adap->pi[i].lport = i;
	}
...
#ifdef __CSIO_FOISCSI_ENABLED__
		csio_spin_unlock_irq(hw, &hw->lock);
		rv = csio_iscsi_transport_init(hw);
		csio_spin_lock_irq(hw, &hw->lock);
		if (rv != CSIO_SUCCESS) {
...
#ifdef __CSIO_FOISCSI_ENABLED__
		if(csio_is_iscsi(hw)) {
			csio_memcpy((void*) &evt_entry->flb, (void*) fl_sg,
						sizeof(struct csio_fl_dma_buf));
		}
#endif
	} else {
		csio_memcpy((void*) evt_entry->data, evt_msg, len);
#ifdef __CSIO_FOISCSI_ENABLED__
		if(csio_is_iscsi(hw)) {
			csio_memset((void*) &evt_entry->flb, 0, sizeof(evt_entry->flb));
		}
#endif
	}
...
csio_free_evt(struct csio_hw *hw, struct csio_evt_msg *evt_entry)
{
	if (evt_entry) {
#ifdef __CSIO_FOISCSI_ENABLED__
		struct csio_dma_buf *fbuf = evt_entry->flb.flbufs;
		uint32_t msg_len = evt_entry->flb.totlen;
		...
		if(csio_is_iscsi(hw)) {
			for (; msg_len; fbuf++) {
				...
				if (!fbuf->vaddr)
					continue;
				csio_dma_pool_free(&fbuf->dmahdl, fbuf->vaddr);
			}
		}
#endif
		csio_spin_lock_irq(hw, &hw->lock);
...
csio_retval_t
csio_portevtq_handler(struct csio_q *iq)
{
	struct csio_hw *hw = (struct csio_hw *)iq->owner;
	enum csio_oss_error rv;

	rv = csio_wr_process_iq(hw, iq, csio_process_fwevtq_entry, NULL);
	return rv;
}
#endif
/*****************************************************************************
...
csio_hw_get_device_id(struct csio_hw *hw)
{
	...
	case CHELSIO_T6:
		...
		break;
	default:
		csio_err(hw, "Device 0x%x is not supported ver 0x%x\n",
			 hw->adap.params.pci.device_id, ver);
		return CSIO_INVAL;
	}
...
csio_hw_init(struct csio_hw *hw, struct csio_hw_os_ops *os_ops)
{
	...
	if(csio_is_fcoe(hw))
		...
		hw->scsi_mode = CSIO_SCSI_MODE_MIXED;
...
#ifdef __CSIO_PORT_EVTQ__
	for (i = 0; i < CSIO_MAX_T4PORTS; i++)
		csio_set_portevt_intr_idx(hw, i, -1);
#endif
	...
	if(csio_is_iscsi(hw)) {
		rv = csio_coiscsi_tgtm_init(csio_hw_to_coiscsi_tgtm(hw), hw);
		if (rv)
			goto coiscsi_tgtm_cleanup;
	}
#endif
	return CSIO_SUCCESS;

#ifdef __CSIO_COISCSI_ENABLED__
coiscsi_tgtm_cleanup:
	if(csio_is_iscsi(hw))
		csio_coiscsi_tgtm_exit(csio_hw_to_coiscsi_tgtm(hw));
#endif
err_evtq_cleanup:
...
#ifdef __CSIO_FOISCSI_ENABLED__
	if (csio_is_iscsi(hw))
		csio_iscsi_transport_uninit(hw);
#endif
