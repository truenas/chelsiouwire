...
CSIO_MODULE_PARAM(dflt_msg_enable, 0, 0, 0x8000000,
		  "Chelsio T4-T6 default message enable bitmap", uint);
...
#ifdef __CSIO_FOISCSI_ENABLED__		
		/* GLUE CHANGE */
		rv = csio_transport_ioctl_handler(csio_oshw_to_hw(oshw),
...
csio_pci_init(struct pci_dev *pdev, int *bars)
{
	...
	*bars = pci_select_bars(pdev, IORESOURCE_MEM);
	...
	/*
	 * Temporary fix: disable all use of the Relaxed Ordering Attribute in
	 * our outbound Transaction Layer Packets.  Some Root Complexes are
	 * experiencing issues with these.  We'll come up with a more nuanced
	 * fix for the next release modelled on the new Linux code for this
	 * issue, which we've been helping to shape.  Note that this also
	 * covers and PCIe SR-IOV Virtual Functions since their Relaxed
	 * Ordering Enable setting tracks that of the corresponding Physical
	 * Function.  That's why we do this for all PFs including those used
	 * solely for managing VFs, not just for our Master PF.
	 */
...
csio_iscsi_get_num_alloc(int idx)
{
	switch (idx) {
	case CSIO_LN_MD:
		return CSIO_FOISCSI_NUM_LNODES;

	case CSIO_RN_MD:
		return CSIO_FOISCSI_NUM_RNODES;
	...
	case CSIO_FLB_PORTEVT_MD:
		return CSIO_MAX_T4PORTS;
#endif
	case CSIO_DDP_MD:
		return csio_ddp_descs;
	...
	case CSIO_FOISCSI_RSESS_MD:
		return CSIO_FOISCSI_NUM_RNODES;

#ifdef __CSIO_COISCSI_ENABLED__         
	case CSIO_COISCSI_LN_MD:
		return CSIO_COISCSI_NUM_LNODES;

	case CSIO_COISCSI_TGT_PORTAL_MD:
		return CSIO_COISCSI_NUM_TGT_PORTALS;

	case CSIO_COISCSI_RN_MD:
		return CSIO_COISCSI_NUM_RNODES;

	case CSIO_COISCSI_SN_MD:
		return CSIO_COISCSI_NUM_SNODES;

	case CSIO_COISCSI_FLB_SCSI_MD:
		return CSIO_MAX_SCSI_QSETS;

	case CSIO_COISCSI_TGTREQ_MD:
		return CSIO_COISCSI_NUM_TGTRQS;

	case CSIO_COISCSI_RCVREQ_MD:
		return CSIO_COISCSI_NUM_RCVRQS;

	case CSIO_COISCSI_DBUF_MD:
		return COISCSI_NUM_DBUFS;

	case CSIO_ISNS_RN_MD:
		return CSIO_ISNS_NUM_RNODES;

	case CSIO_ISNS_SN_MD:
		return CSIO_ISNS_NUM_SNODES;
#endif	
	default:
		return -1;
	}
...
static struct csio_alloc_desc csio_iscsi_alloc_descs[CSIO_MAX_MEM_DESCS] = {
	...
	{ sizeof(struct csio_evt_msg),		CSIO_EVTQ_MD		},
#ifdef __CSIO_FOISCSI_ENABLED__
	{ sizeof(struct foiscsi_cls_session),	CSIO_FOISCSI_RSESS_MD	},
#endif
#ifdef __CSIO_COISCSI_ENABLED__         
	{ sizeof(struct csio_os_lnode),		CSIO_COISCSI_LN_MD	},

	{ sizeof(struct coiscsi_portal),	CSIO_COISCSI_TGT_PORTAL_MD },

	{ sizeof(struct csio_os_rnode),		CSIO_COISCSI_RN_MD	},

	{ sizeof(struct coiscsi_snode),		CSIO_COISCSI_SN_MD	},

	{ CSIO_COISCSI_TGT_FLLEN * sizeof(struct csio_dma_buf),
					CSIO_COISCSI_FLB_SCSI_MD	},

	{ sizeof(struct csio_coiscsi_tgtreq),	CSIO_COISCSI_TGTREQ_MD	},

	{ sizeof(struct csio_coiscsi_rcvreq),	CSIO_COISCSI_RCVREQ_MD } ,

	{ sizeof(struct coiscsi_dbuf),		CSIO_COISCSI_DBUF_MD	},

	{ sizeof(struct csio_os_rnode),		CSIO_ISNS_RN_MD		},

	{ sizeof(struct coiscsi_snode),		CSIO_ISNS_SN_MD		},
#endif
	/* The following must be the last entry */
	{ 		-1,				-1		},
};
...
csio_resource_alloc(struct csio_os_hw *oshw)
{
	...
	csio_dbg(hw, "hw:%p wrm:%p, wrm->num_q:%d\n", hw, wrm, wrm->num_q);
	return 0;
...
csio_create_queues(struct csio_hw *hw)
{
	...
	for (i = 0; i < hw->num_t4ports; i++) {
		/* port event queue */
		rv = csio_wr_iq_create(hw, hw->portevt_iq_idx[i],
			       csio_get_portevt_intr_idx(hw, i),
			       hw->t4port[i].portid, CSIO_FALSE);

		if (rv != CSIO_SUCCESS) {
			csio_err(hw, "port event IQ config failed!: %d\n", rv);
			return rv;
		}
	}
#endif
...
csio_config_queues(struct csio_hw *hw)
{
	...
	oshw->num_sqsets = num_online_cpus() * CSIO_MAX_SQS_CLNT /*hw->num_t4ports*/;
...
#ifdef __CSIO_PORT_EVTQ__
	for (i = 0; i < hw->num_t4ports; i++) {
		/* Allocate the port evt queue */
		hw->portevt_iq_idx[i] = csio_wr_alloc_q(hw, CSIO_FWEVT_IQSIZE,
					   CSIO_FWEVT_WRSIZE,
					   CSIO_INGRESS, (void *)hw,
					   CSIO_FWEVT_FLBUFS, 0,
					   CSIO_FLB_PORTEVT_MD,
					   NULL);
		if (hw->portevt_iq_idx[i] == -1) {
			csio_err(hw, "port evt queue creation failed\n");
			goto intr_disable;
		}
	}
#endif
	...
	for (i = 0; i < CSIO_MAX_SQS_CLNT; i++) {
		...
		for (j = 0; j < oshw->num_scsi_msix_cpus; j++) {
			...
			if (j >= info->max_cpus) {
				...
				csio_dbg(hw,
	    			    "sqset[%d][%d]->int:%d, iq:%d, physiqid:%d, eq:%d\n",
				    i, j, sqset->intr_idx,
				    sqset->iq_idx, csio_q_physiqid(hw, sqset->iq_idx),
					    sqset->eq_idx);
			...
			csio_dbg(hw, "%s: sqset[%d][%d]->int:%d iq:%d eq:%d\n",
					__FUNCTION__, i, j, sqset->intr_idx,
...
static struct csio_hw_os_ops iscsi_os_ops = {
	...
	.os_alloc_rnode 	= csio_alloc_rnode,
#ifdef __CSIO_COISCSI_ENABLED__	
	.os_alloc_coiscsi_rnode = csio_coiscsi_alloc_rnode,
	.os_free_coiscsi_rnode  = csio_coiscsi_free_rnode,
	.os_alloc_isns_rnode 	= csio_alloc_isns_rnode,
	.os_free_isns_rnode  	= csio_free_isns_rnode,
#endif
	.os_free_rnode 		= csio_free_rnode,
...
csio_oshw_init(struct pci_dev *pdev)
{
	...
	if (CHECK_PF_MASK(PF_ISCSI, pdev->device))
		prv_buf_size = sizeof(struct csio_control_dev);

	oshw = kzalloc(sizeof(struct csio_os_hw) + prv_buf_size, GFP_KERNEL);
	if (!oshw)
		goto err;
	oshw->pdev = pdev;
...
csio_probe_one(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	struct csio_os_hw *oshw;
	struct csio_hw *hw;
	bool is_t4 = false, is_t5 = false, is_t6 = false;

	if ((pdev->device == CSIO_HW_T6FPGA_FOISCSI) ||
	    ((pdev->device & CSIO_HW_CHIP_MASK) == CSIO_HW_T6))
		is_t6 = true;
	else if (((pdev->device & CSIO_HW_CHIP_MASK) == CSIO_HW_T5))
		is_t5 = true;
	else if (((pdev->device & CSIO_HW_CHIP_MASK) == CSIO_HW_T4))
		is_t4 = true;
	else {
		dev_err(&pdev->dev, "NOT a Chelsio Device ID 0x%x\n", 
			 pdev->device);
		return -ENODEV;
	}
	...
	if (csio_chip < 3) {
		...
		/* probe only T4 cards */
		if ((csio_chip == 0 && !is_t4) ||
		    (csio_chip == 1 && !is_t5) ||
		    (csio_chip == 2 && !is_t6))
			return -ENODEV;
	}

	if (CHECK_PF_MASK(PF_FCOE, pdev->device) && is_t4) {
		dev_err(&pdev->dev, "FCoE not supported in T4\n");
		return -ENODEV;
	}
