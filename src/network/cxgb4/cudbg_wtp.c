...
static u32 read_sge_debug_data(struct cudbg_init *pdbg_init, u32 *sge_dbg_reg)
{
	...
	u32 valuelo = 0, valuehi = 0;
	int i = 0;

	for (i = 0; i <= 15; i++) {
		if (pdbg_init->recon_en) {
			cudbg_recon_read_sge_indirect(pdbg_init,
						      A_SGE_DEBUG_DATA_LOW,
						      &valuelo, 1, i);
			cudbg_recon_read_sge_indirect(pdbg_init,
						      A_SGE_DEBUG_DATA_HIGH,
						      &valuehi, 1, i);
		} else {
			t4_write_reg(padap, A_SGE_DEBUG_INDEX, (u32)i);
			valuelo = t4_read_reg(padap, A_SGE_DEBUG_DATA_LOW);
			valuehi = t4_read_reg(padap, A_SGE_DEBUG_DATA_HIGH);
		}
		/*printf("LOW	 0x%08x\n", valuelo);*/
		sge_dbg_reg[(i << 1) | 1] = HTONL_NIBBLE(valuelo);
		/*printf("HIGH	 0x%08x\n", valuehi);*/
		sge_dbg_reg[(i << 1)] = HTONL_NIBBLE(valuehi);
	}
	return 0;
}
			    ...
			    struct tp_mib_data **ppTp_Mib)
{
	...
	for (i = 0; i < TP_MIB_SIZE; i++) {
		cudbg_tp_mib_read(pdbg_init, &tp_mib[i].value, 1,
		       ...
		       struct cudbg_error *cudbg_err)
{
	...
	rc = cudbg_recon_dump_status(pdbg_init, CUDBG_SGE_INDIRECT);
	if (rc)
		return rc;
	...
	for (i = 0; i < 4; i++) {
		cudbg_tp_pio_read(pdbg_init, &value, 1,
				  (u32)(A_TP_DBG_CSIDE_RX0 + i), true);
	...
	for (i = 0; i < 4; i++) {
		cudbg_tp_pio_read(pdbg_init, &value, 1,
				  (u32)(A_TP_DBG_ESIDE_PKT0 + i), true);
