				    ...
				    const struct in6_addr *sip6)
{
	...
	if (chip_ver <= CHELSIO_T5) {
		if (use_ipv6)
			fidx = cxgb4_alloc_ftid(&adap->tids, PF_INET6, chip_ver);
		else
			fidx = cxgb4_alloc_ftid(&adap->tids, PF_INET, chip_ver);
		if (fidx < 0) {
		...
		if (use_ipv6)
			fidx = cxgb4_alloc_hpftid(&adap->tids, PF_INET6, chip_ver);
		else
			fidx = cxgb4_alloc_hpftid(&adap->tids, PF_INET, chip_ver);
		if (fidx < 0) {
			printk("%s: Could not get valid fidx..\n", __func__);
			return fidx;
		}
...
free_ftid:
	if (is_t5(adap->params.chip)) {
		if (use_ipv6)
			cxgb4_clear_ftid(&adap->tids, fidx, PF_INET6, chip_ver);
		else
			cxgb4_clear_ftid(&adap->tids, fidx, PF_INET, chip_ver);
	} else {
		if (use_ipv6)
			cxgb4_clear_hpftid(&adap->tids, fidx, PF_INET6, chip_ver);
		else
			cxgb4_clear_hpftid(&adap->tids, fidx, PF_INET, chip_ver);
	}
	return ret;
}
