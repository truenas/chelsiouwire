...
#define WDTOE_FLSIZE (16 * 16)
#define WDTOE_MMAPNUM_RXQ (WDTOE_FLSIZE + 5)
...
/* Must be >= WDTOE_FLSIZE */
#define NRXBUF WDTOE_FLSIZE
struct sw_t4_raw_fl {
	u64 sw_queue[NRXBUF];
	u16 size;
	u16 cidx;
...
struct wdtoe_device {
	...
	struct wdtoe_tid_list tid_list;
	int devno;
	int index;
	...
	u32 qpmask;
	struct work_struct free_wd_res;
	bool killed;
};
...
ssize_t wdtoe_pass_pid(struct cxgb4_lld_info *, 
			const char __user *buf, 
			int, int);
ssize_t wdtoe_pass_tid(struct cxgb4_lld_info *,
		       struct wdtoe_device *wd_dev,
		       const char __user *buf,
		       int, int);
...
void wdtoe_dec_conn_num(struct sock *);
