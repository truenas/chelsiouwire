				...
				struct cxgb4_lld_info *lldi)
{
	...
	t4_eq_status_entries = lldi->sge_egrstatuspagesize / 64;
	...
	fl->fl_shared_params =
			(struct t4_fl_shared_params_entry *)fl_params->desc;
	...
	c.iqns_to_fl0congen = htonl(
		...
		V_FW_IQ_CMD_FL0DATARO(USE_FL0DATARO(relaxed_ordering)) |
#ifdef notyet                
...
static int insert_node(struct pid_node **list, pid_t new_pid)
{
				...
				int pid)
{
	...
	if (idx >= WDTOE_DEV_TABLE_ENTRY || idx < 0 || wd_dev != t[idx].wd_dev) {
		printk(KERN_ERR "[wdtoe] %s: invalid index [%d] "
				"for pid [%d]\n", __func__, idx, pid);
		return -1;
	}

	err = insert_node(&t[idx].pid_list, pid);
	if (err < 0)
		return -1;
	/* flag the device entry to be "CREATED" now */
	t[idx].state = WD_DEV_CREATED;
	return 0;
}
			...
			if (t[i].wd_dev != NULL) {
				...
				for (j = 0; j < NWDTOECONN; j++) {
					if (tid == stack->conn_info[j].tid) {
						...
						*dev_idx = i;
						*tbl_idx = j;
						spin_unlock_bh(&t[i].wd_dev->lock);
						return 0;
					}
				}
				spin_unlock_bh(&t[i].wd_dev->lock);
...
void wdtoe_dec_conn_num(struct sock *sk)
{
	int dev_idx, tbl_idx, ret;
	struct wdtoe_device *dev;

	ret = wdtoe_find_dev_by_tid(wdtoe_dev_table, &dev_idx, &tbl_idx,
				    CPL_IO_STATE(sk)->tid);
	if (ret)
		return;

	dev = wdtoe_dev_table[dev_idx].wd_dev;
	if (dev->killed) {
		spin_lock(&dev->lock);
		dev->conn_num--;
...
ssize_t wdtoe_pass_tid(struct cxgb4_lld_info *cached_lldi,
		       struct wdtoe_device *wd_dev,
		       const char __user *buf,
		       int in_len, int out_len)
{
	struct wdtoe_pass_tid_cmd cmd;
	struct wdtoe_tid_list *new, *temp;

	if (copy_from_user(&cmd, buf, sizeof cmd))
		return -EFAULT;

	if (cmd.action) {
		new = kmalloc(sizeof(struct wdtoe_tid_list), GFP_KERNEL);
		if (!new)
			return -ENOMEM;
		new->tid = cmd.tid;
		list_add_tail(&new->list, &wd_dev->tid_list.list);
	} else {
		list_for_each_entry_safe(new, temp, &wd_dev->tid_list.list, list) {
			if (cmd.tid == new->tid) {
				list_del(&new->list);
				kfree(new);
			}
				...
				int in_len, int out_len)
{
	...
	index = wdtoe_get_next_idx(wdtoe_dev_table, wdtoe_dev);
...
int wdtoe_open(struct inode *inode, struct file *filp)
{
	...
	unsigned idx = MINOR(devno) - MINOR(wdtoe_dev);

	dev = wdtoe_dev_table[idx].wd_dev;
	if (dev == NULL) {
		pr_err("[wdtoe] %s: could not get "
				"'struct wdtoe_device *'\n",
				__func__);
		return -ENODEV;
	}
	...
	INIT_LIST_HEAD(&dev->tid_list.list);
...
static inline void t4_raw_fl_consume(struct t4_fl *f)
{
...
static inline void t4_raw_fl_produce(struct t4_fl *f)
{
	f->fl_shared_params->in_use++;
...
static inline void t4_ring_fl_db(struct t4_fl *f)
{
	if (f->fl_shared_params->pend_cred >= 8) {
		wmb();
		writel(F_DBPRIO | V_QID(f->cntxt_id) |
		       V_PIDX(f->fl_shared_params->pend_cred / 8), f->db);
		f->fl_shared_params->pend_cred &= 7;
	}
}
/* func is copied from libwdtoe */
static inline int wdtoe_refill_fl(struct t4_fl *f)
{
	int i;
	int len_refill_fl;

	len_refill_fl = (f->size - 8) - f->fl_shared_params->in_use;
	if (len_refill_fl >= (f->size >> 4)) {
		for (i = 0; i < len_refill_fl; i++)
			t4_raw_fl_produce(f);
		f->fl_shared_params->pend_cred += len_refill_fl;
		t4_ring_fl_db(f);
	}
	return 0;
}
...
static int wdtoe_process_responses(struct wd_raw_rxq *rxq_list)
{
	struct t4_iqe *iqe;
	struct t4_iq *iq;
	struct t4_fl *fl;
	...
	} else if (IQE_IQTYPE(iqe) == X_RSPD_TYPE_FLBUF) {
		u32 nfrags, i;

		nfrags = DIV_ROUND_UP(IQE_DATADMALEN(iqe), PAGE_SIZE);
		for (i = 0; i < nfrags; i++)
...
static void wdtoe_drain_iq(struct wd_raw_rxq *rxq_list)
{
	int ret = 0;
	/* process all the CPLs in the iq */
	while (!ret)
		ret = wdtoe_process_responses(rxq_list);
}

static void wdtoe_free_res_work(struct work_struct *work)
{
	struct wdtoe_device *wd_dev = container_of(work, struct wdtoe_device,
						   free_wd_res);
	int i, idx = wd_dev->index;
	...
	/* FIXME For now just sleep for 100ms, and then drain the IQs. */

	while(wd_dev->conn_num > 0) {
		for (i = 0; i < cached_lldi->nports; i++)
			wdtoe_drain_iq(wd_dev->rxq_list[i]);
		msleep(100);
	}
	...
	BUG_ON(wd_dev != wdtoe_dev_table[idx].wd_dev);
...
int wdtoe_close(struct inode *inode, struct file *filp)
{
	...
	if (!wd_dev->in_use) {
		WARN_ONCE(1, "close on freed wd_dev %p idx %u\n", wd_dev, idx);
		goto out;
	}
	wd_dev->killed = true;
	INIT_WORK(&wd_dev->free_wd_res, wdtoe_free_res_work);
	schedule_work(&wd_dev->free_wd_res);
out:
			     ...
			     struct wdtoe_device *wd_dev)
{
	...
	return t4_calc_opt2(cplios, s, iq_id, cong_algo);
}
