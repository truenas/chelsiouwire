...
static int syn_netfilter;
module_param(syn_netfilter, int, 0644);
MODULE_PARM_DESC(syn_netfilter, "Enable netfilter SYN processing");
...
struct t4_tcp_congestion_ops {
	struct tcp_congestion_ops	ops;
	int				key;
};

#define T4_CONG_OPS(__s, __k) \
	{ { .name = __s, .owner = THIS_MODULE }, .key = CONG_ALG_##__k, }

#define CONG_ALG_NONE (-1)

static struct t4_tcp_congestion_ops t4_cong_ops[] = {
	T4_CONG_OPS("reno",      RENO),
	T4_CONG_OPS("tahoe",     TAHOE),
	T4_CONG_OPS("newreno",   NEWRENO),
	T4_CONG_OPS("highspeed", HIGHSPEED),
	T4_CONG_OPS("none",      NONE),
};
...
static struct bpf_insn drop_insnsi[] = {
	BPF_ALU32_IMM(BPF_MOV, BPF_REG_A, 0x0),
	BPF_EXIT_INSN(),
};
			  ...
			  struct sk_buff *skb, int through_l2t)
{
	...
	if (unlikely(sk->sk_state == TCP_SYN_SENT)) {
		/* defer */
		__skb_queue_tail(&cplios->ooo_queue, skb);
	} else if (through_l2t) {
		 /* send through L2T */
		cxgb4_l2t_send(cplios->egress_dev, skb, cplios->l2t_entry);
	} else {
		/* send directly */
 ...
 */
inline int sk_insert_tid(struct tom_data *d, struct sock *sk,
			  unsigned int tid)
{
	int id = tid;
	int err;
	...
	err = 0;
	id = bh_insert_handle(d, sk, tid);
#else
	err = xa_insert_bh(&d->hwtid_idr, tid, sk, GFP_NOWAIT);
	if (err < 0) {
#ifdef T4_TRACE
		T4_TRACE1(TIDTB(sk),
			  "Failed to allocate tid XA entry = %d\n", tid);
#endif
	}
#endif
	return id;
...
static unsigned int select_mss(const struct cpl_io_state *cplios,
 ...
 */
		...
		cplios_reset_flag(CPL_IO_STATE(sk), CPLIOS_UPDATE_RCV_WND);
        }

}

/* Assumes DACK_CONFIG[ByteThreshold]=26856 and DACK_CONFIG[MSSThreshold]=4 */
unsigned int t4_select_delack(struct sock *sk)
{
	...
	const unsigned int MSSThreshold = 4;
	const unsigned int ByteThreshold = 26856;

	dack_mode = TOM_TUNABLE(dev, delack);
	if (!dack_mode || !inet_csk(sk)->icsk_ack.pingpong)
		return 0;

	if ((dack_mode == 2) && (MSS_CLAMP(tp) > ByteThreshold/(MSSThreshold*2)))
		dack_mode = 3;

	if ((dack_mode == 3) && (tp->rcv_wnd < (ByteThreshold + MSS_CLAMP(tp))))
		dack_mode = 1;

	if ((dack_mode == 2) && (tp->rcv_wnd < ((MSSThreshold*2+1)*MSS_CLAMP(tp))))
		dack_mode = 1;
		
	if ((dev->ttid >= TOE_ID_CHELSIO_T4) && (cplios->delack_mode == 0) &&
		(tp->rcv_wnd > 3 * MSS_CLAMP(tp)))
		dack_mode = 1;
 ...
 */
inline unsigned long long calc_opt0(struct sock *sk, int nagle)
{
	struct cpl_io_state *cplios = CPL_IO_STATE(sk);
	const struct tcp_sock *tp = tcp_sk(sk);

	/* If nagle policy is not modified by user, determine nagle on/off
	 * based on sysctl and application socket settings/kernel settings
	 */
	if (likely(nagle == -1))
		nagle = (TOM_TUNABLE(cplios->toedev, nagle) &&
			 ((tp->nonagle & TCP_NAGLE_OFF) == 0));
	/* else, determine nagle on/off based on nagle policy or sysctl */
	else
		nagle = (nagle || TOM_TUNABLE(cplios->toedev, nagle));

	cplios->nagle = nagle;

	return V_NAGLE(nagle) | F_TCAM_BYPASS |
...
int get_ca_ops(const struct sock *sk)
{
	const struct tcp_congestion_ops *ca = inet_csk(sk)->icsk_ca_ops;
	int cong_algo;

	for (cong_algo = 0; cong_algo < ARRAY_SIZE(t4_cong_ops); cong_algo++)
		if (!strcmp(t4_cong_ops[cong_algo].ops.name, ca->name))
			break;

	return (cong_algo < ARRAY_SIZE(t4_cong_ops)) ? cong_algo : -1;
}

unsigned int t4_calc_opt2(const struct cpl_io_state *cplios,
			  const struct offload_settings *s,
			  unsigned int iq_id, int cong_algo)
{
	const struct sock *sk = cplios->sk;
	...
	if (tcp_win_scaling_enabled(sock_net(sk)))
		opt2 |= F_WND_SCALE_EN;
	...
	else if (tcp_timestamps_enabled(sock_net(sk)))
		opt2 |= F_TSTAMPS_EN;

	if (s && s->sack >= 0)
		opt2 |= V_SACK_EN(s->sack);
	else if (tcp_sack_enabled(sock_net(sk)))
		opt2 |= F_SACK_EN;
	...
	opt2 |= V_CONG_CNTRL(cong_algo < 0 ? 2 : cong_algo);
 ...
 *      payload limit.
 */
static inline int is_ofld_imm(struct cpl_io_state *cplios,
			      const struct sk_buff *skb)
{
	...
	if (likely(ULP_SKB_CB(skb)->flags & ULPCB_FLAG_NEED_HDR)) {
		...
		if ((cplios->ulp_mode == ULP_MODE_TLS) &&
		    is_tls_offload_skb(cplios->sk, skb))
			length += tls_wr_size(cplios->sk, skb, true);
		else
...
/**
 *	flowc_wr_credits - return number of credits needed for a FW_FLOWC_WR
 *	@nparams: number of parameters
 *	@flowclenp: return value pointer for FW_FLOWC_WR length 
 *
 *	Return number of 16-byte "credits" needed to send a Firmware FlowC
 *	Work Request containing @nparams parameters.  If a non-NULL @flowclenp
 *	is provided, the length of the FlowC Work Request will be returned via
 *	that pointer.
 */
...
/**
 *	create_flowc_wr_skb - allocate and initialize a FlowC Work Request skb
 *	@sk: the associated socket
 *	@flowc: the FW_FLOWC_WR
 *	@flowclen: the FLOWC_WR length (rounded up to 16-bytes)
 *
 *	Allocates an skb using the connections Control skb Cache and returns
 *	it filled in with the FlowC Work Request.  NULL is returned if an
 *	skb can't be allocated.
 */
struct sk_buff *create_flowc_wr_skb(struct sock *sk, struct fw_flowc_wr *flowc,
				   int flowclen)
{
	struct cpl_io_state *cplios = CPL_IO_STATE(sk);
	struct sk_buff *skb;
...
/**
 *	send_flowc_wr - send a FW_FLOWC_WR; return credits consumed
 *	@sk: the associated socket
 *	@flowc: the FW_FLOWC_WR
 *	@flowclen: the FLOWC_WR length (rounded up to 16-bytes)
 *
 *	Send the Firmware FlowC Work Request and return the number of
 *	16-byte credits consumed, a negative error number, or 0 if the
 *	socket is in shutdown state.
 */
			...
			int flowclen)
{
	...
	if (toedev_in_shutdown(cplios->toedev))
		return 0;

	/*
	 * If the first data has already been sent on the connection, then
	 * we'll need to queue up an skb to carry the FlowC Work Request.
	 * As such, it won't use any credits [yet] on the connection.
	 */
	...
	if (!syn_sent) {
		...
		ret = cxgb4_immdata_send(cplios->egress_dev,
					 cplios->txq_idx,
					 flowc, flowclen);
		if (!ret)
			return flowclen16;
...
/**
 *	send_tx_flowc_wr - send first FW_FLOWC_WR
 *	@sk: the associated socket
 *	@compl: 0 => no completion; 1 => completion after processing
 *	@snd_nxt: initial TCP Send Next Sequence Number
 *	@rcv_nxt: initial TCP Receive Next Sequence Number
 *
 *	Send the first Firmware FlowC Work Request just before the first
 *	data is sent on an Offload Connection.  Returns the number of
 *	16-byte credits used by the FlowC Work Request, a negative error
 *	number, or 0 if the socket is in shutdown.
 */
int send_tx_flowc_wr(struct sock *sk, int compl, u32 snd_nxt, u32 rcv_nxt)
{
	struct cpl_io_state *cplios = CPL_IO_STATE(sk);
	...
	if (ret == 0) {
		...
		iqid = wd_dev->rxq_list[cplios->port_id]->iq.cntxt_id;
	} else {
		/*
	...
	/*
	 * Initialize the FlowC Work Request.  We do the parameters first
	 * because there are a variable number.
	 */
...
#define FLOWC_PARAM(__m, __v) \
	do { \
		...
		flowc->mnemval[paramidx].val = htonl(__v); \
		paramidx++; \
	} while (0)

	paramidx = 0;

	/* FW_FLOWC_MNEM_CH as first parameter for fixup path */
	FLOWC_PARAM(CH, cplios->tx_c_chan);
	FLOWC_PARAM(PFNVFN, d->pfvf);
	FLOWC_PARAM(IQID, iqid);
	FLOWC_PARAM(SNDNXT, snd_nxt);
	FLOWC_PARAM(RCVNXT, rcv_nxt);
	FLOWC_PARAM(SNDBUF, cplios->sndbuf);
	...
	if (!cxgb4_dcb_enabled(cplios->egress_dev))
		...
		if (vlan == CPL_L2T_VLAN_NONE) {
			...
			dcbprio = (vlan & VLAN_PRIO_MASK) >> VLAN_PRIO_SHIFT;
	}
	FLOWC_PARAM(DCBPRIO, dcbprio);
#endif
	...
	if (cplios->sched_cls != SCHED_CLS_NONE)
		FLOWC_PARAM(SCHEDCLASS, cplios->sched_cls);

	/* The maximum amount of data the firmware will push
	 * to the ULP_TX in a single request. Usually, this is
	 * set to 0 and firmware chooses an adequate default.
	 */
	if (cplios->txplen_max)
		FLOWC_PARAM(TXDATAPLEN_MAX, cplios->txplen_max);

	if (SND_WSCALE(tp))
		FLOWC_PARAM(RCV_SCALE, SND_WSCALE(tp));

	if (cplios->ulp_mode == ULP_MODE_TLS)
		FLOWC_PARAM(ULP_MODE, cplios->ulp_mode);

	if (is_tls_offload(sk) && tls_ofld->fcplenmax) {
		FLOWC_PARAM(TXDATAPLEN_MAX,
			 tls_ofld->fcplenmax);
	}
...
/**
 *	send_tx_schedclass_wr - send a FlowC TX Schedule Class Work Request
 *	@sk: the associated socket
 *	@compl: 0 => no completion; 1 => completion after processing
 *
 *	Send a FlowC FW_FLOWC_MNEM_SCHEDCLASS Work Request for an Offloaded
 *	Connection.  This is used to change the TX Scheduling Class bound to
 *	the connection.  Returns the number of 16-byte credits consumed by the
 *	FlowC Work Request, a negative error number, or 0 if the socket is in
 *	shutdown.
 */
int send_tx_schedclass_wr(struct sock *sk, int compl)
{
	const struct cpl_io_state *cplios = CPL_IO_STATE(sk);
	unsigned int fw_schedclass;

	/*
	 * Translate TX Scheduling Class into Firmware value.
	 */
	if (cplios->sched_cls == SCHED_CLS_NONE)
		fw_schedclass = 0xff;
	else
		fw_schedclass = cplios->sched_cls;

	return send_tx_param_update_wr(sk, FW_FLOWC_MNEM_SCHEDCLASS,
				       fw_schedclass, compl);
}

/**
 *      send_tx_param_update_wr - send a FlowC param Work Request
 *      @sk: the associated socket
 *      @param: for example Maximum Segment Size for the connection
 *      @compl: 0 => no completion; 1 => completion after processing
 *
 *      Send a FlowC FW_FLOWC_MNEM_XXX Work Request for an Offloaded
 *      Connection.  For example, to change the TX MSS for the connection.
 *      Returns the number of 16-byte credits consumed by the Work Request,
 *      a negative error number, or 0 if the socket is in shutdown.
 */
				   ...
				   u32 credits, u32 compl)
{
	...
	if (ULP_SKB_CB(skb)->flags & ULPCB_FLAG_ISCSI_FORCE)
			wr_ulp_mode_force |=
			    (CHELSIO_CHIP_VERSION(adapter_type) <= CHELSIO_T5
			     ? F_TX_FORCE
			     : F_T6_TX_FORCE);

	/* always set shove bit */
	req->lsodisable_to_flags = htonl(wr_ulp_mode_force |
			V_TX_URG(skb_urgent(skb)) | V_TX_SHOVE(1U));
	req->plen = htonl(len);
}
...
int t4_push_frames(struct sock *sk, int req_completion)
{
	...
	while (cplios->wr_credits && (skb = skb_peek(&cplios->tx_queue)) &&
	       !cplios_flag_nochk(cplios, CPLIOS_TX_WAIT_IDLE) &&
	       (!(ULP_SKB_CB(skb)->flags & ULPCB_FLAG_HOLD) ||
		skb_queue_len(&cplios->tx_queue) > 1)) {

		unsigned int immdlen;
		int len;	/* length with ulp bytes inserted by h/w */
		int tls_len;    /* TLS data len before IV/key */
		unsigned int credits_needed, credit_len;
		unsigned int completion=0;
		int flowclen16=0;
		int tls_imm = 0;

		immdlen = len = credit_len = tls_len = skb->len;
		if (!is_ofld_imm(cplios, skb)) {
			immdlen = skb_transport_offset(skb);
			if (is_tls_offload_skb(sk, skb))
				wr_size = tls_wr_size(sk, skb, false);
			credit_len = 8*calc_tx_flits_ofld(skb, immdlen);
		} else {
			if (is_tls_offload_skb(sk, skb)) {
				wr_size = tls_wr_size(sk, skb, false);
				tls_imm = 1;
		...
		if (!cplios_flag_nochk(cplios, CPLIOS_TX_DATA_SENT)) {
		...
		if (cplios->wr_credits < credits_needed) {
			/* Revert IV DSGL accounted for credits count */
			if (is_tls_offload_skb(sk, skb) &&
		...
		if (is_tls_offload(sk))
			TLS_IO_STATE(sk)->tx_qid = (skb->queue_mapping >> 1);
		/* remember credits for TX data and non-data WR until WR_ACK */
		skb->csum = credits_needed + cplios->wr_nondata;
		...
		if (likely(ULP_SKB_CB(skb)->flags & ULPCB_FLAG_NEED_HDR)) {
			...
			if (is_tls_offload_skb(sk, skb)) {
				make_tlstx_data_wr(sk, skb, tls_imm,
			...
			if (cmd == CPL_CLOSE_CON_REQ)
				cplios_set_flag(cplios,
						CPLIOS_CLOSE_CON_REQUESTED);
...
void free_atid(struct cpl_io_state *cplios, struct tom_data *td,
	       unsigned int atid)
{
	struct tid_info *tids = td->tids;

	conn_remove_handle(td, atid);
...
void t4_release_offload_resources(struct sock *sk)
{
	...
	if (sk->sk_family != AF_INET)
		cxgb4_clip_release(cplios->egress_dev,
			(const u32 *)((&inet6_sk(sk)->saddr)->s6_addr), 1);

	if (sk->sk_state == TCP_SYN_SENT) {               // we have ATID
		free_atid(cplios, td, tid);
		__skb_queue_purge(&cplios->ooo_queue);
	} else {                                          // we have TID
		bh_remove_handle(td, cplios->idr);
...
static void dummy_tcp_keepalive_timer(struct timer_list *t)
{
}
#endif
 ...
 */
static void install_offload_ops(struct cpl_io_state *cplios,
...
#if defined(CONFIG_TCPV6_OFFLOAD)
	if (sk->sk_family == AF_INET) {
		sk_copy_ofldproto(oproto, &t4_tcp_prot);
		ps = &t4_tcp_prot.proto;
	} else {
		sk_copy_ofldproto(oproto, &t4_tcp_v6_prot);
		ps = &t4_tcp_v6_prot.proto;
	}
#else
	sk_copy_ofldproto(oproto, &t4_tcp_prot);
	ps = &t4_tcp_prot.proto;
#endif
	sk_ofld_proto_set_tomhandlers(oproto, ps);
	sk_ofld_proto_set_ptr(oproto, cplios);
	smp_store_release(&sk->sk_prot, &oproto->proto);

	sk->sk_backlog_rcv = t4_backlog_rcv;
	if (sk->sk_write_space == sk_stream_write_space_compat)
			    ...
			    const struct offload_settings *s)
{
	...
	req->opt0 = cpu_to_be64(calc_opt0(sk, s->nagle) |
				V_L2T_IDX(e->idx) |
				V_SMAC_SEL(cplios->smac_idx) |
				V_ULP_MODE(cplios->ulp_mode) |
			    ...
			    const struct l2t_entry *e)
{
	...
	struct offload_req oreq;

	offload_req_from_sk(&oreq, sk, OPEN_TYPE_ACTIVE);

	rcu_read_lock();
	s = *lookup_ofld_policy(cplios->toedev, &oreq,
				d->conf.cop_managed_offloading);
	rcu_read_unlock();
	...
	req->tcb.opt0 = cpu_to_be64(calc_opt0(sk, s.nagle) |
				V_L2T_IDX(e->idx) |
				V_SMAC_SEL(cplios->smac_idx) |
				V_TX_CHAN(cplios->tx_c_chan));
...
#if defined(CONFIG_IPV6) || defined(CONFIG_IPV6_MODULE)
		t5req->rsvd = cpu_to_be32(secure_tcpv6_seq(
...
static void act_open_retry_timer(struct timer_list *t)
{
	...
	if (sock_owned_by_user(sk))         /* try in a bit */
		...
		if (!skb)
			...
			offload_req_from_sk(&oreq, sk, OPEN_TYPE_ACTIVE);
			rcu_read_lock();
			settings = *lookup_ofld_policy(tdev,
						       &oreq,
						       d->conf.cop_managed_offloading);
			rcu_read_unlock();
			...
			cxgb4_l2t_send(cplios->egress_dev, skb,
				       cplios->l2t_entry);
		}
	}
out_err:
...
static void fixup_and_send_ofo(struct cpl_io_state *cplios, unsigned int tid);
...
static void active_open_rpl(struct sock *sk, struct sk_buff *skb)
{
	...
	if (is_neg_adv(status)) {
		...
		if (cplios_flag_nochk(cplios, CPLIOS_ABORT_RPL_PENDING)) {
			if (!lookup_tid(td->tids, tid))
				cplios->idr = sk_insert_tid(td, sk, tid);
		}
		cplios->neg_adv_tid = tid;
		fixup_and_send_ofo(cplios, tid);
		kfree_skb(skb);
		return;
	}
...
#endif
			skb->sk = sk;
			t4_defer_reply(skb, tdev, deferred_tnl_connect);
			return;
			}
 ...
 */
static unsigned int select_rcv_wnd(struct cpl_io_state *cplios)
{
	struct sock *sk = cplios->sk;
	...
	cplios_set_flag(cplios, CPLIOS_UPDATE_RCV_WND);
...
static void pivot_ca_ops(struct sock *sk, int cong)
{
	...
	icsk->icsk_ca_ops = &t4_cong_ops[cong < 0 ? 2 : cong].ops;
}
#endif
 ...
 */
static void init_offload_sk(struct cpl_io_state *cplios,
			    struct toedev *dev,
			    ...
			    u16 peer_mss)
{
	...
	int rxq_perchan, rxq_idx, cong_algo;
	struct sock *sk = cplios->sk;
	...
	if (!(sk->sk_userlocks & SOCK_RCVBUF_LOCK))
		tcp_fixup_rcvbuf(sk);
	tp->rcv_wnd = select_rcv_wnd(cplios); /* depends in ulp_mode */
	...
	cplios->txplen_max = TOM_TUNABLE(cplios->toedev, txplen);
	...
	} else if (rxq_perchan > 1 && s->rssq == QUEUE_RANDOM) {
	...
	cong_algo = s->cong_algo >= 0 ? s->cong_algo : get_ca_ops(sk);
	cplios->opt2 = t4_calc_opt2(cplios, s, cplios->rss_qid, cong_algo);
	cplios->mtu_idx = select_mss(cplios, dst_mtu(dst), peer_mss);
	...
	/* mss configured in multiples of 1K */
	cplios->mss = min_not_zero(s->mss * 1024, TOM_TUNABLE(cplios->toedev, mss));
	skb_queue_head_init(&cplios->tx_queue);
	skb_queue_head_init(&cplios->ooo_queue);
	reset_wr_list(cplios);
...
#if defined(TCP_CONGESTION_CONTROL)
	pivot_ca_ops(sk, cong_algo);
#endif
}

static inline void check_sk_callbacks(struct cpl_io_state *cplios)
{
	struct sock *sk = cplios->sk;

	if (unlikely(sk->sk_user_data &&
		     !cplios_flag_nochk(cplios, CPLIOS_CALLBACKS_CHKD))) {
		if (install_special_data_ready(sk) > 0)
			sock_set_flag(sk, SOCK_NO_DDP);
		cplios_set_flag(cplios, CPLIOS_CALLBACKS_CHKD);
	}
}
	       ...
	       struct net_device *egress_dev)
{
	...
	/* check if we are in Ring-BB mode and so, if egress_dev
	 * needs to be updated.
	 */
	egress_dev = cxgb4_get_ringbb_egress(egress_dev);
	...
	if (!cplios)
		goto out_err;
	oproto = kzalloc(sizeof(*oproto), GFP_USER);
	if (!oproto)
		goto free_cplios;

	cplios->txdata_skb_cache = alloc_skb(TXDATA_SKB_LEN, GFP_KERNEL);
	if (!cplios->txdata_skb_cache)
		goto free_oproto;

	kref_init(&cplios->kref);
	atid = cxgb4_alloc_atid(d->tids, cplios);
	if (atid < 0)
		goto free_txdata_skb;
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 0, 0)
	err = conn_insert_handle(d, sk, atid);
#else
	err = xa_insert_bh(&d->aidr, atid, sk, GFP_NOWAIT);
#endif
	if (err < 0)
		goto free_tid;
	...
	if (sk->sk_family == AF_INET)
		...
		neigh = t4_dst_neigh_lookup(dst, &sk->sk_v6_daddr);
#endif
	if (!neigh) {
	...
	install_offload_ops(cplios, oproto);
	check_sk_callbacks(cplios);

	cplios->tx_c_chan = cxgb4_port_chan(egress_dev);
	cplios->rx_c_chan = cxgb4_port_e2cchan(egress_dev);
	init_offload_sk(cplios, tdev, atid,
			e, dst, egress_dev, &settings, 0);
	...
	cplios->toedev = tdev;
	pi = (struct port_info *)netdev_priv(egress_dev);
	...
	if (sk->sk_family == AF_INET)
		...
		mk_act_open_req6(sk, skb, qid_atid, e, &settings,
				 ...
				 &sk->sk_v6_daddr);
#endif
	cxgb4_l2t_send(cplios->egress_dev, skb, e);
	return 0;
...
static void t4_send_abort(struct sock *sk, int mode, struct sk_buff *skb)
{
	...
	if ((sk->sk_state == TCP_SYN_SENT) &&
		...
		cplios->idr = sk_insert_tid(TOM_DATA(cplios->toedev), sk, tid);
		use_negadv_tid = true;
	} else
		tid = cplios->tid;
	...
	req->rsvd1 = !cplios_flag_nochk(cplios, CPLIOS_TX_DATA_SENT);
	req->cmd = mode;
...
int t4_send_reset(struct sock *sk, int mode, struct sk_buff *skb)
{
	...
	if (unlikely(cplios_flag_nochk(cplios, CPLIOS_ABORT_SHUTDOWN) ||
		     !cplios->toedev)) {
		if (sk->sk_state == TCP_SYN_RECV)
			cplios_set_flag(cplios, CPLIOS_RST_ABORTED);
		goto out;
	}
	...
	cplios_set_flag(cplios, CPLIOS_ABORT_RPL_PENDING);
	 ...
	 */
	if (sk->sk_state == TCP_SYN_RECV) {
		t4_set_tcb_field_rpl_skb(sk, W_TCB_RX_DDP_BUF0_OFFSET, 0, 0,
					 DDP_COOKIE_OFFSET, 1);
		cplios_set_flag(cplios, CPLIOS_ABORT_SHUTDOWN);
	} else {
		cplios_set_flag(cplios, CPLIOS_ABORT_SHUTDOWN);
...
void __set_tcb_field_direct(struct cpl_io_state *cplios,
		     ...
		     u64 mask, u64 val, u8 cookie, int no_reply)
{
	...
	__set_tcb_field_direct(cplios, req, word, mask, val, cookie, no_reply);
	set_wr_txq(skb, CPL_PRIORITY_CONTROL, cplios->port_id);
}

void t4_set_tcb_field_skb(struct sock *sk, u16 word, u64 mask, u64 val)
{
	struct sk_buff *skb;
	struct cpl_set_tcb_field *req;
	struct ulptx_idata *sc;
...
static inline int __t4_set_tcb_field(struct sock *sk, u16 word,
				     u64 mask, u64 val, u8 cookie, int noreply)
{
	struct packed_wr {
		struct cpl_set_tcb_field cpl;
		struct ulptx_idata sc;
	} __packed sreq, *req = &sreq;

	unsigned int wrlen = roundup(sizeof(struct packed_wr), 16);
	int useskb = (sk->sk_state == TCP_SYN_SENT);
	struct cpl_io_state *cplios = CPL_IO_STATE(sk);
	struct toedev *tdev;
	...
	if (!useskb) {
		__set_tcb_field_direct(cplios, &sreq.cpl,
				       word, mask, val, cookie, noreply);
		useskb = cxgb4_ctrl_send(tdev->lldev[cplios->port_id], req, wrlen);
	}
	return useskb;
}

void t4_set_tcb_field_rpl_skb(struct sock *sk, u16 word,
...
void t4_set_tcb_field(struct sock *sk, u16 word,
		      u64 mask, u64 val)
{
	if (__t4_set_tcb_field(sk, word, mask, val, 0, 1))
		t4_set_tcb_field_skb(sk, word, mask, val);
}

void t4_set_tcb_field_rpl(struct sock *sk, u16 word,
			  u64 mask, u64 val, u8 cookie)
{
	if (__t4_set_tcb_field(sk, word, mask, val, cookie, 0))
		t4_set_tcb_field_rpl_skb(sk, word, mask, val, cookie, 0);
}
...
int t4_get_tcb(struct sock *sk, unsigned short cookie)
{
	struct cpl_io_state *cplios = CPL_IO_STATE(sk);
	...
	req->reply_ctrl = htons(V_REPLY_CHAN(cplios->rx_c_chan) |
		      V_QUEUENO(cplios->rss_qid));
	req->cookie = htons(cookie);
	set_wr_txq(skb, CPL_PRIORITY_CONTROL, cplios->port_id);

	if (sk->sk_state == TCP_SYN_SENT)
		__skb_queue_tail(&cplios->ooo_queue,
					 skb);
	else
		cxgb4_ofld_send(cplios->egress_dev, skb);
...
int t4_get_tcb_cong_algo(struct sock *sk)
{
	struct cpl_io_state *cplios = CPL_IO_STATE(sk);
	struct tcb_ofld_info *tcb_info;
	int cong_algo, ret;

	tcb_info = &cplios->tcb_info;
	init_completion(&tcb_info->completion);
	t4_get_tcb(sk, TCB_FIELD_COOKIE_CONG);
	/* Wait for reply */
	ret = wait_for_completion_timeout(&tcb_info->completion, 10 * HZ);
	if (!ret)
		return -ETIMEDOUT;

	for (cong_algo = 0; cong_algo < ARRAY_SIZE(t4_cong_ops); cong_algo++)
		if (t4_cong_ops[cong_algo].key == tcb_info->cong_key)
			break;
...
void update_cong_ops(struct sock *sk)
{
	struct inet_connection_sock *icsk = inet_csk(sk);
	int cong_algo;

	cong_algo = t4_get_tcb_cong_algo(sk);
	if (cong_algo >= 0)
		icsk->icsk_ca_ops = &t4_cong_ops[cong_algo].ops;
}
...
int t4_set_cong_control(struct sock *sk, const char *name)
{
	...
	for (cong_algo = 0; cong_algo < ARRAY_SIZE(t4_cong_ops); cong_algo++)
		if (!strcmp(name, t4_cong_ops[cong_algo].ops.name))
			break;

	if (cong_algo >= ARRAY_SIZE(t4_cong_ops))
		return -EINVAL;

	mask = V_TF_TURBO(1) | V_TF_CCTRL_SEL0(3);
	if (t4_cong_ops[cong_algo].key == CONG_ALG_NONE)
		val = V_TF_TURBO(1);
	else
		val = V_TF_CCTRL_SEL0(t4_cong_ops[cong_algo].key);
	t4_set_tcb_field(sk, W_TCB_T_FLAGS, mask, val);
	update_cong_ops(sk);
...
static void tcb_rpl_as_ddp_complete(struct sock *sk, struct sk_buff *skb)
{
	...
	inet_csk(sk)->icsk_ack.lrcvtime = tcp_jiffies32;
...
static void new_rx_data(struct sock *sk, struct sk_buff *skb)
{
	...
	if (unlikely(hdr->status)) {
		u8 status = READ_ONCE(hdr->status);
...
static void new_rx_data_ddp(struct sock *sk, struct sk_buff *skb)
{
	...
	if (unlikely(ntohl(hdr->ddpvld) & DDP_ERR)) {
		struct ddp_gather_list *gl = bsp->gl;

		if (ddp_report & F_DDP_INV)
			bsp->flags &= ~DDP_BF_NOCOPY;

		netdev_err(cplios->egress_dev, "RX_DATA_DDP: CPL TID=%u error=0x%x ppod addr=0x%x\n",
		       GET_TID(hdr), ntohl(hdr->ddpvld), gl->tag<<6);
		kfree_skb(skb);
		return;
	}
...
static void tls_skb_data_len(struct sk_buff *skb, unsigned int new_len)
{
	struct tlsrx_hdr_pkt *tls_hdr_pkt = cplhdr(skb);

	skb->hdr_len = ntohs(tls_hdr_pkt->length);
	tls_hdr_pkt->length = ntohs(new_len);
}

static void new_rx_tls_cmp(struct sock *sk, struct sk_buff *skb)
{
	struct cpl_io_state *cplios = CPL_IO_STATE(sk);
	struct tls_ofld_info *tls_ofld = TLS_IO_STATE(sk);
	...
	tls_hdr_pkt = (struct tlsrx_hdr_pkt *)skb->data;
	...
	tp->rcv_nxt +=
		G_CPL_RX_TLS_CMP_PDULENGTH(ntohl(cmp_cpl->pdulength_length));

	ULP_SKB_CB(skb)->flags |= ULPCB_FLAG_TLS_HDR;
	skb_data = __skb_dequeue(&tls_ofld->sk_recv_queue);
	if (!skb_data) {
		__skb_queue_tail(&sk->sk_receive_queue, skb);
	} else {
		tls_skb_data_len(skb, tls_ofld->pld_len);
		tls_ofld->pld_len = 0;
		__skb_queue_tail(&sk->sk_receive_queue, skb);
		__skb_queue_tail(&sk->sk_receive_queue, skb_data);
	}
...
static int do_cpl_rx_tls_cmp(struct tom_data *td, struct sk_buff *skb)
{
	struct sock *sk;
	struct cpl_rx_data *req = cplhdr(skb);
	unsigned int hwtid = GET_TID(req);
	...
	VALIDATE_SOCK(sk);
	skb_gl_set(skb, NULL);
	process_cpl_msg(new_rx_tls_cmp, sk, skb);

	return 0;
}

static void new_tls_data(struct sock *sk, struct sk_buff *skb)
{
	struct cpl_io_state *cplios = CPL_IO_STATE(sk);
	struct tls_ofld_info *tls_ofld = TLS_IO_STATE(sk);
...
#if VALIDATE_SEQ
	if (unlikely(ULP_SKB_CB(skb)->seq != tp->rcv_nxt)) {
		pr_err("%s: TID %u: Bad sequence number %u, expected%u\n",
		       cplios->toedev->name, cplios->tid,
		       ULP_SKB_CB(skb)->seq, tp->rcv_nxt);
		__kfree_skb(skb);
		return;
	}
	...
	if (!skb->data_len)
		__skb_trim(skb,
			   G_CPL_TLS_DATA_LENGTH(ntohl(hdr->length_pkd)));
	...
	tcp_hdr(skb)->fin = 0;
	inet_csk(sk)->icsk_ack.lrcvtime = tcp_jiffies32;
	tls_ofld->pld_len = G_CPL_TLS_DATA_LENGTH(ntohl(hdr->length_pkd));
	__skb_queue_tail(&tls_ofld->sk_recv_queue, skb);
}

/*
 *  Handler for CPL_TLS_DATA messages
 */
static int do_cpl_tls_data(struct tom_data *td, struct sk_buff *skb)
{
	struct sock *sk;
	struct cpl_rx_data *req = cplhdr(skb);
	unsigned int hwtid = GET_TID(req);
	...
	VALIDATE_SOCK(sk);
	skb_gl_set(skb, NULL);
	process_cpl_msg(new_tls_data, sk, skb);
	return 0;
}

void process_tcb_info_rpl(struct sock *sk, struct sk_buff *skb)
{
	struct cpl_io_state *cplios = CPL_IO_STATE(sk);
	struct tcb_ofld_info *tcb_info;
	struct cpl_get_tcb_rpl *rpl;
	u64 t_flags_64;
	__be64 *tcb;
	...
	tcb_info->cong_key = (t_flags_64 & (V_TF_CCTRL_SEL0(1) |
					    V_TF_CCTRL_SEL1(1))) >>
			      S_TF_CCTRL_SEL0;
	complete(&tcb_info->completion);
	kfree_skb(skb);
}
...
static int do_get_tcb_rpl(struct tom_data *td, struct sk_buff *skb)
{
	...
	if (rpl->cookie == TCB_FIELD_COOKIE_CONG) {
		process_cpl_msg(process_tcb_info_rpl, sk, skb);
		return 0;
	}

#ifdef CONFIG_T4_MA_FAILOVER	
	process_cpl_msg(process_get_tcb_mafo_rpl, sk, skb);
#endif
	return 0;
}
	...
	case TCP_FIN_WAIT1:
		...
		if (!sock_flag(sk, SOCK_DEAD))
			sk->sk_state_change(sk); // Wake up lingering close()
			 ...
			 !cplios_flag_nochk(cplios, CPLIOS_ABORT_SHUTDOWN))
...
static void process_abort_rpl(struct sock *sk, struct sk_buff *skb)
{
	...
	if (rpl->rss_hdr.channel != cplios->tx_c_chan) {
		cplios_reset_flag(cplios, CPLIOS_TX_WAIT_IDLE);
		cplios_reset_flag(cplios, CPLIOS_TX_FAILOVER);
	}

	if (cplios_flag_nochk(cplios, CPLIOS_ABORT_RPL_PENDING)) {
		cplios_reset_flag(cplios, CPLIOS_ABORT_RPL_PENDING);
		if (!cplios_flag_nochk(cplios, CPLIOS_ABORT_REQ_RCVD)) {
			if (sk->sk_state == TCP_SYN_SENT) {
				struct cpl_io_state *cplios = CPL_IO_STATE(sk);
				struct tom_data *td = TOM_DATA(cplios->toedev);
				struct tid_info *tids = td->tids;
				...
				bh_remove_handle(td, cplios->idr);
				cxgb4_remove_tid(tids,
						 cplios->port_id, GET_TID(rpl),
						 sk->sk_family);
				sock_put(sk);
			}
			t4_release_offload_resources(sk);
			   ...
			   struct toedev *tdev, int rst_status, int queue)
{
	...
	if (sock_flag(sk, SOCK_OFFLOADED)) {
		...
		if (e && (sk->sk_state != TCP_SYN_RECV)) {
			cxgb4_l2t_send(cplios->egress_dev, reply_skb, e);
			return;
		}
	}
	cxgb4_ofld_send(tdev->lldev[0], reply_skb);
...
{
	struct cpl_io_state *cplios = CPL_IO_STATE(child);
	struct request_sock *req = cplios->passive_reap_next;

	if (!req)
		return;
	reqsk_queue_removed(&inet_csk(parent)->icsk_accept_queue, req);
	__skb_unlink((struct sk_buff *)&cplios->synq, &cplios->listen_ctx->synq);
	t4_reqsk_free(req);
	cplios->passive_reap_next = NULL;
}
 ...
 */
static int abort_syn_rcv(struct cpl_io_state *cplios, struct sk_buff *skb)
{
	struct sock *parent, *sk = cplios->sk;
	struct toedev *tdev = cplios->toedev;
	const struct request_sock *oreq = cplios->passive_reap_next;
...
static void process_abort_req(struct sock *sk, struct sk_buff *skb)
{
	...
	if (is_neg_adv(req->status)) {
		struct dst_entry *dst = __sk_dst_get(sk);
		unsigned int mtu_idx = select_mss(cplios, dst_mtu(dst), 0);

		if (mtu_idx < cplios->mtu_idx) {
			int flowclen16;

			/* Do not update MSS if the first TX_DATA is not sent
			 * or if ABORT_REQ/CLOSE_CON_REQ is received.
			 */
			if (!cplios_flag_nochk(cplios, CPLIOS_TX_DATA_SENT) ||
			    cplios_flag_nochk(cplios, CPLIOS_ABORT_RPL_PENDING) ||
			    cplios_flag_nochk(cplios, CPLIOS_CLOSE_CON_REQUESTED))
				goto free_skb;

			flowclen16 = send_tx_param_update_wr(sk,
						FW_FLOWC_MNEM_MSS,
						tcp_sk(sk)->advmss, 0);
...
free_skb:
	...
	cplios_reset_flag(cplios, CPLIOS_ABORT_REQ_RCVD);

	if (req->rss_hdr.channel != cplios->tx_c_chan) {
		cplios_reset_flag(cplios, CPLIOS_TX_WAIT_IDLE);
		cplios_reset_flag(cplios, CPLIOS_TX_FAILOVER);
	}
	/*
	 ...
	 */
	if (!cplios_flag_nochk(cplios, CPLIOS_ABORT_SHUTDOWN) &&
	    !cplios_flag_nochk(cplios, CPLIOS_TX_DATA_SENT)) {
		struct tcp_sock *tp = tcp_sk(sk);
	...
	if (!cplios_flag_nochk(cplios, CPLIOS_ABORT_RPL_PENDING)) {
		 ...
		 */
		if (sk->sk_state == TCP_SYN_RECV && !abort_syn_rcv(cplios, skb))
			return;
#ifdef WD_TOE
		if (is_wdtoe(sk))
			wdtoe_dec_conn_num(sk);
				 ...
				 const struct offload_settings *s)
{
	...
	*retry = 0;
	 ...
	 * needs to be updated.
	 */
	egress = cxgb4_get_ringbb_egress(egress);
	...
	if (!newcplios)
		goto free_l2t;
	oproto = kzalloc(sizeof(*oproto), GFP_ATOMIC);
	if (!oproto) {
		kfree(newcplios);
		goto free_l2t;
	}
	newcplios->txdata_skb_cache =  alloc_skb(TXDATA_SKB_LEN, GFP_ATOMIC);
	if (!newcplios->txdata_skb_cache) {
		kfree(newcplios);
		goto free_l2t;
	}
	...
	newcplios->tx_c_chan = cxgb4_port_chan(egress);
	newcplios->rx_c_chan = cxgb4_port_e2cchan(egress);
	...
	if (iph->version == 0x4) {
		...
		inet6_request_sock_t *treq = inet6_rsk(oreq);
...
static int syn_netfilter_fini(struct net *net, struct sock *sk,
			      struct sk_buff *skb)
{
	kfree_skb(skb);
	return 0;
...
static void process_pass_accept_req(struct sock *sk, struct sk_buff *skb)
{
	...
	rcu_read_lock();
	if (!rcu_access_pointer(tdev->can_offload) ||
	    !tdev->can_offload(tdev, sk)) {
		rcu_read_unlock();
		goto reject;
	}
	...
	if (vlan_id != CPL_L2T_VLAN_NONE && vlan_id) {
		if (master)
			vlan_dev = __vlan_find_dev_deep_rcu(master,
							    htons(ETH_P_8021Q),
							    vlan_id);
		else
			vlan_dev = __vlan_find_dev_deep_rcu(egress_dev,
							    htons(ETH_P_8021Q),
							    vlan_id);
		if (!vlan_dev) {
...
if (syn_netfilter) {
	struct net *net;
	struct sk_buff *nf_skb;
	int nf_retval;

	nf_skb = skb_clone(skb, GFP_ATOMIC);
	if (!nf_skb)
		goto defer;

	/* Implementation doesn't handle VLAN cases */
	if (master)
		nf_skb->dev = master;
	net = dev_net(nf_skb->dev);
	__skb_pull(nf_skb, sizeof(*req));
	nf_skb->ip_summed = CHECKSUM_UNNECESSARY;
	nf_skb->protocol = eth_type_trans(nf_skb, nf_skb->dev);
	nf_retval = NF_HOOK(iph->version == 0x4 ? NFPROTO_IPV4 : NFPROTO_IPV6,
			    NF_INET_LOCAL_IN,
			    net, NULL, nf_skb, nf_skb->dev, NULL,
			    syn_netfilter_fini);
	if (nf_retval < 0)
		goto reject;
}
	...
	if (req->tcpopt.wsf <= 14 && tcp_win_scaling_enabled(sock_net(sk))) {
	...
	cplios->listen_ctx = listen_ctx;
	cplios->lsk = sk;
	__skb_queue_tail(&listen_ctx->synq, (struct sk_buff *)&cplios->synq);

	reply_skb->sk = newsk;
	t4_set_arp_err_handler(reply_skb, NULL, pass_accept_rpl_arp_failure);

	e = cplios->l2t_entry;
	pi = (struct port_info *)netdev_priv(cplios->egress_dev);
	cplios->smac_idx = pi->smt_idx;
...
toe:
	 ...
	 */
	cplios->mtu_idx = select_mss(cplios, dst_mtu(__sk_dst_get(newsk)),
...
#if VALIDATE_TID
	if (unlikely(tid_out_of_range(td->tids, tid))) {
...
static void build_cpl_pass_accept_req(struct sock *sk, struct sk_buff *skb,
				      int stid, u8 tos, enum chip_type type)
{
	u32 l2info;
	...
	t4_tcp_parse_options(sk, skb, &tmp_opt, &hash_location, 0);
...
static int do_rx_pkt(struct tom_data *td, struct sk_buff *skb)
{
	...
	cplios = kzalloc(sizeof(struct cpl_io_state), GFP_ATOMIC);
	if(!cplios)
		return -ENOMEM;
	 ...
	 */
	build_cpl_pass_accept_req(lsk, skb, stid, tos, td->lldi->adapter_type);
				    ...
				    struct toedev *dev)
{
	...
	if (unlikely(!sock_flag(child, SOCK_OFFLOADED)))
		return;

	cplios = CPL_IO_STATE(child);
	oreq = cplios->passive_reap_next;
	cplios->passive_reap_next = NULL;
	...
	if (sk_acceptq_is_full(lsk) && !TOM_TUNABLE(dev, soft_backlog_limit)) {
		...
		/* The refcnt will be zero here, set it to one to allow the
		 * stack to free the request_sock when it is removed from the
		 * accept queue else we will leak the memory.
		 */
...
static int do_pass_establish(struct tom_data *td, struct sk_buff *skb)
{
	...
	if (unlikely(sock_owned_by_user(sk))) {
		...
		bh_lock_sock(lsk);
		if (unlikely(skb_queue_empty(&listen_ctx->synq))) {
			/* removed from synq */
			bh_unlock_sock(lsk);
			kfree_skb(skb);
			goto unlock;
...
static void fixup_and_send_ofo(struct cpl_io_state *cplios, unsigned int tid)
{
	...
	while ((skb = __skb_dequeue(&cplios->ooo_queue)) != NULL) {
		...
		if (G_FW_WR_OP(ntohl(flowc->op_to_nparams)) == FW_FLOWC_WR) {
			...
			if (flowc->mnemval[0].mnemonic == FW_FLOWC_MNEM_CH)
...
static void sock_act_establish(struct sock *sk, struct sk_buff *skb)
{
	 ...
	 */
	if (skb_queue_len(&cplios->ooo_queue))
		fixup_and_send_ofo(cplios, cplios->tid);
	...
	/* For TLS connect start handshake parse timer */
	if(is_tls_offload(sk))
		start_hndsk_work(sk);
}
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 0, 0)
	cplios->idr = bh_insert_handle(td, sk, tid);
#else
	err = xa_insert_bh(&td->hwtid_idr, tid, sk, GFP_NOWAIT);
	cplios->idr = tid;
#endif
	bh_conn_remove_handle(td, atid);
...
#endif
			if (cplios->wr_nondata)
				cplios->wr_nondata -= credits;
#if DEBUG_WR > 1
/* Currently, wr_nondata is only used for sending
 * flowc WR for failover so should only be needed
 * once before TX data
 */
			if (cplios->wr_nondata)
				 netdev_err(cplios->egress_dev,
					    "WR credits=%u nondata=%u\n",
					    credits, cplios->wr_nondata);
#endif
#ifdef WD_TOE
wdtoe_ack_out:
#endif
        ...
        if (hdr->flags & CPL_FW4_ACK_FLAGS_SEQVAL) {
			...
			if ((tp->snd_una == tp->snd_nxt) &&
				!cplios_flag_nochk(cplios, CPLIOS_TX_FAILOVER))
					cplios_reset_flag(cplios,
							  CPLIOS_TX_WAIT_IDLE);
                }
	...
	if (hdr->flags & CPL_FW4_ACK_FLAGS_FLOWC) {
		if (cplios_flag_nochk(cplios, CPLIOS_TX_FAILOVER)) {
			struct cpl_io_state *cplios = CPL_IO_STATE(sk);
			struct l2t_entry *e = cplios->l2t_entry;
	...
	if (hdr->flags & CPL_FW4_ACK_FLAGS_CH) {
		unsigned int flowclen16 =
			DIV_ROUND_UP(failover_flowc_wr_len, 16);
...
static void uo_wr_ack(struct sock *sk, struct sk_buff *skb)
{
	...
	if (sk->sk_family == AF_INET) {
		...
		if (skb_queue_len(&cplios->tx_queue) &&
			!chelsio_udp_v6_push_pending_frames(sk, NULL))
				sk->sk_write_space(sk);
	}
 ...
 */
void t4_reset_synq(struct listen_ctx *listen_ctx)
{
	...
	while (!skb_queue_empty(&listen_ctx->synq)) {
		struct cpl_io_state *cplios =
			container_of((struct synq *)skb_peek(&listen_ctx->synq),
				     struct cpl_io_state, synq);
		struct sock *child = cplios->sk;

		if ((sk_ofld_proto_get_tomhandlers(child) ==
			&t4_tcp_prot.proto) ||
		    (sk_ofld_proto_get_tomhandlers(child) ==
...
int t4_set_dsgl_ppods(const struct cpl_io_state *cplios,
		      const struct ddp_gather_list *gl,
		      const struct dsgl_req *dsgl,
		      const unsigned int ppod_addr,
		      const unsigned int nppods,
		      const unsigned int maxoff,
		      const unsigned int startidx)
{
	...
	unsigned int podchunk = nppods*PPOD_SIZE;

	len = roundup(sizeof(*req) + sizeof(*sgl), 16);
	...
	req->wr.wr_hi = cpu_to_be32(V_FW_WR_OP(FW_ULPTX_WR)|V_FW_WR_COMPL(1));
	req->wr.wr_mid = cpu_to_be32(V_FW_WR_LEN16(DIV_ROUND_UP(len, 16)));
	req->wr.wr_lo = (uintptr_t)dsgl;
	req->cmd = htonl(V_ULPTX_CMD(ULP_TX_MEM_WRITE) |
			 V_T5_ULP_MEMIO_ORDER(1) |
			 V_T5_ULP_MEMIO_FID(cplios->rss_qid) |
			 F_ULP_TXPKT_RO);
	req->dlen = cpu_to_be32(V_ULP_MEMIO_DATA_LEN(podchunk>>ULP_TX_MEMWRITE_ALIGN));
	req->len16 = cpu_to_be32(DIV_ROUND_UP(len-sizeof(req->wr), 16));
	req->lock_addr = htonl(V_ULP_MEMIO_ADDR(ppod_addr>>ULP_TX_MEMWRITE_ALIGN));

	sgl = (struct ulptx_sgl *)(req + 1);
	sgl->cmd_nsge = cpu_to_be32(V_ULPTX_CMD(ULP_TX_SC_DSGL) |
				    V_ULPTX_NSGE(1));
	sgl->len0 = cpu_to_be32(podchunk);
	sgl->addr0 = cpu_to_be64(dsgl->dsgl_iova);

	p = (struct pagepod *)dsgl->dsgl_vaddr;
	i = startidx;
	do {
		pidx = 4 * i;
		p->vld_tid_pgsz_tag_color =
			cpu_to_be64(F_PPOD_VALID |
				V_PPOD_TID(tid) |
				V_PPOD_TAG(gl->tag) |
				V_PPOD_COLOR(gl->color));
		p->len_offset = cpu_to_be64(V_PPOD_LEN(maxoff) |
					V_PPOD_OFST(gl->offset));
		p->rsvd = 0;
		for (j = 0; j < 5; ++j, ++pidx)
...
int t4_setup_ppods(const struct cpl_io_state *cplios,
		   const struct ddp_gather_list *gl,
		   const unsigned int nppods,
		   ...
		   const unsigned int maxoff)
{
	...
	for (i = 0; i < nppods; ppod_addr += podchunk) {
		...
		req->lock_addr = htonl(V_ULP_MEMIO_ADDR(
				       ppod_addr>>ULP_TX_MEMWRITE_ALIGN));
		sc = (struct ulptx_idata *)(req+1);
		sc->cmd_more = htonl(V_ULPTX_CMD(ULP_TX_SC_IMM));
...
static int do_fw6_msg(struct tom_data *td, struct sk_buff *skb)
{
	...
	if (p->type == FW6_TYPE_OFLD_CONNECTION_WR_RPL) {
		...
		if (!t4_cpl_iscsi_callback(td, sk, skb, CPL_FW6_MSG))
			return 0;
	} else if (p->type == FW_TYPE_WR_RPL) {
		struct cpl_fw6_msg *rpl = cplhdr(skb);

		if (rpl->data[1]) {
			struct dsgl_req *req = (struct dsgl_req *)rpl->data[1];

			dma_pool_free(td->dma_pool, req->dsgl_vaddr,
				      req->dsgl_iova);
			kfree(req);
		}
	}
...
int t4_init_sk_filter(void)
{
	...
	BUG_ON(ARRAY_SIZE(drop_insnsi) != 2);
	...
	bpf_prog_select_runtime(drop_bpf, &err);
	...
	refcount_set(&drop_all->refcnt, 1);
	return 0;
err:
	return -1;
}
...
int __init t4_init_cpl_io(void)
{
	...
	t4tom_register_cpl_handler(CPL_TLS_DATA, do_cpl_tls_data);
	t4tom_register_cpl_handler(CPL_RX_TLS_CMP, do_cpl_rx_tls_cmp);
