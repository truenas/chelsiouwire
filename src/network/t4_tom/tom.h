...
struct tom_tunables {
	 ...
	 */
	int offload_vlan;
#define MAX_TLS_PORTS 12
	int tls_ports[MAX_TLS_PORTS];
	int txplen;
	int nagle; /* NAGLE enabled/disabled */
};
...
#define T4_TRACE_TOM_BUFFERS 8
	...
	unsigned int rss_qid[2]; // per Rx C-channel
	...
	struct idr hwtid_idr;  /* TID id space */
#else
	struct xarray aidr; /* ATID id space */
	struct xarray hwtid_idr;  /* TID id space */
#endif
	struct key_map kmap;
};
 ...
 */
static inline void *t4tom_alloc_mem(unsigned long size)
{
 ...
 */
static inline void t4tom_free_mem(void *addr)
{
