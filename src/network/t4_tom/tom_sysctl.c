...
static struct tom_tunables default_tunable_vals = {
	...
	.zcopy_sendmsg_partial_thres = 65536,
	 ...
	 */
	.offload_vlan = 0,
	/* Some of the standard ports which runs over TLS/SSL
	 * 443: HTTPS
	 * 989: FTPS protocol (data)
	 * 990: FTPS protocol (control)
	 * 992: Telnet protocol
	 * 993: Internet Message Access protocol (IMAPS)
	 * 994: Internet Relay Chat (IRCS)
	 * 995: Post Office Protocol 3 (POP3S)
	 * 1364: IBM COnnect: Direct FTP+
	 * 4116: Smartcard-TLS
	 * 5349: STUN protocol for NAT traversal
	 * 6514: Syslog over TLS
	 * 8531: Windows Server Update Services
	 * TLS offload can also be enabled through COP
	 */
	.tls_ports = {0},
	.txplen = 64000,
	.nagle = 0,
...
static struct tom_sysctl_table tom_sysctl = {
	.tunables = {
		...
		TUNABLE_INT(OFFLOAD_VLAN, "offload_vlan",
                            offload_vlan),
		TUNABLE_INT(TLS_PORTS, "tls_ports", tls_ports),
		TUNABLE_INT(TXPLEN, "txplen", txplen),
		TUNABLE_INT(NAGLE, "nagle", nagle),
	},
	.tom_instance_dir = {
		{
#if defined(SYSCTL_CTL_NAME)
