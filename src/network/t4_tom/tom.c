...
static int dsgl_enable = 1;
module_param(dsgl_enable, int, 0644);
MODULE_PARM_DESC(dsgl_enable,
		 "Enable Direct Scatter-Gather lists");
...
static ssize_t (*wdtoe_cmd_table_new[])(struct cxgb4_lld_info *lldi,
	...
	*/
	...
	[WDTOE_CMD_PASS_TID] = wdtoe_pass_tid,
};
#endif
...
int t4_close(struct toedev *dev)
{
	...
	spin_lock_bh(&d->idr_lock);
		if (!idr_is_empty(&d->hwtid_idr)) {
			spin_unlock_bh(&d->idr_lock);
			return -EIO;
		}
	spin_unlock(&d->idr_lock);

	spin_lock(&d->aidr_lock);
		if (!idr_is_empty(&d->aidr)) {
	...
	local_bh_disable();
	if (xa_empty(&d->hwtid_idr) && xa_empty(&d->aidr))
		err = 0;
	local_bh_enable();
	return err;
#endif
...
int t4_can_offload(struct toedev *dev, struct sock *sk)
{
	...
	accept = inet_sk(sk)->inet_opt == NULL && d->conf.activated;
	if (!accept)
		return 0;

	if (d->conf.max_conn < 0)
		return 1;

	barrier();
	accept  = atomic_read(&t->conns_in_use) +
...
static uint32_t decompress_txt(uint8_t *c_txt, uint8_t *uc_txt)
{
	uint32_t c_ptr, cod_pos, ptr_ofst, uc_sz;
	uint16_t in_ptr, ptr_pos;
	uint8_t ptr_len;

	uc_sz = *((uint32_t *)c_txt);
	c_ptr = 4;

	for (cod_pos = 0; cod_pos < uc_sz; ++cod_pos) {
		in_ptr = *((uint32_t *)(c_txt + c_ptr));
		c_ptr += 2;
		ptr_pos = in_ptr >> 4;
		ptr_len = in_ptr & 15;
		if (ptr_pos)
			for (ptr_ofst = cod_pos - ptr_pos;
			     ptr_len > 0;
			     --ptr_len)
				uc_txt[cod_pos++] = uc_txt[ptr_ofst++];
		*(uc_txt + cod_pos) = *(c_txt + c_ptr++);
	}

	return cod_pos;
}

static int proc_cop_show(struct seq_file *seq, void *v)
{
	struct tom_data *t = seq->private;
	unsigned int version;
	void *data = NULL;
	int ret;

	ret = get_cop_txt_data(&t->tdev, &data, &version);
	if ((ret > 0) && (data != NULL)) {
		uint8_t *c_txt = (uint8_t *)data;
		uint8_t *uc_txt;
		uint32_t uc_sz;

		uc_sz = *((uint32_t *)c_txt);
		uc_txt = (uint8_t *)kmalloc(uc_sz + 1, GFP_KERNEL);
		if (!uc_txt)
			return 0;
		if (decompress_txt(c_txt, uc_txt) != uc_sz) {
			kfree(uc_txt);
			return 0;
		}
		uc_txt[uc_sz] = '\0';
		seq_printf(seq, "COP text header version %u\n", version);
		seq_printf(seq, "\nCOP policies:\n");
		seq_printf(seq, "%s\n",uc_txt);
		kfree(uc_txt);
	}
...
static int proc_cop_open(struct inode *inode, struct file *file)
{
...
{
        if (dir)
                remove_proc_entry("read-cop", dir);
}


static int tom_cop_proc_setup(struct proc_dir_entry *dir, struct tom_data *d)
{
        ...
        p = proc_create_data("read-cop", S_IRUGO, dir,
                             &proc_cop_fops, d);
        if (!p)
                return -ENOMEM;
...
static int proc_info_show(struct seq_file *seq, void *v)
{
	...
	seq_printf(seq, "C-channel 0 SYN queue ID: %u\n", d->rss_qid[0]);
	seq_printf(seq, "C-channel 1 SYN queue ID: %u\n", d->rss_qid[1]);
#ifdef DEBUG
...
static void tom_proc_init(struct toedev *dev)
{
        ...
        tom_cop_proc_setup(dev->proc_dir, TOM_DATA(dev));
}
...
static void tom_proc_cleanup(struct toedev *dev)
{
	if (dev->offload_mod) {
		...
		tom_cop_proc_free(dev->proc_dir);
	}
}
#else
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 0, 0)
	spin_lock_init(&t->aidr_lock);
	spin_lock_init(&t->idr_lock);
#endif
...
static int t4tom_set_tx_wait_idle(int id, void *p, void *data)
{
	struct sock *sk = p;
	bool lock;
	...
	sock_hold(sk);
	lock = spin_trylock_bh(&sk->sk_lock.slock);
	cplios_set_flag(CPL_IO_STATE(sk),
			CPLIOS_TX_WAIT_IDLE);
	if (lock)
...
static int abort_ofld_conn(int id, void *p, void *data)
{
	...
	struct sock *sk = p;
	struct net_device *egress_dev = data;
	...
	if (sk->sk_family == AF_INET) {
		...
		init_toe_hash_params(&hash_params, neigh->dev, neigh,
				     ...
				     inet_sk(sk)->inet_dport,
				     &inet6_sk(sk)->saddr.s6_addr32[0],
				     &sk->sk_v6_daddr.s6_addr32[0],
				     true, IPPROTO_TCP);
		netdev = offload_get_phys_egress(&hash_params, TOE_FAILOVER);
#endif
	}
			     ...
			     bool release_ofld_res)
{
	...
	xa_for_each(&td->hwtid_idr, index, sk) {
		abort_ofld_conn(index, sk, egress_dev);
		if (release_ofld_res) {
...
static int t4_toe_detach(struct toedev *dev)
{
	...
	cancel_work_sync(&TOM_DATA(dev)->deferq_task);
...
static void *t4tom_uld_add(const struct cxgb4_lld_info *infop)
{
	...
	*lldi = *infop;
	...
	t->pfvf = V_FW_PFVF_CMD_PFN(lldi->pf);

	/* OK if this fails, we just can't do DDP */
	spin_lock_init(&t->ppod_map_lock);
	t->start_tag = 0;
	t->dma_pool = NULL;
	if (dsgl_enable && lldi->ulptx_memwrite_dsgl && lldi->vr->ddp.size) {
		t->dma_pool = dma_pool_create("TOE page pods",
					      &t->pdev->dev,
					      PPOD_CLUSTER_SIZE*PPOD_SIZE,
					      PPOD_SIZE, 0);
		if (!t->dma_pool)
			netdev_info(lldi->ports[0], "TOE page pod dmapool failed\n");
	} else
		netdev_info(lldi->ports[0], "TOE page pod dmapool disabled\n");
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 0, 0)
	idr_init(&t->aidr);
	idr_init(&t->hwtid_idr);
#else
	xa_init_flags(&t->aidr, XA_FLAGS_LOCK_BH);
	xa_init_flags(&t->hwtid_idr, XA_FLAGS_LOCK_BH);
#endif
out:
	return t;
}
...
int t4_recv_rsp(struct tom_data *td, const __be64 *rsp)
{
	...
	skb = td->rspq_skb_cache[rspq_bin];
		...
		!skb_shared(skb) && !skb_cloned(skb)) {
			if (atomic_inc_return(&skb->users.refs) == 2) {
                		__skb_trim(skb, 0);
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 0, 0)
	idr_destroy(&dev->hwtid_idr);
	idr_destroy(&dev->aidr);
#else
	xa_destroy(&dev->hwtid_idr);
	xa_destroy(&dev->aidr);
#endif
}
...
static int t4tom_uld_state_change(void *handle, enum cxgb4_state new_state)
{
	...
	switch(new_state) {
	case CXGB4_STATE_UP: {
		rcu_assign_pointer(tdev->in_shutdown, NULL);

		for (chan = 0; chan < 2; chan++) {
			for (i = 0; i < NCHAN; i++) {
				if (t->egr_dev[i] && (chan == cxgb4_port_e2cchan(t->egr_dev[i]))) {
				t->rss_qid[chan] =
					t->lldi->rxq_ids[i*t->lldi->nrxq / t->lldi->nchan];
				break;
				}
			}
			if (!t->rss_qid[chan])
				t->rss_qid[chan] = t->lldi->rxq_ids[0];
		}
		if (!activate_offload(&t->tdev))
			walk_listens(&t->tdev, listen_offload, DISABLE_WALK_SUPPORT);
	...
	case CXGB4_STATE_DETACH:
		...
		t4_rst_all_conn(tdev, tdev->lldev[0], true);
...
static int t4tom_uld_control(void *handle, enum cxgb4_control control, ...)
{
	...
	case CXGB4_CONTROL_SET_OFFLOAD_POLICY:
		va_start(ap, control);
		opf = va_arg(ap, struct ofld_policy_file *);
		length = va_arg(ap, uint32_t);

		ret = set_offload_policy(&t->tdev, opf, length);
		va_end(ap);
		break;
				...
				size_t len, loff_t *loff)
{
	...
	if (hdr.command >= NUM_WDTOE_CMDS || !wdtoe_cmd_table_new[hdr.command])
		return -EINVAL;
				...
				size_t len, loff_t *loff)
{
	...
	if (hdr.command >= NUM_WDTOE_CMDS || !wdtoe_cmd_table[hdr.command])
		return -EINVAL;
...
int __init t4_tom_init(void)
{
	...
	cxgb4_register_uld_type(CXGB4_ULD_TOE, &t4tom_uld_info);
...
static void __exit t4_tom_exit(void)
{
	...
	if (unsupported_allow_unload) {
		t4_inet_twsk_purge(&tcp_hashinfo, AF_INET);
#if defined(CONFIG_TCPV6_OFFLOAD)
		t4_inet_twsk_purge(&tcp_hashinfo, AF_INET6);
#endif
	}
	...
	cxgb4_unregister_uld_type(CXGB4_ULD_TOE);
