...
static inline void t4_tls_purge_receive_queue(struct sock *sk)
{
	...
	while ((skb = __skb_dequeue(&tls_ofld->sk_recv_queue)) != NULL) {
		skb_gl_set(skb, NULL);
		kfree_skb(skb);
	}
}
...
void free_atid(struct cpl_io_state *cplios,
	       struct tom_data *td,
	       unsigned int atid);
...
void t4_reset_synq(struct listen_ctx *ctx);
...
void t4_release_offload_resources(struct sock *);
...
static inline int send_failover_flowc_wr(struct sock *sk) {return 0; }
#endif
#define failover_flowc_wr_len offsetof(struct fw_flowc_wr, mnemval[1])
...
int chelsio_udp_v6_push_pending_frames(struct sock *sk, struct rt6_info *);
...
extern void (*ipv6_push_nfrag_opts_p)(struct sk_buff *skb,
