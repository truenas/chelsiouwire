...
extern unsigned long long calc_opt0(struct sock *sk, int nagle);
extern int sk_insert_tid(struct tom_data *d, struct sock *sk,
			 unsigned int tid);
...
void process_get_tcb_mafo_rpl(struct sock *sk, struct sk_buff *skb)
{
	if(cplios_flag(sk, CPLIOS_MA_FAILOVER)) {
		...
		u32 cong_ctrl, core_fin, peer_fin, nagle;
		...
		nagle = (t_flags_64 & V_TF_NAGLE(1)) >> S_TF_NAGLE;
		...
		if (likely(cplios->ma_fail_info.flags == MA_FAIL_OVER)) {
			if (!cplios_flag(sk, CPLIOS_TX_DATA_SENT)) {
				if (send_tx_flowc_wr(sk, 0,
					    tp->snd_nxt, tp->rcv_nxt) < 0)
					BUG_ON(1);
			}
		}
		...
		old_info.idr = cplios->idr;
		cplios->idr = cplios->ma_fail_info.idr;
		...
		/* If we are aborting and we have pending Tx, don't touch wr_credits */
		if (!(cplios->ma_fail_info.flags == MA_FAIL_ABORT &&
		      after(tp->snd_nxt, tp->snd_una))) {
			cplios->wr_max_credits = cplios->wr_credits =
				min_t(unsigned int,
				      (TOM_DATA(cplios->toedev))->max_wr_credits,
				      TOM_TUNABLE(cplios->toedev, max_wr_credits));
			cplios->wr_unacked = 0;
		}
		cplios_reset_flag(cplios, CPLIOS_TX_DATA_SENT);
		smp_mb();

		/* send flowc */
		flowclen16 = send_tx_flowc_wr(sk, 1, tp->snd_nxt, tp->rcv_nxt);
		if (flowclen16 < 0)
			BUG_ON(1);
		...
		cplios_reset_flag(cplios, CPLIOS_TX_WAIT_IDLE);
		cplios_reset_flag(cplios, CPLIOS_MA_FAILOVER);
...
void ma_fail_active_open_rpl(struct sock *sk, struct sk_buff *skb)
{
	...
	if (cplios_flag(sk, CPLIOS_MA_FAILOVER)) {
		...
		cplios->ma_fail_info.idr = sk_insert_tid(t, sk, tid);
		free_atid(cplios, t, atid);
			       ...
			       const struct l2t_entry *e)
{
	if (cplios_flag(sk, CPLIOS_MA_FAILOVER)) {
		...
		req->tcb.opt0 = cpu_to_be64(calc_opt0(sk, 0) |
		...
		if (tcp_timestamps_enabled(sock_net(sk)))
			opt2 |= F_TSTAMPS_EN;

		if (tcp_sack_enabled(sock_net(sk)))
			opt2 |= F_SACK_EN;

		req->tcb.opt2 = htonl(opt2);
		req->cookie = (u64) atid;
		set_wr_txq(skb, CPL_PRIORITY_CONTROL, cplios->ma_fail_info.port_id);
		cxgb4_l2t_send(cplios->ma_fail_info.egress_dev, skb,
				cplios->ma_fail_info.l2t_e);
		return 1;
	}
	return 0;
...
void ma_fail_do_fw6_msg(struct sock *sk, struct sk_buff *skb)
{
	...
	if (cplios_flag(sk, CPLIOS_MA_FAILOVER)) {
		if (p->type == FW6_TYPE_OFLD_CONNECTION_WR_RPL) {
			...
			cplios->ma_fail_info.idr = sk_insert_tid(t, sk, htonl(req->tid));
			free_atid(cplios, t, atid);
...
{
	struct cpl_io_state *newcplios = (struct cpl_io_state *)data;
...
void ma_failover_cleanup(struct sock *sk, int closed)
{
	...
	if (closed) {
		...
		bh_remove_handle(TOM_DATA(cplios->ma_fail_info.toedev),
				 cplios->ma_fail_info.idr);
		...
		if (sk->sk_family != AF_INET) {
			cxgb4_clip_release(cplios->ma_fail_info.egress_dev,
				(const u32 *)((&inet6_sk(sk)->saddr)->s6_addr),
									1);
		}
	}
	...
	if (!atomic_read(&pi->ma_fail_data.conn_moved)) {
		...
		if (closed) {
			write_ofld_smt(cplios->egress_dev, 0x0, pi->vivld);
				     ...
				     const struct l2t_entry *e)
{
	...
	req6->local_ip_hi = *(__be64 *)((&sk->sk_v6_rcv_saddr)->s6_addr);
	req6->local_ip_lo = *(__be64 *)((&sk->sk_v6_rcv_saddr)->s6_addr + 8);
	req6->peer_ip_hi = *(__be64 *)((&sk->sk_v6_daddr)->s6_addr);
	req6->peer_ip_lo = *(__be64 *)((&sk->sk_v6_daddr)->s6_addr + 8);
	req6->opt0 = cpu_to_be64(calc_opt0(sk, 0) |
				 V_L2T_IDX(e->idx) |
				 V_SMAC_SEL(cplios->ma_fail_info.smac_idx) |
				 V_TX_CHAN(cplios->ma_fail_info.tx_c_chan) |
...
int t4_toe_ma_failover_idr(int id, void *p, void *data)
{
	struct sock *sk = p;
	struct port_info *pi = data;
	struct port_info *pinfo;

	struct net_device *slave_dev = pi->ma_fail_data.backup_dev;
	struct net_device *failed_dev = pi->ma_fail_data.this_dev;
	struct tcp_sock *tp;
	struct neighbour *neigh = NULL;
	...
	struct tom_data *t = TOM_DATA(TOEDEV(failed_dev));
	...
	cplios_set_flag(cplios, CPLIOS_MA_FAILOVER);
	cplios_set_flag(cplios, CPLIOS_TX_WAIT_IDLE);
	...
	if (sk->sk_family == AF_INET) {
		...
		rt = find_route(inet_sk(sk)->inet_saddr,
				inet_sk(sk)->inet_daddr,
				inet_sk(sk)->inet_sport,
				inet_sk(sk)->inet_dport,
				RT_CONN_FLAGS(sk));
		if (!rt) {
			pr_err("%s: No rt found.\n", __func__);
			goto unlock;
		}
	...
	id = idr_alloc(&d->aidr, sk, atid, atid+1, GFP_NOWAIT);
	spin_unlock(&d->aidr_lock);
#else
	id = xa_insert(&d->aidr, atid, sk, GFP_NOWAIT);
#endif
	if (id < 0) {
#ifdef T4_TRACE
		T4_TRACE1(TIDTB(sk),
		   "Failed to allocate atid idr = %d\n", atid);
#endif
		goto free_atid;
	}
	...
	cplios->ma_fail_info.rx_c_chan = cxgb4_port_e2cchan(slave_dev);
	pinfo = (struct port_info *)netdev_priv(slave_dev);
	cplios->ma_fail_info.smac_idx = pinfo->smt_idx;
...
#endif
	cxgb4_l2t_send(cplios->ma_fail_info.egress_dev, skb, e);
			...
			unsigned int bond_req, void *data)
{
	...
	write_ofld_smt(failed_dev, 0x0, 0);
