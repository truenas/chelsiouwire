...
int t4_udp_release_resources(struct sock *sk)
{
...
#ifdef CONFIG_UDPV6_OFFLOAD
        if (sk->sk_family == AF_INET)
		smp_store_release(&sk->sk_prot, &udp_prot);
        else
		smp_store_release(&sk->sk_prot, udpv6_prot_p);
#else
	smp_store_release(&sk->sk_prot, &udp_prot);
#endif
...
#define FLOWC_PARAM(__m, __v) \
	...
	FLOWC_PARAM(IQID, cplios->rss_qid);
	FLOWC_PARAM(EOSTATE, state);
...
int t4_udp_offload_init(struct toedev *tdev, struct sock *sk,
			struct net_device *egress_dev, int ipver)
{
				  ...
				  int credits)
{
	...
	u64 cntrl;
	struct adapter *adap = netdev2adap(cplios->toedev->lldev[0]);
...
out:
	if (!imm)
		if (refcount_sub_and_test(total_size, &sk->sk_wmem_alloc))
			;
		   ...
		   unsigned int flags)
{
	...
	*rtp = NULL;
	...
	inet->cork.base.tx_flags = 0;
	sock_tx_timestamp(sk, ipc->sockc.tsflags, &inet->cork.base.tx_flags);
...
int t4_udp_push_frames(struct sock *sk)
{
	...
	if (err) {
		if (err == -ENOBUFS && !inet->recverr) {
			T4_UDP_INC_STATS_USER(sock_net(sk),
...
void chelsio_udp_flush_pending_frames(struct cpl_io_state *cplios)
{
	...
	if (up->pending) {
		...
		skb_queue_purge(&cplios->tx_queue);
...
int udpoffload_sendmsg(struct sock *sk, struct msghdr *msg, size_t len)
{
	...
	if (sk->sk_prot->sendmsg != udpoffload_sendmsg)
	...
	if (rt == NULL) {
		...
		if (IS_ERR(rt)) {
			...
			rt = NULL;
			if (err == -ENETUNREACH)
				T4_IP_INC_STATS_BH(net, IPSTATS_MIB_OUTNOROUTES);
			goto out;
		}
	...
	if (unlikely(!cplios_flag(sk, CPLIOS_TX_DATA_SENT))) {
		...
		err = t4_udp_offload_init(tdev, sk, rdev, 4);
		if (err)
			goto fallback;
...
do_append_data:
	...
	if (err)
		chelsio_udp_flush_pending_frames(cplios);
	else if (!corkreq)
...
fallback:
	...
	/* Tunnel the connection back to kernel stack */
	if (cplios->sk_gso_type)
		sk->sk_gso_type = cplios->sk_gso_type;
	else
		sk->sk_gso_type -= hdr_len;
	...
	const struct sockcm_cookie *sockc)
{
	...
	np->cork.hop_limit = ipc6->hlimit;
	np->cork.tclass = ipc6->tclass;
	...
	cork->length = 0;
	cork->tx_flags = 0;
	sock_tx_timestamp(sk, ipc6->sockc.tsflags, &cork->tx_flags);
	sk_sendmessage_page(sk) = NULL;
	sk->sk_frag.offset = 0;
...
static int chelsio_ip6_push_pending_frames(struct sock *sk, struct rt6_info *rt)
{
	struct in6_addr final_dst_buf, *final_dst = &final_dst_buf;
	int credits, total_size = 0, imm = 0, err = 0, len;
	struct cpl_io_state *cplios = CPL_IO_STATE(sk);
	struct inet_sock *inet = inet_sk(sk);
	struct ipv6_pinfo *np = inet6_sk(sk);
	struct net *net = sock_net(sk);
	...
	if (!rt)
		rt = (struct rt6_info *)inet->cork.base.dst;
	...
	while (cplios->wr_credits &&
		...
		*final_dst = fl6->daddr;
		...
		if (opt && opt->opt_nflen)
			ipv6_push_nfrag_opts_p(skb, opt, &proto,
					       &final_dst, &fl6->saddr);
		...
		if (proto == IPPROTO_ICMPV6) {
			...
			T4_ICMP6MSGOUT_INC_STATS_BH(net, idev,
						 icmp6_hdr(skb)->icmp6_type);
			T4_ICMP6_INC_STATS_BH(net, idev, ICMP6_MIB_OUTMSGS);
		}
		 ...
		 * solicitation (modelled on cxgb4_l2t_send()), toss the skb
		 * and bail out.
		 */
		if (!((n->nud_state & NUD_VALID) ||
		      (n->nud_state & (NUD_CONNECTED|NUD_DELAY|NUD_PROBE)))) {
...
int chelsio_udp_v6_push_pending_frames(struct sock *sk, struct rt6_info *rt)
{
	struct cpl_io_state *cplios = CPL_IO_STATE(sk);
	int is_udplite = IS_UDPLITE(sk), err = 0;
	struct inet_sock *inet = inet_sk(sk);
	struct udp_sock  *up = udp_sk(sk);
	struct sk_buff *skb;
	struct flowi6 *fl6;
...
send:
	err = chelsio_ip6_push_pending_frames(sk, rt);
	if (err) {
		if (err == -ENOBUFS && !inet6_sk(sk)->recverr) {
			T4_UDP6_INC_STATS_USER(sock_net(sk),
					    UDP_MIB_SNDBUFERRORS, is_udplite);
			err = 0;
		}
	} else
 ...
 */
static void chelsio_udp_v6_flush_pending_frames(struct cpl_io_state *cplios)
{
	struct sock *sk = cplios->sk;
	struct udp_sock *up = udp_sk(sk);
...
int udpv6offload_sendmsg(struct sock *sk, struct msghdr *msg, size_t len)
{
	...
	if (sk->sk_prot->sendmsg != udpv6offload_sendmsg)
		return sk->sk_prot->sendmsg(sk, msg, len);
...
do_udp_sendmsg:
	...
	if (ipv6_addr_any(&fl6.saddr) && !ipv6_addr_any(&inet6_sk(sk)->saddr))
	...
	dst = ip6_sk_dst_lookup_flow_compat(sk, &fl6, final_p, true, connected);
	if (IS_ERR(dst)) {
		err = PTR_ERR(dst);
		dst = NULL;
		goto out;
	...
	if (ipc6.hlimit < 0) {
		if (ipv6_addr_is_multicast(&fl6.daddr))
			ipc6.hlimit = np->mcast_hops;
		else
			ipc6.hlimit = np->hop_limit;
		if (ipc6.hlimit < 0)
			ipc6.hlimit = ip6_dst_hoplimit(dst);
	}
	...
	if (unlikely(!cplios_flag(sk, CPLIOS_TX_DATA_SENT))) {
		...
		err = t4_udp_offload_init(tdev, sk, rdev, 6);
		if (err)
			goto fallback;
	} else if (unlikely(TOEDEV(rdev) != cplios->toedev)) {
		err = -ENXIO;
...
do_append_data:
	...
	err = chelsio_ip6_append_data(sk, getfrag, msg, ulen,
				      ...
				      msg->msg_flags, &sockc);
	if (err)
		chelsio_udp_v6_flush_pending_frames(cplios);
	else if (!corkreq)
		err = chelsio_udp_v6_push_pending_frames(sk, (struct rt6_info *)dst);
	else if (unlikely(skb_queue_empty(&cplios->tx_queue)))
		up->pending = 0;
	...
	if (dst) {
		if (connected) {
			ip6_dst_store(sk, dst,
				      ...
				      ipv6_addr_equal(&fl6.saddr, &inet6_sk(sk)->saddr) ?
				      &inet6_sk(sk)->saddr :
#endif
				      NULL);
		} else {
...
void udpv6offload_destroy(struct sock *sk)
{
	...
	if (sock_flag(sk, SOCK_OFFLOADED)) {
		...
		if (cplios->tid) {
			...
			BUG_ON(err < 0);
			cplios_set_flag(cplios, CPLIOS_CLOSE_CON_REQUESTED);
			sock_hold(sk);
		} else
			t4_udp_release_resources(sk);
	}
...
static inline int chelsio_uo_init(struct sock *sk, struct proto *udp_proto)
{
	...
	if (!sock_flag(sk, SOCK_OFFLOADED)) {
		...
		oproto = kzalloc(sizeof(*oproto), GFP_KERNEL);
		if (oproto == NULL)
			goto out_err;

		kref_init(&cplios->kref);
		cplios->sk = sk;
		oproto->proto = *udp_proto;
		sk_ofld_proto_set_tomhandlers(oproto, udp_proto);
				...
				sockptr_t optval, unsigned int optlen)
{
	...
	if (optname == UDP_SCHEDCLASS || optname == UDP_FRAMESIZE ||
		...
		if (optname == UDP_FRAMESIZE) {
			cplios->sk_gso_type = sk->sk_gso_type;
			sk->sk_gso_type = val;
			return 0;
		}
