...
/* DDP CPL_SET_TCB_FIELD cookies */
	DDP_COOKIE_OFFSET = 5,
	DDP_COOKIE_INDOUT = 6,
/* ENABLE cookie for syncing with clearing ddp_off field */
	DDP_COOKIE_ENABLE = 7,
	DDP_COOKIE_MAXNUM = 8,

/* Congestion control CPL_GET_TCB cookie for syncing
 * between sw and hw tcp sock field in case of any
 * updation in hw congestion control field
 */
...
struct ma_failover_info {
	...
	unsigned int idr;		/* atid IDR alloc token */
...
struct cpl_io_state {
	...
	unsigned int wr_nondata;	/* number of used non-data WR credits */
	...
	u8 nagle;			/* COP override nagle setting */
	u8 port_id;			/* egress netdev port id */
	u32 port_speed;			/* egress netdev link_cfg.speed */
	struct toedev *toedev;		/* TOE device */
	...
	struct sk_buff_head ooo_queue;  /* queue of WR skbs not sent to HW */
...
#define CPL_IO_STATE(sk) \
	(*(struct cpl_io_state * const *)sk_ofld_proto_get_ptr_addr(sk))
#define DDP_STATE(sk)	 (&(CPL_IO_STATE(sk)->ddp_state))
#define TLS_IO_STATE(sk)	(&(CPL_IO_STATE(sk)->tls_ofld))
...
static inline void cplios_set_flag(struct cpl_io_state *cplios,
				   enum cplios_flags flag)
{
	__set_bit(flag, &cplios->flags);
}

static inline void cplios_reset_flag(struct cpl_io_state *cplios,
				     enum cplios_flags flag)
{
	__clear_bit(flag, &cplios->flags);
}

static inline int cplios_flag(const struct sock *sk, enum cplios_flags flag)
{
...
static inline int cplios_flag_nochk(const struct cpl_io_state *cplios,
				    enum cplios_flags flag)
{
	return test_bit(flag, &cplios->flags);
}

void t4_cplios_release(struct kref *ref);
static inline void cplios_oproto_put(struct sock *sk,
...
static inline void reset_wr_list(struct cpl_io_state *cplios)
{
...
enum {
	...
	ULPCB_FLAG_NO_HDR = 1 << 13, /* packet isn't a OFLD_TX_WR */
	ULPCB_FLAG_TLS_HDR = 1 << 14, /* packet contain TLS Hdr */
};
