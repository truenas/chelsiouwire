...
static void cxgb4_dcb_cleanup_apps(struct net_device *dev)
{
	...
	for (i = 0; i < CXGB4_MAX_DCBX_APP_SUPPORTED; i++) {
		...
		if (dcb->dcb_version == FW_PORT_DCB_VER_IEEE) {
			app.priority = (ffs(dcb->app_priority[i].user_prio_map) 
					- 1);
		...
		dcb->app_priority[i].protocolid = 0;
			 ...
			 enum cxgb4_dcb_state_input input)
{
	...
	netdev_dbg(dev, "%s: State change from %d to %d for %s\n",
		    __func__, dcb->state, input, dev->name);
	...
	case CXGB4_DCB_STATE_START: {
		...
		case CXGB4_DCB_INPUT_FW_INCOMPLETE: {
			...
			/* if version is IEEE or AUTO, supported will be 
 			 * set as DCB_CAP_DCBX_VER_IEEE, else 
	...
	case CXGB4_DCB_STATE_FW_INCOMPLETE: {
		if (input != CXGB4_DCB_INPUT_FW_DISABLED) {
				...
				const struct fw_port_cmd *pcmd)
{
	...
	int port = G_FW_PORT_CMD_PORTID(be32_to_cpu(pcmd->op_to_portid));
				...
				u8 up_tc_map)
{
	...
	INIT_PORT_DCB_WRITE_CMD(pcmd, pi->port_id);
	if (pi->dcb.state == CXGB4_DCB_STATE_HOST)
		pcmd.op_to_portid |= cpu_to_be32(F_FW_PORT_CMD_APPLY);
...
u8 cxgb4_getpfcstate(struct net_device *dev)
{
	struct port_info *pi = netdev2pinfo(dev);
