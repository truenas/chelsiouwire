...
#define	PAUSE_RX	(1 << 0)	/* RX Pause Frame processing on */
#define	PAUSE_TX	(1 << 1)	/* TX Pause Frame generation on */
#define	PAUSE_AUTONEG	(1 << 2)	/* Pause Frame autonegotiation on */
typedef unsigned char cc_pause_t;

#define	FEC_AUTO	(1 << 0)	/* IEEE 802.3 "automatic" */
#define	FEC_RS		(1 << 1)	/* Reed-Solomon */
#define	FEC_BASER_RS	(1 << 2)	/* BaseR/Reed-Solomon */
#define	FEC_FORCE	(1 << 3)	/* Forcefully set FEC*/
...
struct link_config {
	...
	/*
	 * Host OS Drivers which have been converted to use the new 32-bit
	 * Port Capabilities should use the "pcaps", "acaps", and "lpacaps"
	 * fields and define the CPP symbol T4_OS_NEW_FW_CAPS32.  Thos Host
	 * OS Drivers which haven't been converted will continue to use the
	 * old "supported", "advertising", and "lp_advertising" fields and
	 * the Common Code will make sure that these are kept synchronized.
	 */
	fw_port_cap16_t supported;	/* link capabilities */
	fw_port_cap16_t advertising;	/* advertised capabilities */
	fw_port_cap16_t lp_advertising;	/* peer advertised capabilities */
	fw_port_cap16_t requested_speed;/* speed(s) user has requested */
#endif
	...
	cc_pause_t	advertised_fc;	/* actual advertised flow control */

	cc_fec_t	requested_fec;	/* Forward Error Correction: */
	cc_fec_t	fec;		/*   requested and actual in use */
	cc_fec_t	admin_fec;	/* Admin requested FEC in use */
...
static inline bool is_x_10g_port(const struct link_config *lc)
{
	...
	high_speeds = speeds & ~(FW_PORT_CAP32_SPEED_100M|FW_PORT_CAP32_SPEED_1G);
