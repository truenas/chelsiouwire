...
#if IS_ENABLED(CONFIG_VXLAN)
		if ((q->lb_queue_type & LB_VXLAN_TYPE) && d->page) {
			...
			for (i = 0; i < d->coalesce.idx; i++) {
				if (need_unmap)
					unmap_sgl(dev, d->coalesce.skb[i],
						  d->coalesce.sgl[i], q);
#if IS_ENABLED(CONFIG_VXLAN)
				if ((q->lb_queue_type & LB_VXLAN_TYPE) &&
				    d->coalesce.page[i]) {
	...
	if (q->pend_cred >= 8) {
		...
		if (unlikely(q->bar2_addr == NULL))
			writel(val | V_QID(q->cntxt_id),
			       adap->regs + MYPF_REG(A_SGE_PF_KDOORBELL));
		else
 ...
 */
netdev_tx_t cxgb4_eth_xmit(struct sk_buff *skb, struct net_device *dev)
{
	u32 wr_mid, ctrl0, op;
...
out_free:	dev_kfree_skb_any(skb);
	...
	if (xfrm_offload(skb) && cxgb4_ulds[CXGB4_ULD_CRYPTO].tx_handler)
		return cxgb4_ulds[CXGB4_ULD_CRYPTO].tx_handler(skb, dev);
#endif
	qidx = skb_get_queue_mapping(skb);
 ...
 *
 */
int cxgb4_vf_eth_xmit(struct sk_buff *skb, struct net_device *dev)
{
	u32 wr_mid;
	u64 cntrl, *end;
 ...
 *      @skb: the packet
 *
 *      Sends an offload packet.  This is an exported version of
 *      @t4_ofld_send,
 *      intended for ULDs.
 */
...
/**
 *	cxgb4_immdata_send - send an immediate mode WR on ofld data queue
 *	@dev: the net device
 *	@idx: the ofld TX queue index
 *	@src: location of work request to copy
 *	@len: WR length
 */
		       ...
		       const void *src, unsigned int len)
{
	...
	local_bh_disable();
	ret = ofld_xmit_direct(&adap->sge.ofldtxq[idx], src, len);
	local_bh_enable();
	return net_xmit_eval(ret);
}
		     ...
		     rspq_flush_handler_t flush_hnd, int cong, bool mirror)
{
	...
	c.type_to_iqandstindex = htonl(V_FW_IQ_CMD_TYPE(FW_IQ_TYPE_FL_INT_CAP) |
		...
		V_FW_IQ_CMD_IQANDSTINDEX(intr_idx >= 0 ? intr_idx :
							-intr_idx - 1));
	c.iqdroprss_to_iqesize = htons(V_FW_IQ_CMD_IQPCIECH(pi->rx_chan) |
			  ...
			  struct net_device *dev, unsigned int iqid)
{
	...
	memset(&c, 0, sizeof(c));
	if (unlikely(txq->q.lb_queue_type & LB_CRYPTO_TYPE))
		cmd = FW_EQ_CTRL_CMD;
	else
		cmd = FW_EQ_OFLD_CMD;
...
void t4_free_sge_resources(struct adapter *adap)
{
	...
	t4_free_ofld_rxqs(adap, adap->sge.nrxcryptoq, adap->sge.cryptorxq);
