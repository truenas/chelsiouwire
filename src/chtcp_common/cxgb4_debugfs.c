...
static int sge_qinfo_show(struct seq_file *seq, void *v)
{
	...
	int eth_entries = DIV_ROUND_UP(s->ethqsets, 4);
	int toe_entries = DIV_ROUND_UP(s->ofldqsets, 4);
	int rdma_entries = DIV_ROUND_UP(s->rdmaqs, 4);
	int ciq_entries = DIV_ROUND_UP(s->rdmaciqs, 4);
	int iscsi_entries = DIV_ROUND_UP(s->niscsiq, 4);
	int iscsit_entries = DIV_ROUND_UP(s->niscsitq, 4);
	int crypto_entries = DIV_ROUND_UP(s->nrxcryptoq, 4);
	int ctrl_entries = DIV_ROUND_UP(adap->tids.nsftids ? 1 : NCHAN, 4);
	int rdmactrl_entries = DIV_ROUND_UP(adap->tids.nsftids ? 0 : NCHAN, 4);
	int trace_entries = DIV_ROUND_UP(s->ntraceq, 4);
	int flover_entries = DIV_ROUND_UP(s->nfailoverq, 4);
	int vxlan_entries = DIV_ROUND_UP(s->nvxlanq, 4);
	int mirror_entries = DIV_ROUND_UP(s->nmirrorq, 4);
	...
	if (r < eth_entries) {
		...
		if (!is_hashfilter(adap)) {
			if ((r -= eth_entries) < trace_entries) {
				const struct sge_eth_rxq *rx = &s->traceq[r * 4];
				int n = min(4, s->ntraceq - 4 * r);
				...
				R("RspQ Gen:", rspq.gen);
				S3("u", "Intr delay:",
				   rspq_intr_timer(s, &rx[i].rspq));
				S3("u", "Intr pktcnt:",
				   rspq_intr_pktcnt(s, &rx[i].rspq));
				R("FL ID:", fl.cntxt_id);
				R("FL size:", fl.size - 8);
				R("FL pend:", fl.pend_cred);
				R("FL avail:", fl.avail);
				R("FL PIDX:", fl.pidx);
				R("FL CIDX:", fl.cidx);
			} else if ((r -= trace_entries) < toe_entries) {
				const struct sge_ofld_rxq *rx = &s->ofldrxq[r * 4];
				const struct sge_ofld_txq *tx = &s->ofldtxq[r * 4];
				int n = min(4, s->ofldqsets - 4 * r);
				...
				const struct sge_ofld_rxq *rx = &s->rdmarxq[r * 4];
				int n = min(4, s->rdmaqs - 4 * r);
				...
				const struct sge_ofld_rxq *rx = &s->rdmaciq[r * 4];
				int n = min(4, s->rdmaciqs - 4 * r);
				...
				const struct sge_ofld_rxq *rx = &s->iscsirxq[r * 4];
				int n = min(4, s->niscsiq - 4 * r);
				...
				R("FL PIDX:", fl.pidx);
				R("FL CIDX:", fl.cidx);
			} else if ((r -= iscsi_entries) < iscsit_entries) {
				const struct sge_ofld_rxq *rx = &s->iscsitrxq[r * 4];
				int n = min(4, s->niscsitq - 4 * r);
				...
				R("FL PIDX:", fl.pidx);
				R("FL CIDX:", fl.cidx);
			} else if ((r -= iscsit_entries) < crypto_entries) {
				const struct sge_ofld_rxq *rx =
					&s->cryptorxq[r * 4];
				const struct sge_ofld_txq *tx =
					&s->cryptotxq[r * 4];
				int n = min(4, s->nrxcryptoq - 4 * r);
				...
				R("FL size:", fl.size - 8);
				R("FL avail:", fl.avail);
			} else if ((r -= crypto_entries) < flover_entries) {
#ifdef CONFIG_T4_MA_FAILOVER
				const struct sge_ofld_rxq *rx = &s->failoverq;
				int n = min(4, s->nfailoverq - 4 * r);
				...
				T("TxQ CIDX:", q.cidx);
				T("TxQ PIDX:", q.pidx);
			} else if ((r -= ctrl_entries) < rdmactrl_entries) {
				const struct sge_ctrl_txq *tx =
					&s->ctrlq[NCHAN + r * 4];
				int n = min(4, adap->params.nports - 4 * r);

				S("QType:", "RDMA Control");
				T("TxQ ID:", q.cntxt_id);
				T("TxQ size:", q.size);
				T("TxQ inuse:", q.in_use);
				T("TxQ CIDX:", q.cidx);
				T("TxQ PIDX:", q.pidx);
			} else if ((r -= rdmactrl_entries) == 0) {
				...
				seq_printf(seq, "%-12s %16u\n", "RspQ Gen:", evtq->gen);
				seq_printf(seq, "%-12s %16u\n", "Intr delay:",
					   rspq_intr_timer(s, evtq));
				seq_printf(seq, "%-12s %16u\n", "Intr pktcnt:",
					   rspq_intr_pktcnt(s, evtq));
			}
		} else {
			if ((r -= eth_entries) < trace_entries) {
				const struct sge_eth_rxq *rx = &s->traceq[r * 4];
				...
				R("FL PIDX:", fl.pidx);
				R("FL CIDX:", fl.cidx);
			} else if ((r -= trace_entries) < mirror_entries) {
				const struct sge_eth_rxq *rx = &s->mirrorq[r * 4];
				int n = min(4, s->nmirrorq - 4 * r);
				...
				R("FL PIDX:", fl.pidx);
				R("FL CIDX:", fl.cidx);
			} else if ((r -= mirror_entries) < vxlan_entries) {
#if IS_ENABLED(CONFIG_VXLAN)
				int base_qset = r * 4;
				const struct sge_eth_txq *tx;
				const struct sge_eth_rxq *rx;
...
int sge_queue_entries(const struct adapter *adap)
{
	return DIV_ROUND_UP(adap->sge.ethqsets, 4) +
		...
		DIV_ROUND_UP(adap->sge.nmirrorq, 4) +
		DIV_ROUND_UP(adap->sge.ofldqsets, 4) +
		DIV_ROUND_UP(adap->sge.rdmaqs, 4) +
		DIV_ROUND_UP(adap->sge.rdmaciqs, 4) +
		DIV_ROUND_UP(adap->sge.niscsiq, 4) +
		DIV_ROUND_UP(adap->sge.niscsitq, 4) +
		DIV_ROUND_UP(adap->sge.nrxcryptoq, 4) +
		DIV_ROUND_UP(adap->tids.nsftids ? 1 : NCHAN, 4) +
		DIV_ROUND_UP(adap->tids.nsftids ? 0 : NCHAN, 4) +
		DIV_ROUND_UP(adap->sge.nvxlanq, 4) +
			    ...
			    size_t count, loff_t *pos)
{
	...
	struct inode const * const ino = file_inode(file);
...
static int sge_stats_show(struct seq_file *seq, void *v)
{
	...
	int crypto_entries = DIV_ROUND_UP(adap->sge.nrxcryptoq, 4);
	int ctrl_entries = DIV_ROUND_UP(adap->tids.nsftids ? 1 : NCHAN, 4);
	int rdmactrl_entries = DIV_ROUND_UP(adap->tids.nsftids ? 0 : NCHAN, 4);
	int trace_entries = DIV_ROUND_UP(adap->sge.ntraceq, 4);
	int vxlan_entries = DIV_ROUND_UP(adap->sge.nvxlanq, 4);
	int mirror_entries = DIV_ROUND_UP(adap->sge.nmirrorq, 4);
	int i, r = (uintptr_t)v - 1;
	...
	if (r < eth_entries) {
		...
		if (!(is_hashfilter(adap) && is_t5(adap->params.chip))) {
			if ((r -= eth_entries) < trace_entries) {
				...
				const struct sge_ofld_rxq *qs = &adap->sge.iscsitrxq[r * 4];
				int n = min(4, adap->sge.niscsitq - 4 * r);
				...
				const struct sge_ofld_rxq *qs =
					&adap->sge.cryptorxq[r * 4];
				const struct sge_ofld_txq *tx =
					&adap->sge.cryptotxq[r * 4];
				int n = min(4, adap->sge.nrxcryptoq - 4 * r);
				...
				R("FLAllocErr:", fl.alloc_failed);
				R("FLstarving:", fl.starving);
			} else if ((r -= crypto_entries) < ctrl_entries) {
				const struct sge_ctrl_txq *tx =
					&adap->sge.ctrlq[r * 4];
				int n = min(4, adap->params.nports - 4 * r);
				...
				T("TxQFull:", q.stops);
				T("TxQRestarts:", q.restarts);
			} else if ((r -= rdmactrl_entries) < vxlan_entries) {
#if IS_ENABLED(CONFIG_VXLAN)
				int base_qset = r * 4;
				const struct sge_eth_txq *tx;
				const struct sge_eth_rxq *rx;
			...
			if ((r -= eth_entries) < trace_entries) {
				...
				const struct sge_eth_rxq *qs = &adap->sge.mirrorq[r * 4];
				int n = min(4, adap->sge.nmirrorq - 4 * r);
				...
				R("FLLow:", fl.low);
				R("FLStarving:", fl.starving);
			} else if ((r -= mirror_entries) < ctrl_entries) {
				const struct sge_ctrl_txq *tx =
					&adap->sge.ctrlq[r * 4];
				int n = min(4, adap->params.nports - 4 * r);
...
static int chcr_show(struct seq_file *seq, void *v)
{
	...
	seq_printf(seq, "Completion: %u \n",
		atomic_read(&adap->chcr_stats.rqst_comp));
	seq_printf(seq, "Error: %u \n",
		atomic_read(&adap->chcr_stats.rsp_error));
	seq_printf(seq, "Fallback: %u \n",
		atomic_read(&adap->chcr_stats.fallback));
	seq_printf(seq, "IPSec Tx Inline: %u \n",
		atomic_read(&adap->chcr_stats.ipsec_cnt));

	return 0;
}

static int chcr_open(struct inode *inode, struct file *file)
{
...
static int tls_show(struct seq_file *seq, void *v)
{
	struct adapter *adap = seq->private;

	seq_puts(seq, "Chelsio Inline TLS Stats \n");
	seq_printf(seq, "TLS PDU Tx: %u \n",
		atomic_read(&adap->tls_stats.tls_pdu_tx)); 
	seq_printf(seq, "TLS PDU Rx: %u \n",
		atomic_read(&adap->tls_stats.tls_pdu_rx)); 
	seq_printf(seq, "TLS Keys (DDR) Count: %u \n",
		atomic_read(&adap->tls_stats.tls_key)); 

	return 0;
}

static int tls_open(struct inode *inode, struct file *file)
{
	return single_open(file, tls_show, inode->i_private);
}

static const struct file_operations tls_stats_debugfs_fops = {
	.owner   = THIS_MODULE,
	.open    = tls_open,
	.read    = seq_read,
	.llseek  = seq_lseek,
...
int cxgb4_setup_debugfs(struct adapter *adap)
{
	static struct t4_linux_debugfs_entry cxgb4_debugfs_files[] = {
...
#endif
		{ "resources", &resources_debugfs_fops, S_IRUSR, 0, 0 },
		...
		{ "crypto", &chcr_stats_debugfs_fops, S_IRUSR, 0, ADAP_NEED_CRYPTO },
		{ "tls", &tls_stats_debugfs_fops, S_IRUSR, 0, ADAP_NEED_CRYPTO },
		{ "cudbg_kcrash_flags", &cudbg_kcrash_flags_fops,
		  S_IRUSR | S_IWUSR, 0, 0 },
	};
