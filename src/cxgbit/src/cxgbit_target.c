		  ...
		  u32 len, u32 credits, u32 compl)
{
	...
	u32 immlen = 0;
	u32 force = is_t5(lldi->adapter_type) ? V_TX_FORCE(!submode) :
		    F_T6_TX_FORCE;
	...
	req->op_to_immdlen = cpu_to_be32(V_FW_WR_OP(opcode) |
					V_FW_WR_COMPL(compl) |
					V_FW_WR_IMMDLEN(immlen));
	req->flowid_len16 = cpu_to_be32(V_FW_WR_FLOWID(csk->tid) |
					V_FW_WR_LEN16(credits));
	req->plen = htonl(len);
	wr_ulp_mode = FW_OFLD_TX_DATA_WR_ULPMODE(ULP_MODE_ISCSI) |
...
static int cxgbit_set_iso_npdu(struct cxgbit_sock *csk)
{
	...
	max_iso_npdu = max_iso_payload / (ISCSI_HDR_LEN + mrdsl +
		       cxgbit_digest_len[csk->submode]);

	csk->max_iso_npdu = min3(max_npdu, max_iso_npdu, 40U);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4,2,0) || defined(RHEL_RELEASE_7_4))
...
#if ((LINUX_VERSION_CODE >= KERNEL_VERSION(5,1,0)) || defined(RHEL_RELEASE_GTE_8_1))
