...
{
	csk->emss = csk->com.cdev->lldi.mtus[G_TCPOPT_MSS(opt)] -
...
static void
cxgbit_get_tuple_info(struct cpl_pass_accept_req *req,
		      enum chip_type adapter_type, int *iptype,
		   ...
		   __u32 sin6_scope_id)
{
	...
	if (IS_ENABLED(CONFIG_IPV6)) {
		...
		if (dst->error ||
		    (!cxgbit_our_interface(cdev, ip6_dst_idev(dst)->dev) &&
...
#define CXGBIT_10G_SND_WIN (128 * 1024)
...
static void
cxgbit_best_mtu(struct cxgbit_sock *csk, unsigned int *idx, int use_ts)
{
	const struct cxgbit_device *cdev = csk->com.cdev;
	const struct cxgb4_lld_info *lldi = &cdev->lldi;
	const unsigned short *mtus = csk->com.cdev->lldi.mtus;
	bool ipv6 = csk->com.remote_addr.ss_family == AF_INET;
	unsigned short hdr_size = (ipv6 ? sizeof(struct ipv6hdr) :
				   sizeof(struct iphdr)) +
				   sizeof(struct tcphdr) +
				   (use_ts ? round_up(TCPOLEN_TIMESTAMP,
				    4) : 0);
	unsigned short data_size = csk->mtu - hdr_size;
...
int cxgbit_rx_data_ack(struct cxgbit_sock *csk)
{
	...
	OPCODE_TID(req) = cpu_to_be32(MK_OPCODE_TID(CPL_RX_DATA_ACK,
						    csk->tid));
	req->credit_dack = cpu_to_be32(F_RX_DACK_CHANGE | V_RX_DACK_MODE(1) |
				       V_RX_CREDITS(csk->rx_credits));
...
cxgbit_pass_accept_rpl(struct cxgbit_sock *csk, struct cpl_pass_accept_req *req)
{
	...
	OPCODE_TID(rpl5) = cpu_to_be32(MK_OPCODE_TID(CPL_PASS_ACCEPT_RPL,
						     csk->tid));
	cxgbit_best_mtu(csk, &mtu_idx, req->tcpopt.tstamp);
	...
	opt0 =  F_TCAM_BYPASS |
		V_WND_SCALE(wscale) |
		V_MSS_IDX(mtu_idx) |
		V_L2T_IDX(csk->l2t->idx) |
		V_TX_CHAN(csk->tx_chan) |
		V_SMAC_SEL(csk->smac_idx) |
		V_DSCP(csk->tos >> 2) |
		V_ULP_MODE(ULP_MODE_ISCSI) |
		V_RCV_BUFSIZ(win);

	opt2 = V_RX_CHANNEL(csk->rx_chan) |
		F_RSS_QUEUE_VALID | V_RSS_QUEUE(csk->rss_qid);
...
cxgbit_pass_accept_req(struct cxgbit_device *cdev, struct sk_buff *skb)
{
	...
	cxgbit_get_tuple_info(req, cdev->lldi.adapter_type, &iptype, local_ip,
	...
	if (iptype == 4)  {
		...
		dst = cxgbit_find_route(cdev, *(__be32 *)local_ip,
					*(__be32 *)peer_ip,
					...
					G_PASS_OPEN_TOS(ntohl(req->tos_stid)));
	} else {
	...
	csk->com.cdev = cdev;
	csk->cnp = cnp;
	csk->tos = G_PASS_OPEN_TOS(ntohl(req->tos_stid));
	csk->dst = dst;
	csk->tid = tid;
	...
	if (iptype == 4)
		cxgb4_insert_tid(t, csk, tid,
				 ((struct sockaddr_in *)&csk->com.local_addr)->sin_family);
	else
		cxgb4_insert_tid(t, csk, tid,
				 ((struct sockaddr_in6 *)&csk->com.local_addr)->sin6_family);
...
u32 cxgbit_send_tx_flowc_wr(struct cxgbit_sock *csk)
{
	...
	flowc->mnemval[0].val = cpu_to_be32(V_FW_PFVF_CMD_PFN
...
static int cxgbit_send_tcb_skb(struct cxgbit_sock *csk,
					struct sk_buff *skb) {
	spin_lock_bh(&csk->lock);
	if (unlikely(csk->com.state != CSK_STATE_ESTABLISHED)) {
		spin_unlock_bh(&csk->lock);
		pr_err("%s: connection csk %p[%u] is already destroyed.\n",
		       __func__, csk, csk->tid);
		__kfree_skb(skb);
		return -1;
	}
...
cxgbit_pass_establish(struct cxgbit_device *cdev, struct sk_buff *skb)
{
	...
	if (csk->rcv_win > (M_RCV_BUFSIZ << 10))
		csk->rx_credits = (csk->rcv_win - (M_RCV_BUFSIZ << 10));
...
static void cxgbit_abort_req_rss(struct cxgbit_sock *csk, struct sk_buff *skb)
{
	...
	set_wr_txq(rpl_skb, CPL_PRIORITY_DATA, csk->txq_idx);
