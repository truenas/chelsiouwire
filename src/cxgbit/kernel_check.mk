...
# Check that kernel is enabled with iscsi target.
ifneq ($(shell $(grep) -c '^\#define[[:space:]]\+CONFIG_ISCSI_\(TARGET\|TARGET_MODULE\)[[:space:]]\+1' $(AUTOCONF_H)),1)
  $(warning The kernel has not been configured with iscsi target support.)
  $(warning Try configuring the kernel enabling ISCSI_TARGET using <make menuconfig> and \
            recompile.)
  $(error ERROR: kernel ISCSI_TARGET not enabled)
endif
