...
int ssl3_get_record(SSL *s)
{
     ...
     */
    if (SSL_READ_ETM(s) && s->read_hash && (BIO_get_offload_rx(s->rbio) <= 0)) {
        unsigned char *mac;
    ...
    first_rec_len = rr[0].length;
     /*
      * With this version of OpenSSL Applications can implement BIOs methods
      * and returns -1, 0 for failure cases.
      */
    if (BIO_get_offload_rx(s->rbio) <= 0) {
    ...
    if (enc_err < 0) {
        ...
        if (num_recs == 1 && ossl_statem_skip_early_data(s)) {
            ...
            if ((BIO_get_offload_rx(s->rbio) > 0) && (*p == 0x7F)) {
