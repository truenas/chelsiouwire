...
#%define openssldir /var/chcrssl
%define openssldir /usr/chssl/openssl 
%define LIBRPATH %{srcdir}/.openssl/lib/
...
#%define CONFIG_FLAGS -DSSL_ALLOW_ADH --prefix=/usr/chssl --openssldir=%{openssldir}
%define CONFIG_FLAGS -Wl,--enable-new-dtags,-rpath,'%{LIBRPATH}' --prefix=/usr/chssl --openssldir=%{openssldir}
...
%endif
%ifarch x86_64
./config %{CONFIG_FLAGS} shared enable-engine enable-dso enable-afalgeng
%endif
%ifarch aarch64
./config %{CONFIG_FLAGS} shared enable-engine enable-dso enable-afalgeng
%endif
...
#make MANDIR=/usr/man MANSUFFIX=chssl INSTALL_PREFIX="$RPM_BUILD_ROOT" install
if [[ %{rhel_flag} -eq 1 ]] ; then
 make DESTDIR=$RPM_BUILD_ROOT install
fi
if [[ %{sles_flag} -eq 1 ]] ; then
 %makeinstall
fi
#echo "install end"
#install -D -v -m 644 apps/openssl-tls.cnf $RPM_BUILD_ROOT%{openssldir}
...
%attr(0755,root,root) /usr/chssl/bin/*
%attr(0755,root,root) /usr/chssl/lib/*.so*
%attr(0755,root,root) /usr/chssl/lib/engines-1.1/*.so* 
%attr(0755,root,root) %{openssldir}/misc/*
#%attr(0644,root,root) /usr/man/man[157]/*

%config %attr(0644,root,root) %{openssldir}/openssl.cnf 
#%config %attr(0644,root,root) %{openssldir}/openssl-tls.cnf 
...
%attr(0644,root,root) /usr/chssl/lib/*.a
%attr(0644,root,root) /usr/chssl/lib/pkgconfig/openssl.pc
%attr(0644,root,root) /usr/chssl/include/openssl/*
