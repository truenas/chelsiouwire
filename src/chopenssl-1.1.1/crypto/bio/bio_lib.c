...
/*
 * These functions are added to support inline tls offload in httpd/mod_ssl.
 */
#ifdef CHELSIO_TLS_OFFLOAD
...
int BIO_set_key_context(BIO *a, void *ptr)
{
    int ret = -1;
    struct tls_key_context *key_context = (struct tls_key_context *)ptr;
    ret = ioctl(a->num, IOCTL_TLSOM_SET_TLS_CONTEXT, key_context);
    return ret;
}
#endif
