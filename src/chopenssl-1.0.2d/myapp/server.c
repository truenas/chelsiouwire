 ...
 * cc -g -ldl -I..  -I../include/ ssl_server.c ../libssl.a ../libcrypto.a -o ssl_server
 *
...
void Servlet(SSL* ssl, char *datafile)	/* Serve the connection */
{   char buf[BUFFSIZZ];
    char reply[BUFFSIZZ];
    int sd;
	...
	{
		len = copy_to_buff(datafile,buf, copied, readtill);
		written_bytes = SSL_write(ssl,buf,len);
		if (written_bytes < 0) {
			printf("wb:%d\n",written_bytes);
			break;
		}
	//	printf("len:%d wb:%d copied:%d\n",len,written_bytes, copied);
		copied += len;
		remaining = filesize - copied;
		if (remaining < readtill)
			readtill = remaining;
	}
    }
...
{   SSL_CTX *ctx;
    ...
    //SSL_CTX_set_cipher_list(ctx, "AES256-SHA");	/* keep changing this */
    ...
    {   struct sockaddr_in addr;
        ...
        //printf("Connection: %s:%d accept gave fd:%d\n",
        //	inet_ntoa(addr.sin_addr), ntohs(addr.sin_port), client);
	if (fork() == 0) {
	        ssl = SSL_new(ctx);         				
        	SSL_set_fd(ssl, client);			
	        Servlet(ssl, datafile);				
		exit(0);
	} 
	close(client);
    }
