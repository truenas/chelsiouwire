...
static inline void __cxgbi_sock_put(const char *fn, struct cxgbi_sock *csk)
{
	log_debug(1 << CXGBI_DBG_SOCK,
		...
		fn, csk,
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,11,0)
		refcount_read(&csk->refcnt.refcount));
#else
		atomic_read(&csk->refcnt.refcount));
#endif
					  ...
					  struct sk_buff *skb)
{
	 ...
	 */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,13,0) || defined(SLES_RELEASE_12_4) \
	|| defined(SLES_RELEASE_15_0) || defined(SLES_RELEASE_15_1)
...
#define CXGBI_FLAG_DEV_LRO_OFF		0x200
#define CXGBI_FLAG_DEV_ISO_OFF		0x400
#define CXGBI_FLAG_DEV_ALIGN8		0x800
#define CXGBI_FLAG_PDU_ALIGN512		0x1000
...
/* Earlier versions of openiscsi used a single lock for both Tx and
 * Rx (session->lock). Current code uses frwd_lock for the Tx path.
 */
#ifdef OISCSI_HAS_FRWD_LOCK
	#define SESS_TX_LOCK(session) session->frwd_lock
#else
	#define SESS_TX_LOCK(session) session->lock
#endif /* __OISCSI_HAS_FRWD_LOCK__ */

#ifdef OISCSI_ERR_TCP_CLOSE
	#define OISCSI_CONN_ERR ISCSI_ERR_TCP_CONN_CLOSE
#else
	#define OISCSI_CONN_ERR ISCSI_ERR_CONN_FAILED
#endif /* __OISCSI_ERR_TCP_CLOSE__ */
...
int cxgbi_ddp_ppm_setup(void **ppm_pp, struct cxgbi_device *,
			struct cxgbi_tag_format *, unsigned int iscsi_size,
			unsigned int llimit, unsigned int start,
			unsigned int rsvd_factor, unsigned int edram_start,
