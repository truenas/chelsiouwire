...
#define DRV_MODULE_VERSION	"3.14.0.3-1405"
					...
					int ifindex)
{
	...
	if (ip_route_output_flow(&init_net, &rt, &fl, NULL, ifindex))
		return NULL;
#else
...
static struct rt6_info *find_route_ipv6(const struct in6_addr *saddr,
			const struct in6_addr *daddr, int ifindex)
{
...
static struct cxgbi_sock *cxgbi_check_route6(struct sockaddr *dst_addr, int ifindex)
{
...
/*
 * If the session is not logged out gracefully before closing/
 * aborting the connection, then fail the connection
 */
static void cxgbi_inform_iscsi_conn_closing(struct cxgbi_sock *csk)
{
	...
	iscsi_conn_failure(conn, OISCSI_CONN_ERR);
	return;
}
...
static void need_active_close(struct cxgbi_sock *csk, struct cxgbi_conn *cconn)
{
	int data_lost;
	int close_req = 0;
			...
			unsigned int xferlen, struct cxgbi_pdu_pi_info *pi_info)
{
	...
	if (!ppm || xferlen < cdev->ddp_threshold || !sgcnt ||
...
static int task_reserve_itt(struct iscsi_task *task, itt_t *hdr_itt)
{
	...
	if (sc &&
...
#elif	defined(HAS_SCSI_BIDI_CMND)
	    (scsi_bidi_cmnd(sc) || sc->sc_data_direction == DMA_FROM_DEVICE)
#else
	    (sc->sc_data_direction == DMA_FROM_DEVICE)
#endif
		...
		scmd_get_params(sc, &ttinfo->sgl, &ttinfo->nents,
				&tdata->dlen, 0);
#ifdef __VARIABLE_DDP_PAGE_SIZE__
		if (ttinfo->nents && tdata->dlen >= cdev->ddp_threshold) {
...
skb_read_pdu_bhs(struct cxgbi_sock *csk, struct iscsi_conn *conn,
		 struct sk_buff *skb, unsigned int offset, unsigned long *flag)
{
	struct iscsi_tcp_conn *tcp_conn = conn->dd_data;
	int err, pi_inline = 0;
	...
	err = read_pdu_skb(conn, skb, offset, 0, pi_inline);
	if (likely(err >= 0)) {
		...
		if (cxgbi_skcb_test_flag(skb, SKCBF_RX_LRO)) {
			...
			skb_prepare_seq_read(skb, offset, offset + sizeof(struct iscsi_hdr), &seq);
			ret = skb_seq_read(0, &ptr, &seq);
			if (unlikely(ret < sizeof(struct iscsi_hdr))) {
				if (ret)
					skb_abort_seq_read(&seq);
				return err;
			}
			skb_abort_seq_read(&seq);
			hdr = (struct iscsi_hdr *)ptr;
		} else {
			hdr = (struct iscsi_hdr *)skb->data;
		}
...
void cxgbi_lro_skb_dump(struct sk_buff *skb)
{
	...
	for (i = 0; i < ssi->nr_frags; i++)
		pr_info("skb 0x%p, frag %d, off %u, sz %u.\n",
			...
			ssi->frags[i].page_offset,
#else
			skb_frag_off(&ssi->frags[i]),
#endif
...
static int
rx_skb_lro_read_pdu(struct cxgbi_sock *csk, struct sk_buff *skb,
		    struct iscsi_conn *conn, unsigned int *off_p, int idx)
{
	struct cxgbi_rx_pdu_cb *pdu_cb = cxgbi_skb_rx_pdu_cb(skb, idx);
	unsigned int offset = *off_p;
	int err = 0;

	err = skb_read_pdu_bhs(csk, conn, skb, offset, &pdu_cb->flags);
	if (err < 0) {
			...
			unsigned int *read)
{
	...
	for (i = 0; i < cnt; i++, pdu_cb++) {
		if (!cxgbi_rx_cb_test_flag(pdu_cb, SKCBF_RX_HDR) ||
			...
			err = rx_skb_lro_read_pdu(csk, skb, conn, &offset, i);
		}
		if (err < 0)
			goto done;
	}
				...
				unsigned int *read)
{
	...
	*read += cxgbi_skcb_rx_pdulen(skb);
	...
	err = skb_read_pdu_bhs(csk, conn, skb, 0, &cxgbi_skcb_flags(skb));
	if (err < 0) {
		...
		unsigned int *read)
{
	...
	*read += cxgbi_skcb_rx_pdulen(skb);
	...
	if (cdev->flags & CXGBI_FLAG_DEV_ALIGN8) {
		struct iscsi_hdr *hdr = (struct iscsi_hdr *)skb->data;
		u8 op = hdr->opcode & ISCSI_OPCODE_MASK;
		unsigned int dlen = ntoh24(hdr->dlength);

		/* drop the NOP-IN with no response needed,
		 * open-iscsi does not handle the ones with ping data anyway.
 		 * chiscsi target uses nop-in to adjust the tx seq to
 		 * be multiple of 8 after login phase is done
 		 */
		if ((op == ISCSI_OP_NOOP_IN) && hdr->itt == RESERVED_ITT &&
	    		hdr->ttt == RESERVED_ITT && dlen) {
			log_debug(1 << CXGBI_DBG_SOCK,
				"conn 0x%p, skb 0x%p, NOP_IN drop, dlen %u.\n",
				conn, skb, dlen);
...
int cxgbi_conn_alloc_pdu(struct iscsi_task *task, u8 op)
{
	...
	if (!(cxgbi_sock_flag(csk, CTPF_PEER_CHECKED)) &&
		...
		if (sess->targetname && strstr(sess->targetname, cht_idstr)) {
			int err = 0;
	 ...
	 * aligned.
 	 */
	if ((op != ISCSI_OP_SCSI_TMFUNC) && (op != ISCSI_OP_NOOP_OUT))
...
recalculate_sgl:
		...
		if (cdev->skb_iso_txhdr && (!csk->disable_iso) &&
			...
			{
				u32 max_iso_data =
					rounddown(CXGBI_MAX_ISO_DATA_IN_SKB, csk->advmss);
...
no_iso:
		...
		if (unlikely(err < 0)) {
			...
			if (dlimit) {
				...
				remaining_data_tosend = rounddown(dlimit,
	...
	if (task->sc) {
		...
		if (!task->hdr) {
			pr_info("FAILURE HDR Allocation for task %p conn %p\n",
				task, conn);
				...
				struct iscsi_stats *stats)
{
	...
	strcpy(stats->custom[3].desc, "dataout_iso");
	stats->custom[3].value = cconn->cep->csk->stats.dataout_iso;
	strcpy(stats->custom[4].desc, "lro_merged");
	stats->custom[4].value = cconn->cep->csk->stats.lro_merged;
	strcpy(stats->custom[5].desc, "ddp_pass");
	stats->custom[5].value = cconn->cep->csk->stats.ddp_pass;
	strcpy(stats->custom[6].desc, "ddp_fail");
	stats->custom[6].value = cconn->cep->csk->stats.ddp_fail;
	strcpy(stats->custom[7].desc, "conn_id");
	stats->custom[7].value = cconn->cep->csk->tid;
}
...
static int cxgbi_conn_max_xmit_dlength(struct iscsi_conn *conn)
{
	...
	if (cdev->flags & CXGBI_FLAG_PDU_ALIGN512)
...
static int cxgbi_conn_max_recv_dlength(struct iscsi_conn *conn)
{
	...
	struct cxgbi_conn *cconn = tcp_conn->dd_data;
	struct cxgbi_device *cdev = cconn->chba->cdev;
	unsigned int max = cdev->rx_max_size;
				...
				u64 transport_eph, int is_leading)
{
	...
	log_debug(1 << CXGBI_DBG_ISCSI,
		"csk 0x%p, session cmd max %u, bits %u.\n",
