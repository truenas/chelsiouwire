...
static inline bool cxgbi_vlan_uses_dev(struct net_device *ndev)
{

#if (defined(RHEL_RELEASE_6_X) || defined(SLES_RELEASE_11_4))
	struct vlan_group *grp;

#if defined(SLES_RELEASE_11_4)
	grp = rcu_dereference_rtnl(ndev->vlgrp);
#else
	grp = rcu_dereference_rtnl(netdev_extended(ndev)->vlgrp);
#endif	
	if (!grp)
		return false;
	return (grp->nr_vlans > 0);
#else
	return vlan_uses_dev(ndev);
#endif
}
