/*
 * cxgb4i.c: Chelsio T4-T6 iSCSI driver.
 *
...
MODULE_PARM_DESC(ppm_rsvd_factor, "iscsi ppm cpu reserve factor N "
				"(only for <= T5) : 1/N reserved "
				"for cpu pool (default 2)");
#ifndef CXGBI_T10DIF_SUPPORT
static unsigned int cxgb4i_ddp_threshold = 8192;
module_param(cxgb4i_ddp_threshold, uint, S_IRUGO);
MODULE_PARM_DESC(cxgb4i_ddp_threshold, "DDP threshold in bytes (default 8192)");
#endif
...
static struct scsi_host_template cxgb4i_host_template = {
	...
	/* dynamically adjusted based on the adapter type */
	.can_queue	= 0,
			...
			unsigned short mtu, unsigned int *idx, int ts)
{
	...
	unsigned short data_size_align = 8;
	...
	cxgb4_best_aligned_mtu(mtus, hdr_size, data_size, data_size_align, idx);
...
static bool is_memfree(struct adapter *adap)
{
	...
	io = t4_read_reg(adap, A_MA_TARGET_MEM_ENABLE);
	if (is_t5(adap->params.chip)) {
		if ((io & F_EXT_MEM0_ENABLE) || (io & F_EXT_MEM1_ENABLE))
			return false;
	} else if (io & F_EXT_MEM_ENABLE) {
		return false;
	}
				...
				struct l2t_entry *e)
{
	...
	opt2 =  F_WND_SCALE_EN | V_RX_CHANNEL(csk->rx_chan) |
				...
				struct l2t_entry *e)
{
	...
	default: {
		...
		opt2 |= F_T5_OPT_2_VALID | F_RX_FC_DISABLE;
		req->opt2 = cpu_to_be32(opt2);
...
static void do_act_establish(struct cxgbi_device *cdev, struct sk_buff *skb)
{
	...
	if (csk->advmss < 128)
		csk->advmss = 128;
	if (csk->advmss & 7)
		pr_debug("misaligned mtu idx %u mss %u\n",
...
static void do_rx_iscsi_hdr(struct cxgbi_device *cdev, struct sk_buff *skb)
{
	...
	if (!csk->skb_ulp_lhdr) {
		...
		if ((CHELSIO_CHIP_VERSION(lldi->adapter_type) <= CHELSIO_T5) &&
		    (cxgbi_skcb_tcp_seq(lskb) != csk->rcv_nxt)) {
...
static int init_act_open(struct cxgbi_sock *csk)
{
	...
	csk->rcv_win = min(csk->rcv_win, M_RCV_BUFSIZ << 10);
	csk->snd_win = min(csk->snd_win, 1U << 19);
				...
				int tid, int imm_write)
{
	...
	req->cmd = htonl(ULPTX_CMD(ULP_TX_MEM_WRITE) |
		...
		V_T5_ULP_MEMIO_ORDER(!is_t4(lldi->adapter_type) && !imm_write) |
		V_T5_ULP_MEMIO_IMM(!is_t4(lldi->adapter_type) && imm_write) |
		V_T5_ULP_MEMIO_FID((!is_t4(lldi->adapter_type)?lldi->rxq_ids[0]:0)));
				...
				int pg_idx)
{
	...
	INIT_TP_WR(req, csk->tid);
	OPCODE_TID(req) = htonl(MK_OPCODE_TID(CPL_SET_TCB_FIELD, csk->tid));
	req->reply_ctrl = htons(V_NO_REPLY(0) | V_QUEUENO(csk->rss_qid) |
				V_REPLY_CHAN(csk->rx_chan));
...
static int cxgb4i_ddp_init(struct cxgbi_device *cdev)
{
...
#ifdef CXGBI_T10DIF_SUPPORT
	cdev->ddp_threshold = DDP_THRESHOLD;
#else
	cdev->ddp_threshold = cxgb4i_ddp_threshold;
#endif
	...
	err = cxgbi_ddp_ppm_setup(lldi->iscsi_ppm, cdev, &tformat,
			    ...
			    lldi->vr->iscsi.start, ppm_rsvd_factor,
...
static void *t4_uld_add(const struct cxgb4_lld_info *lldi)
{
	...
	struct port_info *pi;
	int i, rc;
	int max_conn = CXGBI_MAX_CONN;
	int max_cmds = CXGB4I_SCSI_HOST_QDEPTH_MAX;
#ifdef CXGBI_T10DIF_SUPPORT
	unsigned int dif_dix = 0, guard = SHOST_DIX_GUARD_IP;
#endif
	struct adapter *adap;
	struct tid_info *t = NULL;
	...
	cdev->mtus = lldi->mtus;
	cdev->nmtus = NMTUS;
	cdev->rx_credit_thres = is_t6(lldi->adapter_type) ? 0 :
	...
	if (CHELSIO_CHIP_VERSION(lldi->adapter_type) <= CHELSIO_T5)
		cdev->flags |= CXGBI_FLAG_PDU_ALIGN512;
	...
	if (rc) {
		pr_info("%s, 0x%p ddp init failed %d.\n", ndev->name, cdev, rc);
		goto err_out;
	}
	...
	if (adap) {
		...
		/* check if SO adapters */
		if (is_memfree(adap)) {
			cdev->flags |= CXGBI_FLAG_DEV_LRO_OFF |
					CXGBI_FLAG_DEV_ISO_OFF |
					CXGBI_FLAG_DEV_ALIGN8;
			max_cmds = CXGB4I_SCSI_HOST_QDEPTH_MAX >> 2;
			pr_info("%s: 0x%p, tid %u, SO adapter.\n",
				ndev->name, cdev, t->ntids);
		}
		cdev->pfvf = V_FW_PFVF_CMD_PFN(adap->pf);
		log_debug(1 << CXGBI_DBG_DEV,
			"t4 0x%p, pfvf %u.\n", cdev, cdev->pfvf);
	} else 
	...
	rc = cxgbi_hbas_add(cdev, CXGB4I_MAX_LUN, max_conn, max_cmds,
	...
	pr_info("%s, 0x%p, lro %s, iso %s, sg_tablesize %u/%u, ddp %s.\n",
		ndev->name, cdev, (cdev->flags & CXGBI_FLAG_DEV_LRO_OFF) ?
			 "disabled" : "enabled",
		cdev->skb_iso_txhdr ? "enabled" : "disabled",
		cxgb4i_host_template.sg_tablesize, cxgb4i_sg_tablesize,
		(cdev->flags & CXGBI_FLAG_DDP_OFF) ? "off" : "on");
...
static void proc_ddp_status(struct cxgbi_sock *csk, struct cpl_rx_data_ddp *cpl,
					...
					const __be64 *rsp) 
{
	...
	proc_ddp_status(lro_cb->csk, cpl, pdu_cb);
				...
				const struct pkt_gl *gl)
{
	...
	u32 skb_frag_idx = ssi->nr_frags, i;
	unsigned int offset;
	unsigned int len;
			...
			const struct pkt_gl *gl, struct t4_lro_mgr *lro_mgr)
{
	...
	if (csk->cdev->flags & CXGBI_FLAG_DEV_LRO_OFF)
		return -EOPNOTSUPP;
	...
	lro_mgr->lro_merged++;
	csk->stats.lro_merged = lro_mgr->lro_merged;
	...
	if (skb) {
		...
		if (lro_mgr->lro_session_cnt >= MAX_LRO_SESSIONS ||
		    ((skb_shinfo(skb)->nr_frags + new_frag) >= MAX_SKB_FRAGS) ||
...
static int __init cxgb4i_init_module(void)
{
	...
	if (rc < 0)
		return rc;
	cxgb4_register_uld_type(CXGB4_ULD_ISCSI, &cxgb4i_uld_info);
...
static void __exit cxgb4i_exit_module(void)
{
	...
	cxgb4_unregister_uld_type(CXGB4_ULD_ISCSI);
