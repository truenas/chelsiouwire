...
ifeq ($(shell [ -e $(KINC)/net/inet_sock.h ] && echo 1),1)
  ...
  ifneq ($(shell $(grep) -c 'ireq_opt' $(KINC)/net/inet_sock.h),0)
    FLAGS += -DIP_OPT_VAR_IREQ_OPT
  endif
endif
...
ifneq ($(shell $(grep) -c 'FIELD_SIZEOF' $(KINC)/linux/kernel.h),0)
  FLAGS += -DHAS_FIELD_SIZEOF
endif

include $(srcdir)/common_flags.mk

ifeq ($(kversion),5)
cxgb4_cmd := $(CXGB4TOE_SRC)/cxgb4/.cxgb4_main.o.cmd
else
...
# kbuild cmd check does not work for RHEL 5
  ...
  ifeq ($(shell $(grep) -c 'T4_OS_NEW_FW_CAPS32' $(cxgb4_cmd)),1)
    FLAGS += -DT4_OS_NEW_FW_CAPS32
  endif
