...
/* Control flag to enable/disable Chelsio Flow offload functionality
*/
static bool en_offload = true;
module_param(en_offload, bool, 0644);
MODULE_PARM_DESC(en_offload, "Enable Chelsio Offload for OVS flows (default = true)");
...
struct sw_flow *ovs_flow_alloc(void)
{
	...
	flow->is_offloaded = false;
	memset(&flow->ofld_info, 0, sizeof(struct offload_info));
			...
			const struct sw_flow_mask *mask)
{
	...
	if (en_offload)
		cxgb_flow_offload_add(table, flow, mask);
