				...
				char *buf)
{
	...
	len += sprintf(buf + len,
			"\tiso: %d, lro: %d, ddp_pass: %d, ddp_fail: %d.\n",
			info->iso_cnt, info->lro_cnt, info->ddp_pass, info->ddp_fail);
                                 ...
                                 struct chiscsi_connection_info *conn_info)
{
	...
	conn_info->cid = conn->c_isock->s_tid;
	conn_info->ddp_pass = conn->c_isock->s_ddp_pass;
	conn_info->ddp_fail = conn->c_isock->s_ddp_fail;
	conn_info->lro_cnt = conn->c_isock->s_lro_cnt;
	conn_info->iso_cnt = conn->c_isock->s_iso_cnt;
