...
int iscsi_target_connection_close(iscsi_connection * conn)
{
	...
	/* Already connection is in closing state. */
	if (conn->c_state == CONN_STATE_CLOSING)
		return 0;
	...
	if (sess->s_type == ISCSI_SESSION_TYPE_NORMAL) {
		if (iscsi_sess_flag_test(sess, SESS_FLAG_FFP_BIT)) {
			...
			/* abort all outstanding scsi commands*/
				...
				if (!iscsi_scmd_state_abortable(sc) &&
...
static void it_session_check_scmd_acked(iscsi_session *sess)
{
...
static int rx_pdu_update_statsn(iscsi_pdu *pdu)
{
	...
	it_session_check_scmd_acked(sess);
...
static void conn_tx_align8(iscsi_connection *conn)
{
	unsigned int seq = 0;

	if (!iscsi_connection_get_tx_seq(conn, &seq) && (seq & 0x7U)) {

		os_log_info("conn 0x%p, seq 0x%x, crc %u,%u, send nop %d.\n",
			conn, seq, conn->c_hdigest_len, conn->c_ddigest_len,
			conn->c_hdigest_len ? 8 : 4);
		it_xmt_nop_in(conn, 0, 0, conn->c_hdigest_len ? 8 : 4, 0,
				NULL, NULL);
	}
	 ...
	 */
	if ((conn->c_hdigest_len + conn->c_ddigest_len) & 7) {
		conn->c_pdudatalen_tmax -= 4; 
		os_log_info("conn 0x%p, crc %u,%u, tx -> %u.\n",
			conn, conn->c_hdigest_len, conn->c_ddigest_len,
