...
int iscsi_connection_get_tx_seq(iscsi_connection *conn, unsigned int *snd_nxt)
{
	if (conn->c_isock)
		return os_socket_get_tx_seq(conn->c_isock, snd_nxt);
	return -EINVAL;
}
...
void iscsi_socket_state_change(iscsi_socket * isock, int tcpclosed)
{
	if (isock) {
		...
		os_log_debug((ISCSI_DBG_CONN),
			...
			"tcpclosed 0x%x.\n",
			conn, isock, isock->s_flag, tcpclosed);
		if (tcpclosed)
