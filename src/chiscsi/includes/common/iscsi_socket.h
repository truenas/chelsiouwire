...
#define ISCSI_SOCKET_PEER_FIN_WAIT	0x100
...
struct iscsi_socket {
	...
	unsigned int	s_ddp_fail;
	unsigned int	s_ddp_pass;
	unsigned int	s_lro_cnt;
	unsigned int	s_iso_cnt;
...
void 		iscsi_socket_state_change(iscsi_socket *, int);
...
int		os_socket_get_tx_seq(iscsi_socket *, unsigned int *);
