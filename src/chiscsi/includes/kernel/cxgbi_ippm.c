						...
						unsigned int *pcpu_ppmax)
{
	...
	bmap = ppmax / (sizeof(unsigned long) * BITS_PER_BYTE);
#else
		...
		unsigned int iscsi_edram_size)
{
	...
	if (iscsi_edram_size) {
		/* disable per cpu alloc, when iscsi ppod is in edram */
		reserve_factor = 0;
	...
	if (iscsi_edram_size) {
		...
		/* Mark the first ppod in DDR as reserved. This to avoid
		 * programming ppods that belong to EDRAM and DDR in a
		 * single command request, as it is not supported by hw.
		 */
		bitmap_set(ppm->ppod_bmap, first_ddr_idx, 1);
		pr_debug("reserved %d ppod in bitmap\n", first_ddr_idx);
	}
