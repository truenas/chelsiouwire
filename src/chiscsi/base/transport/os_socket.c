...
void os_socket_release(iscsi_socket * isock)
{
	...
	/* For listening connections, we need to destroy it here itself*/
	if (sk && sk_tx_closed(sk) && !osock->orig_state_change) {
		isock->s_flag |= ISCSI_SOCKET_TX_CLOSED;
		iscsi_socket_state_change(isock, 1);
	}
}
...
static void os_sock_state_change(struct sock *sk)
{
	...
	//os_log_debug(ISCSI_DBG_TRANSPORT,
	os_log_info(
		"%s: sk [0x%p, state change 0x%x, shut 0x%x, f 0x%x], "
		"isock [0x%p, 0x%p, 0x%p, f 0x%x]\n",
		__func__, sk, sk->sk_state, sk->sk_shutdown,
		sk->sk_flags, isock, isock ? isock->s_private : 0,
		isock ? isock->s_appdata : 0, isock ? isock->s_flag : 0);
		...
		if (osock) {
			...
			if (sk->sk_state != TCP_ESTABLISHED) {
				...
				os_log_info("%s: isock 0x%p, f 0x%x, sk 0x%p,"
					...
					sk->sk_state, sk->sk_flags, delay);

				iscsi_socket_state_change(isock,
					((sk->sk_state == TCP_CLOSE) && !delay));
...
#else
	rv = lsock->ops->accept(lsock, sock, O_NONBLOCK, false);
#endif
	if (rv < 0 || !sock->sk) {
		if (rv != -EAGAIN)
...
int os_socket_get_tx_seq(iscsi_socket *isock, unsigned int *snd_nxt)
{
