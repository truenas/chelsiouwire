...
static inline int blk_set_integrity_type(chiscsi_target_lun *lu)
{
	...
	if (lu && chiscsi_target_lun_flag_test(lu, LUN_T10DIX_BIT)) {
		...
		if (bdev && (bi = bdev_get_integrity(bdev))) {
			...
			bi_name = bi->profile->name;
#else
			bi_name = bi->name;
#endif
			if (!strcmp(bi_name, "T10-DIF-TYPE1-IP")) {
				dif_type = ISCSI_PI_DIF_TYPE_1;
				guard = ISCSI_PI_GUARD_TYPE_IP;
			} else if (!strcmp(bi_name, "T10-DIF-TYPE2-IP")) {
				dif_type = ISCSI_PI_DIF_TYPE_2;
				guard = ISCSI_PI_GUARD_TYPE_IP;
			} else if (!strcmp(bi_name, "T10-DIF-TYPE3-IP")) {
				dif_type = ISCSI_PI_DIF_TYPE_3;
				guard = ISCSI_PI_GUARD_TYPE_IP;
			} else if (!strcmp(bi_name, "T10-DIF-TYPE1-CRC")) {
				dif_type = ISCSI_PI_DIF_TYPE_1;
				guard = ISCSI_PI_GUARD_TYPE_CRC;
			} else if (!strcmp(bi_name, "T10-DIF-TYPE2-CRC")) {
				dif_type = ISCSI_PI_DIF_TYPE_2;
				guard = ISCSI_PI_GUARD_TYPE_CRC;
			} else if (!strcmp(bi_name, "T10-DIF-TYPE3-CRC")) {
				dif_type = ISCSI_PI_DIF_TYPE_3;
				guard = ISCSI_PI_GUARD_TYPE_CRC;
			}
		}
...
#endif
{
	...
	os_log_debug(ISCSI_DBG_SCSI,
		...
		__func__, bioc->itt, bio->bi_status, atomic_read(&bioc->count));
				...
				int dlen, int *sgoffset)
{
	...
	/* For dlen bytes of data in bio, how many bytes of pi to
	   pass in bip */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,14,0) || defined(SLES_RELEASE_15_0)
	bi = blk_get_integrity(bio->bi_disk);
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4,4,0))
	if (bi && bi->profile->verify_fn && (bi->flags & BLK_INTEGRITY_VERIFY))
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4,4,0))
	sc->pi_info.interval = (bi->interval_exp == 4096)?\
...
static int blk_scmd_execute(chiscsi_scsi_command *sc)
{
	...
	while (bio_head) {
		...
		bio->bi_next = NULL;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4,8,0) || defined(SLES_RELEASE_12_3))
