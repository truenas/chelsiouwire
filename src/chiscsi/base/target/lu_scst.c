...
extern bool chiscsi_context_thread;
...
int cb_chiscsi_scsi_cmd_cdb_rcved(chiscsi_scsi_command *scmd)
{
	...
	unsigned int opcode;
	enum scst_exec_context perf_context;
	...
	if (chiscsi_context_thread)
		perf_context = (scmd->sc_flag & SC_FLAG_WRITE) ?
				SCST_CONTEXT_DIRECT : SCST_CONTEXT_THREAD;
	else
		perf_context = SCST_CONTEXT_DIRECT;

	scst_cmd_init_done(scst_cmd, perf_context);
			...
			unsigned int xfer_offset, unsigned int xfer_buflen)
{
	...
	if (scmd->sc_flag & SC_FLAG_WRITE) {
		enum scst_exec_context perf_context = chiscsi_context_thread ?
		...
		scst_rx_data(cmd, SCST_RX_STATUS_SUCCESS, perf_context);
	}
	else {
		...
		scmd_fscsi_clear_bit(scmd, CH_SFSCSI_TIMER_SET_BIT);
	}
...
static int cb_chiscsi_scsi_cmd_abort(chiscsi_scsi_command *sc)
{
	...
	if (!cmd) {
		os_log_error("%s: sc:0x%x itt:0x%x s:%d scst cmd is NULL\n",
			 __func__, sc, sc->sc_itt, sc->sc_state);
		return -1;
	}
