...
unsigned int completion_on = 1;
module_param(completion_on, uint, 0644);
MODULE_PARM_DESC(completion_on, "iscsi completion, Only for T6 adapters (default 1: enabled)");

static unsigned int enable_dsgl = 1;
module_param(enable_dsgl, uint, 0644);
MODULE_PARM_DESC(enable_dsgl, "enable mem_write via dsgl (default 1: enabled)");
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,13,0) || defined(SLES_RELEASE_15_0)
	inet_csk(sk)->icsk_ack.lrcvtime = tcp_time_stamp_raw();
#else
...
static int do_rx_iscsi_data(struct tom_data *td, struct sk_buff *skb)
{
	...
	odev = isock_get_odev(isock);

	if (odev->d_flag & ODEV_FLAG_ULP_COMPL_ENABLED)
...
static void proc_ddp_status(iscsi_socket *isock, unsigned int val,
...
static void process_rx_iscsi_cmp(struct sock *sk, struct sk_buff *skb)
{
	...
	proc_ddp_status(isock, ntohl(cpl->ddpvld), &lcb->flags);
					...
					const __be64 *rsp) 
{
	...
	proc_ddp_status(isock, ntohl(cpl->ddpvld), &pdu_cb->flags);
...
add_packet:
	...
	lro_mgr->lro_merged++;
	isock->s_lro_cnt = lro_mgr->lro_merged;
...
static inline void ulp_mem_io_set_hdr(struct iscsi_socket *isock,
				struct ulp_mem_io *req,
				unsigned int wr_len, unsigned int dlen,
				unsigned int pm_addr, int idata)
{
	...
	unsigned int tid = isock->s_tid;
	
	INIT_ULPTX_WR(req, wr_len, 0, tid);

	req->cmd = htonl(V_ULPTX_CMD(ULP_TX_MEM_WRITE) |
			 V_ULP_MEMIO_ORDER(is_t4(lldi->adapter_type)) |
			 V_T5_ULP_MEMIO_ORDER(!is_t4(lldi->adapter_type) &&
			 !idata) |
			 V_T5_ULP_MEMIO_IMM(!is_t4(lldi->adapter_type) && idata) |
			 V_T5_ULP_MEMIO_FID((!is_t4(lldi->adapter_type) ?
					    lldi->rxq_ids[0] : 0)));
				...
				unsigned *sg_idx, unsigned int *sg_offset)
{
	...
	ulp_mem_io_set_hdr(isock, req, wr_len, dlen, pm_addr, 1);
				...
				dma_addr_t paddr)
{
	...
	req = (struct ulp_mem_io *)__skb_put(skb, wr_len);
	ulp_mem_io_set_hdr(isock, req, wr_len, dlen, pm_addr, 0);
...
{
	if (!(odev->d_flag & ODEV_FLAG_ULP_DSGL_ENABLED))
		return;
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,13,0) || defined(SLES_RELEASE_15_0)
			refcount_read(&ppm->refcnt.refcount));
#else
					...
					struct toedev *tdev)
{
	...
	/* Enable iscsi_completion by default for T6 adapters.
	 * This value will be overridden by module param completion_on.
	 * For T4/T5 adapters completion is disabled.
	 */
	if (completion_on && is_t6(td->lldi->adapter_type))
		odev->d_flag |= ODEV_FLAG_ULP_COMPL_ENABLED;

	if (enable_dsgl && td->lldi->ulptx_memwrite_dsgl &&
	    !is_t4(td->lldi->adapter_type)) {
		odev->d_flag |= ODEV_FLAG_ULP_DSGL_ENABLED;
		os_log_info("%s: ulp_mem_write via dsgl.\n",
			    pci_name(td->lldi->pdev));
	}
	...
	if (t4_ddp_init(odev) < 0) {
		offload_device_free(odev);
		return NULL;
	}
...
static void t4_sock_setup(iscsi_socket *isock, void *toedev)
{
	...
	if (odev->d_flag & ODEV_FLAG_ULP_ISO_ENABLED) {
		...
		isock->s_isomax = rounddown(cplios->txplen_max, tp->mss_cache);
