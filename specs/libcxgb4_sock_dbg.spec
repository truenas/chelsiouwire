...
find %{srcdir}/libcxgb4_sock_debug/ -name libcxgb4_sock_debug\* -type f -exec cp {} %{name}-%{version}%{lib} \;
...
%preun
if [ -f %{lib}/libcxgb4_sock_debug.so ]; then 
   unlink %{lib}/libcxgb4_sock_debug.so;
fi;
if [ -f %{lib}/libcxgb4_sock_debug.so.1 ]; then 
   unlink %{lib}/libcxgb4_sock_debug.so.1;
fi;

%post
ln -f -s  %{lib}/libcxgb4_sock_debug.so.1.0.0 %{lib}/libcxgb4_sock_debug.so
ln -f -s %{lib}/libcxgb4_sock_debug.so.1.0.0 %{lib}/libcxgb4_sock_debug.so.1
...
%files
%if %{udp_enable}
 /usr/lib64/libcxgb4_sock_debug.so.1
%endif
/usr/lib64/libcxgb4_sock_debug.so.1.0.0
/usr/lib64/libcxgb4_sock_debug.la
