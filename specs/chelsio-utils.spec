...
#if [ -f %{srcdir}/chelsio_adapter_config_v4/bin/chelsio_adapter_config.py ]; then
#     %{__cp} -a %{srcdir}/chelsio_adapter_config_v4/bin/chelsio_adapter_config.py %{name}-%{version}/sbin/
#     echo "%{bin}/chelsio_adapter_config.py" >> %{rpmfiles}
#fi;
...
if [ -f %{srcdir}/chelsio_adapter_config_v4/bin/t6seeprom ]; then
    %{__cp} -a %{srcdir}/chelsio_adapter_config_v4/bin/t6seeprom %{name}-%{version}/sbin/
    echo "%{bin}/t6seeprom" >> %{rpmfiles}
fi; 

for i in  eth_check eth_throughput eth_check.sh ; do 
  if [ -f %{srcdir}/checktool/${i} ]; then
    %{__cp} -a %{srcdir}/checktool/${i} %{name}-%{version}/sbin/
    echo "%{bin}/${i}" >> %{rpmfiles}
  fi;
done
...
#for dbins in chelsio_adapter_config.py t5seeprom t6seeprom ; do
for dbins in t5seeprom t6seeprom ; do
    if [ -f ${dbins} ]; then
        %{__install} -D -v -m 755 ${dbins} %{buildroot}%{bin}/${dbins}
    fi;
done;

for dbins in eth_check eth_throughput eth_check.sh ; do
    if [ -f ${dbins} ]; then
        %{__install} -D -v -m 755 ${dbins} %{buildroot}%{bin}/${dbins}
    fi;
done;
