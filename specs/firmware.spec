...
if [ -f %{fwdir}/t6fw.bin ] && [ ! -h %{fwdir}/t6fw.bin ]; then
    %{__mv} -f %{fwdir}/t6fw.bin %{fwdir}/t6fw.bin_bak ;
fi ;
if [ -h %{fwdir}/t6fw.bin ]; then
    /bin/rm -f %{fwdir}/t6fw.bin ;
fi ;
...
for file in $(/bin/ls %{fwdir}/*.bin 2>/dev/null); do
    ...
    if [ $cfirm == "/lib/firmware/cxgb4/t6fw" ] ; then
        /bin/ln -s %{fwdir}/$(basename $file) %{fwdir}/t6fw.bin
    fi 
done 
...
if [ "$1" = "0" ] ; then
    ...
    if [ -h %{fwdir}/t6fw.bin ] ; then
         %{__rm} -f %{fwdir}/t6fw.bin 2>/dev/null;
    fi 
    ...
    if [ -f %{fwdir}/t6fw.bin_bak ]; then
         %{__mv} %{fwdir}/t6fw.bin_bak %{fwdir}/t6fw.bin ;
    fi
fi

%prep
%{__mkdir} -p %{name}-%{version}/%{fwdir}/config
...
%{__cp} -a %{toolsdir}/chelsio_adapter_config_v4/vpds/*.bin %{name}-%{version}%{fwdir}/config/

for file in $(/bin/ls %{_topdir}/BUILD/%{name}-%{version}%{fwdir}/*.bin 2>/dev/null); do
  echo "%{fwdir}/$(basename $file)" >> %{rpmfiles}
done
for file in $(/bin/ls %{_topdir}/BUILD/%{name}-%{version}%{fwdir}/config/*.bin 2>/dev/null); do
  echo "%{fwdir}/config/$(basename $file)" >> %{rpmfiles}
done
...
for file in $(/bin/ls %{_topdir}/BUILD/%{name}-%{version}%{fwdir}/config/*.bin 2>/dev/null); do
  %{__install} -D -v -m 644 $file %{buildroot}/%{fwdir}/config/$(basename $file)
done
for file in $(/bin/ls %{_topdir}/BUILD/%{name}-%{version}%{fwdir}/*.txt 2>/dev/null); do
  %{__install} -D -v -m 644 $file %{buildroot}/%{fwdir}/$(basename $file)
done
