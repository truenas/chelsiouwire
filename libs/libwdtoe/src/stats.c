...
static void dump_stats(int s, struct sockaddr_un *sunp)
{
	...
	for (i = 0; i < NWDTOECONN; i++) {
		...
		if (c->tcp_state > TCP_IDLE) {
			...
			cc = sprintf(buf, "sockfd %d iface %s lhost %s:%u fleq %u fliq %u txq %u tid %u "
					  "rhost %s:%u state %s "
					  "fast_sends %llu "
					  "fast_recvs %llu "
					  "waits %llu\n",
					  ...
					  wd_dev->fl_list[c->port_num]->qid,
					  wd_dev->iq_list[c->port_num]->qid,
					  wd_dev->txq_list[c->port_num]->qid,
					  c->tid,
