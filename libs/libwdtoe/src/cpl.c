			     ...
			     u8 opcode)
{
	...
	wdtoe_add_rem_tid(tid, WDTOE_TID_REMOVE);
	return 0;
}
...
static int do_cpl_abort_req_rss(const __be64 *rsp, const __be64 *rsp_end,
				u8 opcode)
{
	unsigned int tid;
	const struct cpl_abort_req_rss *req;

	req = (struct cpl_abort_req_rss *)rsp;
	tid = GET_TID(req);

	DBG(DBG_CONN, "received CPL_ABORT_REQ_RSS (%#x) for tid %d\n", opcode,
	    tid);
	if (is_neg_adv(req->status)) {
                DBG(DBG_CONN, "Neg advice on connection - tid %u status %d\n",
                        tid, req->status);
                goto out;
        }
...
static int (*cpl_handler[NUM_CPL_CMDS])(const __be64 *rsp, const __be64 *rsp_end,
			    u8 opcode) = {
	...
	[CPL_ABORT_REQ_RSS]	= do_cpl_abort_req_rss,
};
