...
int connect(int sockfd, const struct sockaddr *addr, socklen_t addrlen)
{
	...
	wdtoe_add_rem_tid(wd_dev->stack_info->conn_info[idx].tid,
			  WDTOE_TID_INSERT);
out:
	return connfd;
}
			...
			socklen_t *addrlen)
{
	...
	flags |= wd_dev->stack_info->conn_info[idx].wd_sock_flags;
...
int shutdown(int sockfd, int how)
{
	...
	if (iq_created) {
		...
		DBG(DBG_CONN, "in shutdown(), socket [%d]\n", sockfd);
		/* remove the entry */
		...
		if (ret == -1) {
			 ...
			 * so try to find it in the listen server tbl
			 */
			DBG(DBG_CONN, "in shutdown(), socket [%d], not a data "
					"socket, trying listening sockets\n",
					sockfd);
			ret = remove_listen_svr(priv_svr_info,
						sockfd, &listen_port);
...
int fcntl(int fd, int cmd, ... /* arg */ )
{
	...
	case F_SETFL:
		...
		if (flag & O_NONBLOCK) {
			wd_dev->stack_info->conn_info[idx].wd_sock_flags
							|= O_NONBLOCK;
			DBG(DBG_CONN, "for sockfd [%d], setting wd_dev"
				"->stack_info->conn_info[%d].wd_sock_flags"
				" with O_NONBLOCK\n", fd, idx);
		}
		va_end(largp);
		break;
