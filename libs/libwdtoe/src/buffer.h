...
static inline void *next_buffer(struct sw_t4_txq *q)
{
...
static inline int sw_txq_next_pidx(struct sw_t4_txq *q, int cur_pidx)
{
	assert(q);
...
static inline void finish_buffer(struct sw_t4_txq *q, size_t copied)
{
	assert(q && q->queue);
...
static inline void credit_enqueue(int idx, struct sw_cred_q_entry cdqe)
{
...
static inline void credit_dequeue(int idx, int credits)
{
	int n_bufs = 0;
